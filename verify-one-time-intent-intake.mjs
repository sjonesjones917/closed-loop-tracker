...
project.projectData.userEntered={productConstraint:'The prompt box must retain its established dimensions.',acceptance:{oneTimeSupply:'Project information is supplied once and remains available to every later stage.'}};
...
const capture={schema:'closed-loop-stage01-capture/1',inputVersion:intake.inputVersion,manifestSha256:intake.manifestSha256,units:intake.units.map((unit,index)=>({sourceUnitId:unit.unitId,sourceRawValueSha256:unit.rawValueSha256,disposition:'incorporated into the job definition',reason:'Preserved once as controlling project authority for downstream reuse.',extractedStatements:[{statementKey:`statement-${String(index+1).padStart(3,'0')}`,text:unit.rawValueText,statementClass:'REQUIREMENT'}]}))};
...
const incompleteCapture=structuredClone(capture);incompleteCapture.units.pop();
...
project.stages[1].agentData={EXACT_DELIVERABLE_REQUESTED:'A working application conforming to every captured project requirement.',ASSUMPTIONS:'NONE',UNKNOWN_INFORMATION:'NONE',INPUT_SET_CONTENTS:JSON.stringify(capture)};
...
project.projectData.sources=[{id:'SOURCE-000001',stage:2,active:true,scope:{inputVersion:'INPUT-v001',sourceSetVersion:'SOURCE-SET-v001'},fields:{SOURCE_ID:'SOURCE-000001',TITLE:'Controlling external source',CONTROLLING_STATE:'CONTROLLING'}}];
project.projectData.research=[{id:'RESEARCH-000001',stage:3,active:true,scope:{inputVersion:'INPUT-v001',sourceSetVersion:'SOURCE-SET-v001'},fields:{RESEARCH_ID:'RESEARCH-000001',SOURCE_ID:'SOURCE-000001',PASS_NUMBER:2,EXACT_PORTION_EXAMINED:'Complete applicable source',MANDATORY_STATEMENTS:'External mandatory obligation retained from Stage 03.',RECOMMENDATIONS:'External recommendation retained from Stage 03.',OPTIONAL_PRACTICES:'NONE',EXAMPLES:'NONE',EXPLANATORY_MATERIAL:'NONE',PROHIBITIONS:'External prohibition retained from Stage 03.',EXCEPTIONS:'NONE',DEPENDENCIES:'NONE',APPLICABILITY_FACTS:'Applies to this project.',RESTRICTIONS:'NONE',INVALIDATING_MATERIAL:'NONE',FINDING_CLASSIFICATION:'COMPLETE',SOURCE_EVIDENCE:'SOURCE-000001 complete review',SATURATION_STATUS:'SATURATED'},relationships:{SOURCE_ID:'SOURCE-000001'},evidenceRefs:[]}];
project.projectData.candidateRequirements=[{id:'CANDIDATE-REQ-000001',stage:3,active:true,scope:{inputVersion:'INPUT-v001',sourceSetVersion:'SOURCE-SET-v001'},fields:{CANDIDATE_REQ_ID:'CANDIDATE-REQ-000001',SOURCE_ID:'SOURCE-000001',SOURCE_LOCATION:'Applicable section',CANDIDATE_OBLIGATION:'Candidate external obligation retained from Stage 03.',CLASSIFICATION:'MANDATORY',APPLICABILITY:'APPLICABLE',DEPENDENCIES:'NONE',EVIDENCE:'SOURCE-000001'},relationships:{SOURCE_ID:'SOURCE-000001'},evidenceRefs:[]}];
project.stages[2].agentData={SOURCE_APPLICABILITY_DETERMINATION:'APPLICABLE',KNOWN_CONTROLLING_SOURCES_EXAMINED:'TRUE',UNRESOLVED_CONTROLLING_CONFLICTS:'NONE'};project.stages[2].status='COMPLETE';project.stages[2].gate={complete:true,blocked:false,reasons:[]};
project.stages[3].agentData={ALL_KNOWN_CONTROLLING_SOURCES_EXAMINED:'TRUE',SECOND_CONFLICT_AND_EXCEPTION_PASS_COMPLETED:'TRUE',LATEST_PASS_NUMBER:2,NEW_MATERIAL_CATEGORY_FOUND_IN_LATEST_PASS:'FALSE',RESEARCH_GAPS_AND_BLOCKERS:'NONE'};project.stages[3].status='COMPLETE';project.stages[3].gate={complete:true,blocked:false,reasons:[]};
...
assert(stage3Prompt.prompt.includes('Never ask the human to repeat, retype, summarize, resend, reopen, or reattach project information already present'),'Stage 03 permits repeated project-data entry.');
...
const obligations=engine.obligationManifest(project);const obligationText=obligations.items.map(item=>item.text);
for(const required of ['The prompt box must retain its established dimensions.','Project information is supplied once and remains available to every later stage.','External mandatory obligation retained from Stage 03.','External recommendation retained from Stage 03.','External prohibition retained from Stage 03.','Candidate external obligation retained from Stage 03.'])assert(obligationText.includes(required),`Stage 04 obligation universe omitted: ${required}`);

const stage4Prompt=prompts.buildPromptRecord(4,project,{operation:'COMPLETE'});const exhausted=stage4Prompt.contextManifest.stage4ExhaustedInputs;
...
assert(stage4Prompt.prompt.includes('Do not request, reopen, attach, resend, restate, summarize, or retype the original intent file'),'Stage 04 permits repeated original-intent transfer.');
...
console.log(JSON.stringify({oneTimeIntentCapture:true,intakeManifestComplete:true,stage03ReceivesCurrentProjectAuthority:true,stage04ReceivesAllStage01AndStage03Data:true,noRepeatIntentTransfer:true,incompleteDownstreamAccountingRejected:true}));
