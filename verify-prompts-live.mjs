...
try{
  ...
  await poll(async()=>{const ok=await evaluate(cdp,`document.body.innerText.includes('1/30 complete')&&globalThis.closedLoopPromptEngine?.version==='2026-08-24-r2'&&globalThis.closedLoopAuthorityGuard?.revision==='closed-loop-authority-20260824-r1'`);if(!ok)throw new Error('waiting for retained project, corrected prompt engine, and authority guard');return true;});
  ...
  for(let n=1;n<=30;n++){
    ...
    assert(prompt.includes('PROJECT-SCOPE BOUNDARY'),`Stage ${n} prompt lacks project-scope boundary`);
    assert(prompt.includes('This instruction belongs only to JOB_ID JOB-20260823144121'),`Stage ${n} prompt is not explicitly bound to the current JOB_ID`);
    assert(prompt.includes('Do not instruct the operator to reuse this prompt for other jobs.'),`Stage ${n} prompt permits cross-job prompt reuse`);
    generated.push(prompt);
  }
  const stage1=generated[0];
  for(const token of ['Initialize only this current job','Do not create, prescribe, or instruct reuse of a master prompt','Do not infer requirements for unrelated jobs'])assert(stage1.includes(token),`Rendered Stage 01 prompt missing current-job isolation control: ${token}`);
  ...
  for(const token of ['USER-SUPPLIED / KNOWN AUTHORITY:','User Job Input controls scope and intent but is not automatically an independent external authority.','User Job Input controls requested scope and intent. It is not automatically an independent external governing source.'])assert(stage2.includes(token),`Stage 02 prompt does not preserve the distinction between user-supplied authority claims and established independent external authority: ${token}`);
  const stage3=generated[2];
  for(const token of ['Research only SOURCE_ID records legitimately established as independent External Governing Sources in Stage 02','Do not research the target product, operating application, repository, prior implementation, or project-generated artifacts as requirement authority','Research only the legitimate Stage 02 external governing source set'])assert(stage3.includes(token),`Rendered Stage 03 prompt missing non-circularity control: ${token}`);
  for(const [n,token] of [[4,'Compile atomic requirements for this current job'],[5,'Resolve the current job’s requirement set'],[6,'Build this job’s verification suite before any production instruction is authored'],[7,'Build this job’s MUTATION-SUITE-vN'],[8,'Author this job’s production instruction only from the resolved current requirement set'],[9,'Preflight this job’s production instruction'],[10,'Freeze the exact candidate for this job and iteration'],[11,'Run exactly ten independent executions for this job'],[12,'REQ_ID × RUN_ID'],[13,'Compare all ten executions for this job requirement-by-requirement'],[14,'Root-cause every material defect in this job'],[15,'Convert every confirmed failure in this job into permanent regression data'],[16,'Revise only the responsible earliest defective layer for this job'],[17,'complete new ten-execution iteration for this job'],[18,'Convergence exists only when required coverage and regression success are 100 percent'],[19,'unchanged confirmation iteration'],[20,'Freeze this job’s production baseline only after successful unchanged confirmation'],[21,'Generate this job’s finished target product here'],[22,'Run deterministic verification against the actual finished product bytes'],[23,'independent meaning/content verification on this job’s actual finished product'],[24,'Perform adversarial verification on this job’s finished product'],[25,'Inspect this job’s exact final delivered representation and package'],[26,'process evidence and product evidence as two separate propositions'],[27,'produce exactly one determination: ACCEPTED, REJECTED, or BLOCKED'],[28,'verify exact artifact identity immediately before release'],[29,'EXTERNAL SOURCE / USER AUTHORITY AS APPLICABLE -> REQUIREMENT -> INSTRUCTION -> EXECUTION'],[30,'append-only defect and regression history']])assert(generated[n-1].includes(token),`Rendered Stage ${n} prompt is missing its corrected controlling procedure: ${token}`);
  ...
  console.log(JSON.stringify({renderedPromptsVerified:30,projectScopeBoundaryVerified:30,stage1CurrentJobOnly:true,stage2ProjectSpecific:true,retainedSourceSetNotApplicable:true,nonCircularAuthorityBoundary:true,circularSourceRejected:true,externalSourceAccepted:true,stage3SourceLinkEnforced:true,allStageProceduresCurrent:true,jobId:'JOB-20260823144121'},null,2));
  cdp.close();
}finally{
  proc.kill('SIGTERM');
}
