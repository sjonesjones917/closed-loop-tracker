...
for(const token of ['STRICT RESPONSE CONTRACT','PROMPT IDENTITY','MANDATORY RESPONSE RULES','PROJECT-SCOPE BOUNDARY','independent external sources','Exhaust the current accepted Stage 02 source universe before Stage 04','initial extraction pass','explicit conflict/exception/completeness pass','latest complete pass must find no new material category'])if(!prompts.includes(token))throw new Error(`Canonical prompt contract missing: ${token}`);
