...
for(const token of ['STRICT RESPONSE CONTRACT','PROMPT IDENTITY','MANDATORY RESPONSE RULES','PROJECT-SCOPE AND CAPABILITY BOUNDARY','SUFFICIENCY AND RECOVERY CHECK','genuinely independent external governing sources','Research only the legitimate Stage 02 external governing source set'])if(!prompts.includes(token))throw new Error(`Canonical prompt contract missing: ${token}`);
