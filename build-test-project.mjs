...
if(fs.existsSync('AUTHORIZED_OPERATION_01.txt'))throw new Error('Obsolete retained-project source materialization artifact remains.');
for(const verifier of ['verify.mjs','verify-live.mjs'])if(fs.readFileSync(verifier,'utf8').includes('AUTHORIZED_OPERATION_01.txt'))throw new Error(`${verifier} still depends on a duplicate retained-project source artifact.`);
...
console.log(JSON.stringify({singleApplicationShell:true,stages:30,retainedJobId:project.jobId,currentStage:2,stage1:'COMPLETE',downstreamFabricated:false,responseSchema:'closed-loop-stage-response/3',obsoleteRuntimeWrappers:false,obsoleteSourceMaterialization:false,verifiersUseCanonicalRetainedState:true},null,2));
