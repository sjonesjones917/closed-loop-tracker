...
for(const token of ['STRICT RESPONSE CONTRACT','PROMPT IDENTITY','MANDATORY RESPONSE RULES','PROJECT-SCOPE BOUNDARY','independent external sources','current accepted Stage 02 independent external source set','A supplied project artifact is project input, not automatically an independent authority.'])if(!prompts.includes(token))throw new Error(`Canonical prompt contract missing: ${token}`);
