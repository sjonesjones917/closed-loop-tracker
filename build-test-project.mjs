...
const scriptTags=[...html.matchAll(/<script\s+defer\s+src="([^"]+)"><\/script>/g)].map(match=>match[1]);
if(scriptTags.length!==runtimeScripts.length)throw new Error(`Expected ${runtimeScripts.length} direct deferred runtime scripts; found ${scriptTags.length}.`);
if(JSON.stringify(scriptTags.map(value=>value.split('?')[0]))!==JSON.stringify(runtimeScripts))throw new Error('Runtime script order is wrong.');
if(new Set(scriptTags.map(value=>value.split('?')[1])).size!==1)throw new Error('Runtime scripts do not share one build token.');
if(fs.existsSync('app.js')||/document\.write/.test(html+app))throw new Error('Dynamic script injection remains.');
