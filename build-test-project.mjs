...
for(const token of ['RESPONSE CONTRACT DEFINITIONS','MANDATORY RESPONSE RULES','PROJECT-SCOPE BOUNDARY','independent external sources','Read and execute the attached instruction.txt as the complete controlling task.'])if(!prompts.includes(token))throw new Error(`Canonical prompt contract missing: ${token}`);
