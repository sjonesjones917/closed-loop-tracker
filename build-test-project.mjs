...
const orderedScripts=['workbook.js','hash.js','workflow-schema.js','workflow-engine.js','prompt-engine.js','response-ingestion.js','project-store.js','app-core.js'];
...
for(const token of ['Parse / validate response','Proposed extracted changes','Accept response','Reject response','Request correction','Human-owned stage input','Application-derived job control','Before the first instruction','This setup message is not an agent prompt','Talk if needed','Paste final JSON'])if(!app.includes(token))throw new Error(`Human-facing ingestion UI missing: ${token}`);
for(const token of ['Talk to the agent normally.','final JSON code block','Use the code block’s Copy control'])if(!html.includes(token))throw new Error(`Collapsible human guide missing: ${token}`);
...
console.log(JSON.stringify({singleApplicationShell:true,stages:30,retainedJobId:project.jobId,currentStage:2,stage1:'COMPLETE',downstreamFabricated:false,responseSchema:'closed-loop-stage-response/2',obsoleteRuntimeWrappers:false,humanFirstAgentLoop:true},null,2));
