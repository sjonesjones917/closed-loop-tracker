...
const required=['index.html','app-core.js','hash.js','workflow-schema.js','test-runtime.js','test-worker.js','workflow-engine.js','prompt-engine.js','response-ingestion.js','project-store.js','workbook.js','TEST_PROJECT.json','AUTHORIZED_OPERATION_01.txt','verify.mjs','verify-live.mjs','verify-browser.mjs','verify-ingestion.mjs'];
...
if(!/worker-src 'self'/.test(html))throw new Error('Same-origin worker permission is missing from CSP.');
...
for(const token of ['closed-loop-stage-response/3','PRODUCER','STAGE_CONTRACTS','sourceClassificationIssues','HUMAN_INTAKE_FIELDS','closed-loop-test-spec/1'])if(!schema.includes(token))throw new Error(`Ownership/response schema control missing: ${token}`);
for(const token of ['strictParse','validateEnvelope','PENDING_OPERATOR_REVIEW','ACCEPTED_DATA_CHANGE','extractionManifests','answerHumanInput','INCOMPLETE_ACCOUNTING'])if(!ingestion.includes(token))throw new Error(`Transactional ingestion control missing: ${token}`);
for(const token of ['STRICT RESPONSE CONTRACT','PROMPT IDENTITY','MANDATORY RESPONSE RULES','PROJECT-SCOPE BOUNDARY','PROJECT MEMORY — CAPTURE ONCE, REUSE EVERYWHERE','APPLICATION-OWNED INTAKE COVERAGE MANIFEST','APPLICATION-OWNED OBLIGATION MANIFEST'])if(!prompts.includes(token))throw new Error(`Canonical prompt contract missing: ${token}`);
...
const activeSource=html+app+prompts+schema+ingestion+fs.readFileSync('test-runtime.js','utf8')+fs.readFileSync('workflow-engine.js','utf8')+fs.readFileSync('project-store.js','utf8')+fs.readFileSync('workbook.js','utf8');
...
if(globalThis.closedLoopCore?.PROJECT_SCHEMA!=='closed-loop-project/3')throw new Error('Runtime project schema is wrong.');
...
if(globalThis.closedLoopTestRuntime?.SPEC_VERSION!=='closed-loop-test-spec/1')throw new Error('Runtime Test IR schema is wrong.');
console.log(JSON.stringify({singleApplicationShell:true,stages:30,retainedJobId:project.jobId,currentStage:2,stage1:'COMPLETE',downstreamFabricated:false,projectSchema:'closed-loop-project/3',responseSchema:'closed-loop-stage-response/3',testIrSchema:'closed-loop-test-spec/1',obsoleteRuntimeWrappers:false},null,2));