...
for(const token of ['STRICT RESPONSE CONTRACT','PROMPT IDENTITY','MANDATORY RESPONSE RULES','PROJECT-SCOPE BOUNDARY','independent external source inventory','strongest source type for each proposition','Research only the current accepted Stage 02 independent external source set'])if(!prompts.includes(token))throw new Error(`Canonical prompt semantic control missing: ${token}`);
