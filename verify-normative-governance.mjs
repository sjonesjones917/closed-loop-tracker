...
const readJson=path=>{try{return JSON.parse(fs.readFileSync(path,'utf8'));}catch(error){fail(`${path} is not valid UTF-8 JSON: ${error.message}`);}};

for(const path of [SPEC_PATH,SPEC_MANIFEST_PATH,REQUIREMENTS_PATH])if(!fs.existsSync(path))fail(`Missing required repository-governance artifact: ${path}`);
