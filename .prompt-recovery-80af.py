...
"- Work too large for the actually available environment, including large repositories or specialist CAD/CAM/simulation/manufacturing work without the required tool access, must never be represented as completed. At Stage 01, when a complete reliable self-contained substitute deliverable can satisfy the underlying objective, propose that substitute as EXACT_DELIVERABLE_REQUESTED for human confirmation rather than blocking the whole job. After Stage 01, return WORK_TOO_LARGE_FOR_ENVIRONMENT or MISSING_CAPABILITY only when the currently approved deliverable itself cannot be completed reliably; otherwise complete the approved self-contained deliverable without claiming unavailable implementation or fabrication.")
