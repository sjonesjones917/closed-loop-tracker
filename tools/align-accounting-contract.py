...
old="const prompt4=prompts.buildPromptRecord(4,p);"
new="const prompt4=prompts.buildPromptRecord(4,p);p.projectData.generatedPrompts.push({...prompt4,generatedAt:new Date().toISOString(),invalidatedBy:null});p.stages[4].currentPromptId=prompt4.instructionId;"
if old not in s: raise SystemExit('Stage4 prompt declaration anchor missing')
s=s.replace(old,new,1)
