...
schema=replaceOnce(schema,
"EXPECTED_REVISION:{type:'INTEGER',producer:'APPLICATION',nullable:false},CHALLENGE_NONCE:{type:'STRING',producer:'APPLICATION',nullable:false}",
"EXPECTED_REVISION:{type:'INTEGER',producer:'APPLICATION',nullable:false},RESERVATION_REVISION:{type:'INTEGER',producer:'APPLICATION',nullable:false},CHALLENGE_NONCE:{type:'STRING',producer:'APPLICATION',nullable:false}",
'operation reservation revision field');
...
const regression=`import fs from 'node:fs';\nimport vm from 'node:vm';\nimport assert from 'node:assert/strict';\n\nglobalThis.crypto=globalThis.crypto||await import('node:crypto').then(m=>m.webcrypto);\nglobalThis.TextEncoder=globalThis.TextEncoder||await import('node:util').then(m=>m.TextEncoder);\nfor(const file of ['workbook.js','hash.js','workflow-schema.js','test-runtime.js','workflow-engine.js'])vm.runInThisContext(fs.readFileSync(new URL('./'+file,import.meta.url),'utf8'),{filename:file});\nconst schema=globalThis.closedLoopWorkflowSchema,engine=globalThis.closedLoopWorkflowEngine;\nassert.deepEqual(schema.CONTROLLING_COMPLETION_ENUMS.reservation,['RESERVED','EXPORTED','ORPHANED','RESUMED','RESPONSE_STAGED','ACCEPTED','REJECTED','CANCELLED','SUPERSEDED','EXPIRED_BY_SCOPE']);\nconst p={revision:7,activeStage:1,job:{JOB_ID:'JOB-RESERVATION-TEST',CONTRACT_PROFILE_ID:'closed-loop-completion-profile/1'},projectData:{},stages:{}};engine.ensureShape(p);\nconst scope={projectRevision:8,inputVersion:'INPUT-v001'};\nconst target=engine.reservationTargetSlot(p,{stage:1,operation:'COMPLETE',scope});\nassert.match(target,/^[a-f0-9]{64}$/);\nassert.throws(()=>engine.reserveOperation(p,{stage:1,operation:'COMPLETE',scope,targetSlot:'caller-owned'}),/application-calculated/);\nconst r=engine.reserveOperation(p,{stage:1,operation:'COMPLETE',scope,owningTabInstance:'TAB-A',payload:{kind:'test'}});\nassert.equal(r.STATUS,'RESERVED');assert.equal(r.EXPECTED_REVISION,8);assert.equal(r.RESERVATION_REVISION,8);assert.equal(r.SCOPE.projectRevision,8);assert.equal(r.TARGET_SLOT,target);assert.match(r.CHALLENGE_NONCE,/^[a-f0-9]{32}$/);\nassert.equal(engine.reserveOperation(p,{stage:1,operation:'COMPLETE',scope,owningTabInstance:'TAB-A',payload:{kind:'test'}}).id,r.id,'exact retry must return the existing reservation');\nengine.transitionOperationReservation(r,'EXPORTED');engine.transitionOperationReservation(r,'RESPONSE_STAGED');engine.transitionOperationReservation(r,'ACCEPTED');assert.equal(r.STATUS,'ACCEPTED');assert.throws(()=>engine.transitionOperationReservation(r,'RESERVED'),/Invalid operation-reservation transition/);\nconsole.log(JSON.stringify({reservationContract:'PASS',targetSlot:target,reservationRevision:r.RESERVATION_REVISION,finalState:r.STATUS}));\n`;
