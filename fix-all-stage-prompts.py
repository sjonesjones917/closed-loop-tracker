...
for old,new in repls.items(): text=text.replace(old,new)
...
p=Path('workbook.js'); text=p.read_text(); text=text.replace("'Every relied-upon supplied file was inspected'","'Supplied project material was not misclassified as independent external authority'",1); p.write_text(text)

p=Path('verify-definition-of-done.mjs'); text=p.read_text().replace("closed-loop-project/2","closed-loop-project/3").replace("closed-loop-stage-response/2","closed-loop-stage-response/3"); p.write_text(text)

p=Path('verify-prompt-semantics.mjs'); text=p.read_text()
oldhonesty="Never claim that a web search, repository edit, build, test, CAD operation, simulation, CNC post-processing step, physical measurement, fabrication, filing, submission, or other external action occurred unless it actually occurred"
newhonesty="Never claim that a search, repository edit, build, test, specialized tool operation, external-system operation, physical observation, submission, or other external action occurred unless it actually occurred"
text=text.replace(oldhonesty,newhonesty)
for line in [
