...
const source=fs.readFileSync('workflow-engine.js','utf8');assert(source.includes('releaseVerificationTrustFailures'),'releaseMetrics is not wired to release-grade verification trust');assert(source.includes('evaluateResultConsistency'),'Central result adjudication is missing');assert(source.includes('effectiveDetermination'),'Effective determination reducer is missing');assert(!source.includes("['SATISFIED','SUCCESS','PASSED'].includes(upper(recordValue(latest,'RESULT')))"),'Legacy regression success shortcut remains');assert(!source.includes(".map(v=>upper(recordValue(v,'DETERMINATION')))"),'Stability diagnostics still trust submitted determinations');assert(source.includes('capabilityAvailability(project,mode,requiredCapability)'),'Execution routing does not require affirmative capability evidence');
