...
// Stage 25 uses one strict structured coverage semantics for both aggregate coverage and effective determination.
{
 ...
 p.projectData.propositions.push({id:'PROP-REP',stage:4,active:true,scope:representationScope,fields:{PROPOSITION_ID:'PROP-REP',PROPOSITION_TEXT:'Every required final representation element is present and inspectable.'},relationships:{REQUIREMENT_ID:'REQ-1'}});
 p.projectData.tests.push({id:'TEST-REP',stage:6,active:true,scope:representationScope,fields:{TEST_ID:'TEST-REP',REQ_ID:'REQ-1',TEST_TYPE:'FINAL_REPRESENTATION',EXECUTION_MODE:'APPLICATION_DETERMINISTIC',TEST_ROLE:'ADVISORY',REQUIRED_CAPABILITY:'application representation coverage evaluator',ARTIFACT_REQUIREMENTS:'NONE',PROCEDURE:'Compare the exact contracted representation identity sets.',EXPECTED_RESULT:'SATISFIED',FAILURE_CONDITION:'A required representation identity is missing.',EVIDENCE_TO_PRESERVE:'Application-normalized coverage observation and entailment.',REQUIRED_EPISTEMIC_BASES:['APPLICATION_OBSERVED'],STATUS:'READY'},relationships:{REQ_ID:'REQ-1',TARGET_PROPOSITION_IDS:['PROP-REP']}});
 ...
 const good=record('representationInspections',{ARTIFACT_ID:'ART-REP',TEST_ID:'TEST-REP',PROPOSITION_ID:'PROP-REP',OBSERVATION_ID:'OBSERVATION-REP',ENTAILMENT_ID:'ENTAILMENT-REP',OBSERVATIONS:structuredObservation,RENDERING_OPENING_EVIDENCE:'EVIDENCE-REP',DETERMINATION:'SATISFIED',APPLICATION_DETERMINATION:'SATISFIED'},{stage:25,scope:{productId:'PRODUCT-REP'},evidenceRefs:['EVIDENCE-REP']});
 good.id='INSPECTION-REP-GOOD';good.relationships={ARTIFACT_ID:'ART-REP',TEST_ID:'TEST-REP',PROPOSITION_ID:'PROP-REP',OBSERVATION_ID:'OBSERVATION-REP',ENTAILMENT_ID:'ENTAILMENT-REP'};p.projectData.representationInspections.push(good);
 p.projectData.observationRecords.push({id:'OBSERVATION-REP',stage:25,active:true,scope:representationScope,fields:{OBSERVATION_ID:'OBSERVATION-REP',ORIGIN_CLASS:'NATIVE_APPLICATION_OBSERVATION',SUBMITTING_ACTOR:'APPLICATION_REPRESENTATION_EVALUATOR',SUBJECT_DESCRIPTION:'Exact representation coverage for INSPECTION-REP-GOOD.',NATIVE_OBSERVED_VALUE:{coverage:'COMPLETE'},NATIVE_OBSERVED_LOCATION:'FINAL_REPRESENTATION',NATIVE_METHOD_OR_TOOL:'APPLICATION_REPRESENTATION_EVALUATOR',EPISTEMIC_BASIS:'APPLICATION_OBSERVED',FRESHNESS_STATUS:'CURRENT',STATUS:'CURRENT'},relationships:{RESULT_ID:'INSPECTION-REP-GOOD',TEST_ID:'TEST-REP',PROPOSITION_ID:'PROP-REP',SOURCE_EVIDENCE_IDS:['EVIDENCE-REP']},evidenceRefs:['EVIDENCE-REP']});
 p.projectData.entailmentReviews.push({id:'ENTAILMENT-REP',stage:25,active:true,scope:representationScope,fields:{ENTAILMENT_ID:'ENTAILMENT-REP',ENTAILMENT_FINDING:'ESTABLISHES',ACCEPTED_RELATION:'ESTABLISHES',REASONING:'Exact equality of every contracted representation identity set establishes the coverage proposition.',STATUS:'ACCEPTED'},relationships:{OBSERVATION_ID:'OBSERVATION-REP',PROPOSITION_ID:'PROP-REP'}});
 const effective=engine.evaluateResultConsistency('representationInspections',good,p.projectData.tests.find(item=>item.id==='TEST-REP'),p);assert(effective.determination==='SATISFIED','Strict Stage 25 coverage JSON was not accepted by effective determination: '+JSON.stringify(effective.reasons));
 ...
 const broken=engine.evaluateResultConsistency('representationInspections',good,p.projectData.tests.find(item=>item.id==='TEST-REP'),p);assert(broken.determination!=='SATISFIED','Incomplete Stage 25 structured coverage was accepted by effective determination');
 p.projectData.entailmentReviews.pop();p.projectData.observationRecords.pop();p.projectData.representationInspections.pop();p.projectData.tests.pop();p.projectData.propositions.pop();p.projectData.evidenceRecords.pop();p.projectData.artifacts.pop();delete p.job.CURRENT_PRODUCT_ID;
}
...
// Evidence-authority regressions: each invalid state must fail, then a repaired state must pass.
{
 ...
 const meaningTest=record('tests',{TEST_TYPE:'MEANING',TEST_ROLE:'ADVISORY',EXECUTION_MODE:'INDEPENDENT_AGENT_REVIEW',REQUIRED_CAPABILITY:'independent meaning review',ARTIFACT_REQUIREMENTS:'NONE',PROCEDURE:'Compare required and observed meaning.',EXPECTED_RESULT:'meaning matches',EVIDENCE_TO_PRESERVE:'meaning comparison'});
 const incompleteMeaning=record('meaningResults',{PRODUCT_LOCATION:'section 1',REQUIRED_MEANING:'RIGHT',OBSERVED_MEANING:'WRONG',EVIDENCE_BASED_COMPARISON:''},{stage:0,evidenceRefs:['EVIDENCE-MEANING']});
 ...
 const completeMeaning=record('meaningResults',{PRODUCT_LOCATION:'section 1',REQUIRED_MEANING:'RIGHT',OBSERVED_MEANING:'RIGHT',EVIDENCE_BASED_COMPARISON:'The observed statement has the same controlling meaning.'},{stage:0,evidenceRefs:['EVIDENCE-MEANING']});
 ...
 const humanTest=record('tests',{TEST_TYPE:'HUMAN_INSPECTION',TEST_ROLE:'ADVISORY',EXECUTION_MODE:'HUMAN_INSPECTION',REQUIRED_CAPABILITY:'human observation',ARTIFACT_REQUIREMENTS:'NONE',PROCEDURE:'A human opens and inspects the representation.',EXPECTED_RESULT:'representation acceptable',EVIDENCE_TO_PRESERVE:'human-owned observation'});
 ...
 assert(!humanEvidence.sufficient&&humanEvidence.requiredEvidenceClasses.includes('EXACT_ARTIFACT_BOUND_INSPECTION_RESERVATION'),'Filename-only human evidence bypassed exact artifact reservation binding');

 const q=core.createBlankState('JOB-HUMAN-INSPECTION-REPAIR');Object.assign(q.job,{CURRENT_INPUT_VERSION:'INPUT-v001',CURRENT_SOURCE_SET_VERSION:'SOURCE-SET-v001',CURRENT_REQUIREMENTS_VERSION:'REQUIREMENTS-v001',CURRENT_TEST_SUITE_VERSION:'TEST-SUITE-v001',CURRENT_INSTRUCTION_VERSION:'INSTRUCTION-v001',CURRENT_PRODUCT_ID:'PRODUCT-HUMAN'});engine.ensureShape(q);q.activeStage=25;const qs=engine.currentScope(q);
 q.projectData.products.push({id:'PRODUCT-HUMAN',stage:21,active:true,scope:{...qs,productId:'PRODUCT-HUMAN'},fields:{PRODUCT_ID:'PRODUCT-HUMAN',PRODUCT_VERSION:'v1',GENERATED_ARTIFACT_INVENTORY:['ART-HUMAN'],STATUS:'COMPLETED'}});
 q.projectData.artifacts.push({id:'ART-HUMAN',stage:21,active:true,scope:{...qs,productId:'PRODUCT-HUMAN'},fields:{ARTIFACT_ID:'ART-HUMAN',FILENAME:'representation.pdf',VERSION:'v1',BYTE_SIZE:8,SHA256:'c'.repeat(64),AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED'},relationships:{PRODUCT_ID:'PRODUCT-HUMAN'}});
 const boundHumanTest={id:'TEST-HUMAN-BOUND',stage:6,active:true,scope:{...qs},fields:{TEST_ID:'TEST-HUMAN-BOUND',REQ_ID:'REQ-1',TEST_TYPE:'HUMAN_INSPECTION',TEST_ROLE:'ADVISORY',RELEASE_BEARING:false,EXECUTION_MODE:'HUMAN_INSPECTION',REQUIRED_CAPABILITY:'human observation',ARTIFACT_REQUIREMENTS:'CURRENT PRODUCT',INPUTS:'current product bytes',PROCEDURE:'Open and inspect the exact current product.',EXPECTED_RESULT:'representation acceptable',EVIDENCE_TO_PRESERVE:'human-owned exact-byte observation',REQUIRED_EPISTEMIC_BASES:['SELF_ASSERTED'],STATUS:'READY'},relationships:{REQ_ID:'REQ-1'}};q.projectData.tests.push(boundHumanTest);
 const boundEvidence=engine.recordHumanInspectionEvidence(q,{testId:'TEST-HUMAN-BOUND',observation:'The exact bound representation was opened and inspected.',evidenceDescription:'Direct exact-artifact human inspection.'}),boundObservation=q.projectData.observationRecords.find(item=>String(item.relationships?.TEST_ID||'')==='TEST-HUMAN-BOUND');
 q.projectData.entailmentReviews.push({id:'ENTAILMENT-HUMAN-BOUND',stage:25,active:true,scope:{...qs},fields:{ENTAILMENT_ID:'ENTAILMENT-HUMAN-BOUND',OBSERVATION_ID:engine.recordId(boundObservation,'observationRecords'),PROPOSITION_ID:null,PROOF_OBLIGATION_ID:null,PROOF_LEAF_KEY:'TEST-HUMAN-BOUND',ENTAILMENT_FINDING:'ESTABLISHES',REASONING:'The direct observation establishes the inspection leaf.',REVIEWER_ID:'HUMAN_OPERATOR',EVIDENCE_IDS:[engine.recordId(boundEvidence,'evidenceRecords')],ACCEPTED_RELATION:'ESTABLISHES',CURRENT_SCOPE:qs,STATUS:'CURRENT'},relationships:{OBSERVATION_ID:engine.recordId(boundObservation,'observationRecords'),EVIDENCE_IDS:[engine.recordId(boundEvidence,'evidenceRecords')]}});
 const boundHumanResult={id:'INSPECTION-HUMAN-BOUND',stage:0,active:true,scope:{...qs},fields:{PRODUCT_SHA256:'c'.repeat(64)},evidenceRefs:[engine.recordId(boundEvidence,'evidenceRecords')]};humanEvidence=engine.evaluateEvidenceSufficiency(q,{test:boundHumanTest,result:boundHumanResult});
 assert(humanEvidence.sufficient,'Exact reserved human observation did not repair human-inspection sufficiency: '+humanEvidence.reasons.join(' | '));
}
