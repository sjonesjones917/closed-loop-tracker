...
function semanticIssues(record){
  ...
  if(record.stage===1){
    if(!record.prompt.includes('HUMAN INTERACTION AND FINAL APP HANDOFF')||!record.prompt.includes('ask the smallest necessary question(s) in normal concise language')||!record.prompt.includes('Do not output the JSON response envelope yet')||!record.prompt.includes('Use HUMAN_INPUT_REQUIRED only when the current environment cannot conduct that conversation'))issues.push('STAGE01_INTERACTIVE_CLARIFICATION_MISSING');
...
if(schema.STAGE_OPERATIONS[19].includes('CONFIRM_FREEZE')||schema.operationContract(19,'CONFIRM_FREEZE'))throw new Error('Stage 19 still exposes application-owned freeze as an agent response operation.');
{
 ...
 if(!ui.includes('answer normally in that same chat')||!ui.includes('do not bring intermediate questions back to this app')||!ui.includes('How to run this stage')||!ui.includes('whole final reply when it contains exactly one fenced json block'))throw new Error('Operator UI does not explain conversational clarification and final app handoff.');
...
// Stage 01 must not create a machine instruction before the minimum human objective exists.
{
 ...
 const patent=baseProject();patent.job.EXACT_USER_OBJECTIVE_VERBATIM='I need a patent application for my project.';const record=prompts.buildPromptRecord(1,patent,{operation:'COMPLETE'});if(!record.prompt.includes('ASCII U+0022')||!record.prompt.includes('never use typographic/curly quotation marks'))throw new Error('Strict JSON quote syntax is not explicit.');for(const token of ['HUMAN INTERACTION AND FINAL APP HANDOFF','ask the smallest necessary question(s) in normal concise language','The strict response-envelope rules below apply to the FINAL APP HANDOFF','exactly one fenced code block labelled json'])if(!record.prompt.includes(token))throw new Error(`Interactive clarification/final handoff rule missing: ${token}`);if(record.prompt.includes('Do not ask conversational questions outside the JSON response'))throw new Error('Stage 01 still suppresses normal interactive clarification.');
}
