...
const collapsedPromptRule=html.match(/\.expandable-prompt\{([^}]*)\}/)?.[1]||'';
const expandedPromptRule=html.match(/\.expandable-prompt\.expanded\{([^}]*)\}/)?.[1]||'';
assert(/(?:^|;)height:280px(?:;|$)/.test(collapsedPromptRule)&&/(?:^|;)max-height:280px(?:;|$)/.test(collapsedPromptRule),'Collapsed prompt preview must remain exactly 280px.');
assert(/(?:^|;)height:auto(?:;|$)/.test(expandedPromptRule)&&/(?:^|;)max-height:none(?:;|$)/.test(expandedPromptRule),'Expanded prompt preview must reveal the complete prompt without a height limit.');
