...
function semanticIssues(record){
  ...
  if(record.stage===1){
    if(!record.prompt.includes('STAGE 01 SUBJECT-NEUTRAL INTAKE ADAPTATION')||!record.prompt.includes('Do not perform source discovery, source research, requirement derivation, verification design, production-instruction authoring, implementation, artifact production'))issues.push('STAGE01_DOMAIN_INTAKE_BOUNDARY_MISSING');
  ...
  }else if(!record.prompt.includes('SUBJECT-NEUTRAL STAGE ADAPTATION')||!record.prompt.includes('never claim an operation occurred without evidence'))issues.push('ENVIRONMENT_LIMIT_RULE_MISSING');
  ...
  if(record.stage===1){
    ...
    if(!record.prompt.includes('MANDATORY STAGE 01 HUMAN-INTAKE GATE')||!record.prompt.includes('BLOCKING_NOW')||!record.prompt.includes('ASK_NOW_NONBLOCKING')||!record.prompt.includes('LATER_RESOLVABLE')||!record.prompt.includes('Nonblocking means the human may defer; it does not mean the agent may skip the question'))issues.push('STAGE01_PROACTIVE_HUMAN_INTAKE_GATE_MISSING');
...
}
{
 ...
 if(!r.prompt.includes('STAGE 01 SUBJECT-NEUTRAL INTAKE ADAPTATION')||!r.prompt.includes('APPLICATION-ENUMERATED INTAKE COVERAGE MANIFEST'))throw new Error('Stage 01 subject-neutral exhaustive intake adaptation is missing.');
...
// stage01-practical-intake-regression-v1
{
 ...
 const required=[
  ...
  'APPLICATION-ENUMERATED INTAKE COVERAGE MANIFEST',
  'Do not use a hard-coded project-subject catalogue',
