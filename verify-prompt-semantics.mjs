...
assert(!promptIdentityRecord.prompt.includes(promptIdentityRecord.bodySha256),'The instruction must not embed its own digest.');
assert(globalThis.closedLoopHash.sha256Text(promptIdentityRecord.prompt)===promptIdentityRecord.bodySha256,'bodySha256 must hash every exported instruction byte.');
assert(promptIdentityRecord.fullTextSha256===promptIdentityRecord.bodySha256,'Prompt byte identities must be identical.');
assert(promptIdentityRecord.prompt.endsWith('\n'),'The authoritative instruction requires its final newline.');
assert(globalThis.closedLoopPromptEngine.promptFileManifest(promptIdentityRecord).promptIdentity.bodySha256===promptIdentityRecord.bodySha256,'The manifest must carry the exact prompt identity.');
...
assert(html.includes('.expandable-prompt{max-height:280px}.expandable-prompt.expanded{max-height:80vh}'),'Prompt preview/collapse sizing changed.');
