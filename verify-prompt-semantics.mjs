...
function semanticIssues(record){
  ...
  for(const forbidden of ['PATENT / REGULATED FILING','SOFTWARE / MULTI-FILE SYSTEM','BUILDING / ARCHITECTURE / AEC','PHYSICAL / MECHANICAL / CAD / CAM / CNC / ADDITIVE'])if(record.prompt.includes(forbidden))issues.push(`PROHIBITED_DOMAIN_BRANCH_${forbidden}`);
  if(record.stage===1){
    if(!record.prompt.includes('Stage 01 is COMPLETE HUMAN-AUTHORITY INTAKE')||!record.prompt.includes('account for EVERY INPUT UNIT ID')||!record.prompt.includes('BLOCKING_NOW')||!record.prompt.includes('ASK_NOW_NONBLOCKING')||!record.prompt.includes('LATER_RESOLVABLE'))issues.push('STAGE01_EXHAUSTIVE_SUBJECT_NEUTRAL_INTAKE_MISSING');
    if(!record.prompt.includes('Do not perform source research')||!record.prompt.includes('requirement atomization')||!record.prompt.includes('test design'))issues.push('STAGE01_LATER_WORK_BOUNDARY_MISSING');
