...
if(schema.STAGE_OPERATIONS[19].includes('CONFIRM_FREEZE')||schema.operationContract(19,'CONFIRM_FREEZE'))throw new Error('Stage 19 still exposes application-owned freeze as an agent response operation.');
{
 ...
 if(!ui.includes('Start with the verbatim job request')||!ui.includes('answer it in that ChatGPT conversation')||!ui.includes('do not re-enter the answer here')||!ui.includes('Paste only the final JSON from the agent'))throw new Error('Stage 01 operator UI does not explain the single human-first ChatGPT intake path.');
