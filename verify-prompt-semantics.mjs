...
function semanticIssues(record){
  ...
  if(!record.prompt.includes('Artifact-generation capability and downstream execution/consumption/verification capability are separate'))issues.push('ARTIFACT_GENERATION_EXECUTION_BOUNDARY_MISSING');
  if(!record.prompt.includes('Successful artifact generation proves only what was actually established about the generated bytes'))issues.push('ARTIFACT_GENERATION_NOT_EXECUTION_PROOF_MISSING');
  ...
  if(record.stage===1){
    ...
    if(!record.prompt.includes('absence of a conventional downstream authoring or consuming application does not by itself make a documented artifact format ungeneratable')||!record.prompt.includes('Generate the actual requested artifact whenever its required representation can be constructed reliably here'))issues.push('STAGE01_ARTIFACT_GENERATION_BOUNDARY_MISSING');
  }
  if(record.stage===6){
  ...
  if(record.stage===8&&(!record.prompt.includes('distinguish what is required to generate its bytes from what is required only to open, import, compile, execute, simulate, manufacture, submit, physically test, or otherwise verify/use it downstream')||!record.prompt.includes('not by itself a reason to replace a reliably generatable artifact with prose')))issues.push('STAGE08_GENERATION_DOWNSTREAM_REQUIREMENTS_MISSING');
  ...
  if(record.stage===21&&(!record.prompt.includes('Do not downgrade an actual artifact to prose merely because the conventional downstream')||!record.prompt.includes('Treat generation as separate from downstream interoperability, execution, simulation, fabrication, submission, physical use, and validation')))issues.push('STAGE21_REAL_ARTIFACT_GENERATION_MISSING');
  if(record.stage===2){
...
const mutants=[
  ...
  {...original,prompt:original.prompt.replace('Artifact-generation capability and downstream execution/consumption/verification capability are separate','Artifact generation requires the downstream application')},
  {...original,prompt:original.prompt.replace('Successful artifact generation proves only what was actually established about the generated bytes','Successful artifact generation proves downstream execution')},
...
}
{
 ...
 if(!r.prompt.includes('patent-application drafts')||!r.prompt.includes('implementation-ready multi-file specification')||!r.prompt.includes('documented engineering interchange/source/vector/model files'))throw new Error('Specialist deliverable generation/fallback coverage is missing.');
}
{
