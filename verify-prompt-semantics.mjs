...
function semanticIssues(record){
  ...
  }else if(!record.prompt.includes('Work too large for the actually available environment must not be represented as completed')||!record.prompt.includes('Browser-stored artifact bytes are not automatically accessible to an external agent.'))issues.push('ENVIRONMENT_LIMIT_RULE_MISSING');
...
// Contract identity must bind the complete stage/record validation contract, not only field names.
{
 ...
 if(descriptor.contractVersion!=='closed-loop-response-contract/3.0')throw new Error('Versioned response-contract descriptor is missing.');
 ...
 if(!record.prompt.includes('RESPONSE CONTRACT DEFINITIONS')||!record.prompt.includes('closed-loop-response-contract/3.0'))throw new Error('The agent cannot inspect the exact contract descriptor whose hash it must echo.');
...
}
{
 ...
 if(!r.prompt.includes('STAGE 01 SUBJECT-NEUTRAL ZERO-LOSS INTAKE')||!r.prompt.includes('Derive subject-specific human-only questions solely from the actual request, accessible supplied materials, and current canonical context. Do not use project-subject branches.')||!r.prompt.includes('Enumerate and preserve every meaning-bearing human unit.')||!r.prompt.includes('For every inputUnitId in the application intake manifest'))throw new Error('Stage 01 subject-neutral exhaustive intake behavior is missing.');
...
// stage01-practical-intake-regression-v1
{
 ...
 const required=[
  ...
  'Derive subject-specific human-only questions solely from the actual request, accessible supplied materials, and current canonical context. Do not use project-subject branches.',
