...
function semanticIssues(record){
  ...
  for(const prohibited of ['PATENT / REGULATED FILING','SOFTWARE / MULTI-FILE SYSTEM','BUILDING / ARCHITECTURE / AEC','PHYSICAL / MECHANICAL / CAD / CAM / CNC / ADDITIVE'])if(record.prompt.includes(prohibited))issues.push('PROJECT_SUBJECT_BRANCH_LEAK_'+prohibited);
  if(!record.prompt.includes('PROJECT-SUBJECT-NEUTRAL'))issues.push('PROJECT_SUBJECT_NEUTRAL_ALGORITHM_MISSING');
  if(record.stage===1){
...
}
{
 ...
 if(!r.prompt.includes('PROJECT-SUBJECT-NEUTRAL INTAKE SEMANTICS'))throw new Error('Stage 01 project-subject-neutral intake algorithm is missing.');
...
// stage01-practical-intake-regression-v2 — project-specific facts are fixture data, never runtime branches.
{
 const p=baseProject();
...
if(prompts.version!=='closed-loop-prompt-engine/28')throw new Error('Persisted prompts were not invalidated after the controlling prompt-engine repair.');
