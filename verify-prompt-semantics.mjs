...
function semanticIssues(record){
  ...
  if(record.prompt.includes('Do not ask conversational questions outside the JSON response')||record.prompt.includes('Missing human-authority information requires HUMAN_INPUT_REQUIRED')||record.prompt.includes('Ask for clarification only when an irreducible human fact or decision is needed now'))issues.push('MACHINE_FIRST_HUMAN_CLARIFICATION_CONTRADICTION');
...
if(schema.STAGE_OPERATIONS[19].includes('CONFIRM_FREEZE')||schema.operationContract(19,'CONFIRM_FREEZE'))throw new Error('Stage 19 still exposes application-owned freeze as an agent response operation.');
{
 ...
 if(!ui.includes('Talk first; paste JSON last.')||!ui.includes('Answer agent questions')||!ui.includes('Paste final JSON')||!ui.includes('Do not copy conversational questions into this app'))throw new Error('Stage 01 operator UI does not explain the human conversation before final JSON.');
