...
try{
  ...
  const scopeNarrative=`${loadedSelf.name||''}\n${JSON.stringify(loadedSelf.job||{})}\n${String(loadedSelf.stages?.[0]?.completionEvidence||'')}`;
  if(/\b(existing )?v13\b|version 13|sidecar-filename defect|repair-task tracker|fix stage|app-arbitrary application version/i.test(scopeNarrative))throw new Error('Completed application project is still defined as a version/repair project instead of the complete application.');
  ...
  if(await destructive.count()){
    const forcedDelete=destructive.first();
    await forcedDelete.evaluate(control=>{control.hidden=false;if('disabled' in control)control.disabled=false});
    await forcedDelete.click({force:true});
    await page.waitForTimeout(100);
    const retainedAfterDeleteAttempt=await page.evaluate(id=>{try{return JSON.parse(localStorage.getItem('closedLoopReliability.projects')||'[]').some(project=>project.projectId===id)}catch{return false}},selfProject.projectId);
    if(!retainedAfterDeleteAttempt||!(await page.locator('[data-retained-application-project="true"]').isVisible()))throw new Error('Completed application project was removed by a forced destructive-control attempt.');
  }
  ...
  if((await page.locator('[data-self-project-proof="true"]').count())!==1||(await page.locator('[data-retained-application-project="true"]').count())!==1)throw new Error('Completed application project proof or native card disappeared after opening the project.');
  if(!/31\/31 stages complete/i.test(await page.locator('#retainedProjectStatus').innerText()))throw new Error('Completed application project proof lost its completion state after the application rerendered.');
