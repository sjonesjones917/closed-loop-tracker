...
# Prompt: preserve one authority, bind the changed behavior to a new prompt-engine version,
# and pass exact run/operation scope into the shared handoff derivation.
p=Path('prompt-engine.js'); s=p.read_text()
s=s.replace("const PROMPT_ENGINE_VERSION='closed-loop-prompt-engine/19';","const PROMPT_ENGINE_VERSION='closed-loop-prompt-engine/20';",1)
...
# Engine: one deterministic handoff authority, contextual execution guidance, no new canonical collection.
p=Path('workflow-engine.js'); s=p.read_text(); start=s.index('function executionHandoff('); end=s.index('function evidenceChainExplanation(',start)
...
# UI: show execution guidance only when it is actually actionable, and expose direct provenance hops.
p=Path('app-core.js'); s=p.read_text(); start=s.index('function testExecutionGuidanceMarkup('); end=s.index('function interactionModeMarkup(',start)
...
start=s.index('function provenanceMarkup('); end=s.index('function workflow(){',start)
...
'''
s=s[:start]+new_prov+s[end:]; p.write_text(s)

# Semantic regression: guidance remains contextual and every provenance trace exposes the full audit path.
p=Path('verify-prompt-semantics.mjs'); s=p.read_text(); needle="if(!ui.includes('Invalid application executor claim')||!ui.includes('No registered application-native executor exists'))throw new Error('Operator UI does not fail unsupported application-native test claims closed.');"
...
# Browser regression: compact per-value trace cards replace the obsolete single giant provenance disclosure.
p=Path('verify-browser.mjs'); s=p.read_text(); a=s.index(' // Every long open record section uses the same matched top/bottom controls, including provenance.'); b=s.index(' // Reload persistence and next-stage prompt consumes accepted Stage 02 data.',a)
replacement=r''' // Provenance is intentionally split into compact per-value trace cards instead of one giant disclosure.
...
# Shared runtime identity after production-byte changes.
