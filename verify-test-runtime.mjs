...
// Stage 04 consumes canonical Stage 01 + Stage 03 state. A filename in Stage 01's
// supplied-material inventory is not an application-observed requirement to re-upload bytes.
...
Object.assign(project.job,{JOB_ID:'JOB-STAGE04-CANONICAL-INPUT',EXACT_USER_OBJECTIVE_VERBATIM:'Compile the accepted researched obligations into atomic requirements.',SUPPLIED_MATERIALS_INVENTORY:'text 30(1).txt',CURRENT_INPUT_VERSION:'INPUT-v001',CURRENT_SOURCE_SET_VERSION:'NOT APPLICABLE',CURRENT_REQUIREMENTS_VERSION:'NOT APPLICABLE',CURRENT_TEST_SUITE_VERSION:'NOT APPLICABLE',CURRENT_INSTRUCTION_VERSION:'NOT APPLICABLE',CURRENT_STAGE:'STAGE 04',CURRENT_STATE:'IN PROGRESS'});project.activeStage=4;

// Exhaust Stage 01 against the exact application-owned intake manifest.
const intake=engine.buildIntakeCoverageManifest(project);
project.stages[1].agentData={EXACT_DELIVERABLE_REQUESTED:'Atomic requirement specification compiled from the complete supplied project authority.',ASSUMPTIONS:'NONE',UNKNOWN_INFORMATION:'NONE',INPUT_SET_CONTENTS:JSON.stringify({units:intake.units.map((unit,index)=>({sourceUnitId:unit.unitId,disposition:'incorporated into the job definition',extractedStatements:[{statementKey:`S${index+1}`,text:unit.label==='EXACT_USER_OBJECTIVE_VERBATIM'?project.job.EXACT_USER_OBJECTIVE_VERBATIM:`Captured project input: ${unit.label}`,statementClass:unit.label==='EXACT_USER_OBJECTIVE_VERBATIM'?'REQUIREMENT':'FACT'}]}))})};
project.stages[1].status='COMPLETE';project.stages[1].gate={complete:true,blocked:false,reasons:[]};
assert.equal(engine.evaluateStage01IntakeAccounting(project).complete,true,'Stage 01 fixture must exhaust the application-owned intake manifest.');

// Establish the legitimate application-derived no-external-source path for Stage 03.
project.stages[2].agentData={SOURCE_APPLICABILITY_DETERMINATION:'NO_APPLICABLE_EXTERNAL_SOURCE',AUTHORITY_HIERARCHY:'No applicable independent external authority for this fixture.',KNOWN_CONTROLLING_SOURCES_EXAMINED:'The controlled fixture establishes no applicable external source.'};
project.stages[2].status='COMPLETE';project.stages[2].gate={complete:true,blocked:false,reasons:[]};
project.stages[3].agentData={EXCEPTIONS_AND_EDGE_CONDITIONS:'NONE',CONFLICTING_OR_INVALIDATING_MATERIAL:'NONE',RESEARCH_GAPS_AND_BLOCKERS:'NONE'};
project.stages[3].status='COMPLETE';project.stages[3].gate={complete:true,blocked:false,reasons:[]};
assert.equal(engine.evaluateStage03Exhaustion(project).complete,true,'Stage 03 fixture must be exhausted by application derivation before Stage 04 prompt generation.');
...
console.log(JSON.stringify({genericTestIr:true,stage01Exhausted:true,stage03Exhausted:true,stage04CanonicalInputBoundary:true,stage04OneTimeIntentReuse:true},null,2));
