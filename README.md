...
When an external agent returns an actual file, its response declares the attachment and the operator attaches the exact returned bytes in the stage file control. The application then verifies the actual filename/media type/byte size/SHA-256 and stores the Blob. If the response was already preserved and failed only because returned files were missing or mismatched, the operator can attach the files and retry validation of that same `RAW_RESPONSE_ID`; the exact raw bytes and raw SHA-256 remain unchanged, while every validation attempt remains preserved. A filename, hash claim, repository path, or code block alone never counts as possession of a file, and browser-local bytes are not assumed to be accessible to an external agent. Stage 06 also rechecks current canonical TEST → evidence → artifact custody, so later loss or loss of verified-byte status fails closed instead of leaving a stale test definition apparently ready.
