...
`index.html` and `app-v13.html` contain the same verified standalone application bytes. `app-payload/` retains the reproducible compressed source payload used by `build-app.mjs`. The build and verification pipeline rejects any divergence back to the obsolete prompt-relay interface.
...
`rebuild-self-project.mjs` reconstructs the retained project as a native `closed-loop-project/1` export. It preserves independently researched external sources, compiles atomic requirements and tests, executes isolated initial, corrected, and unchanged-confirmation browser batches, creates complete verification matrices, records a real source-of-truth defect and responsible-layer correction, freezes the accepted baseline, attaches the exact application bytes as the finished product, audits the process and product, verifies SHA-256 identity, and creates the release record.

`migrate-self-project.mjs` no longer rewrites legacy prose or substitutes invented terminology. It only validates an already current-schema project. A legacy retained project must be rebuilt into native records instead of being cosmetically renamed.
