...
## Human collaboration and final machine payload

Generated instructions use one interaction model across all 30 stages: the external agent first reads the available files and canonical context, then asks concise plain-language questions only for missing human-only facts or decisions. Stage 01 collects the human-only information already foreseeable from the job and domain. Source discovery, research, and requirement work may expose additional human-only facts or choices later; the stage that discovers the need asks then without restarting the project or repeating earlier questions. Public, common, technical, legal, and regulatory facts are established by the proper source/research stage rather than delegated to the human.

After the conversation is sufficient, the agent returns one final JSON object in a single `json` code block. The response field accepts raw JSON or that one code block. The parser preserves the exact raw response, deterministically unwraps the single block, and can normalize typographic JSON delimiters with explicit validation warnings. Prose around the block, multiple blocks, ambiguous JSON, stale prompt identity, and contract violations remain rejected before canonical mutation. `HUMAN_INPUT_REQUIRED` remains the structured fallback when conversation is unavailable, the human cannot answer, a file reference is missing, or an application-owned human field requires typed confirmation.
