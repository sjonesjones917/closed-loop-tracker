...
`build-test-project.mjs` is idempotent repository/deployment materialization and validation for the authorized retained project and targeted shared-renderer repairs. `verify.mjs` verifies the repository state. `verify-live.mjs` verifies the deployed state. The Pages workflow also renders the live application at 320 and 393 CSS-pixel widths.
