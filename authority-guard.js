...
const forbiddenClassification=new Set(['TARGET PRODUCT','CURRENT APPLICATION','REPOSITORY / IMPLEMENTATION','PROJECT ARTIFACT','USER JOB INPUT','SUPPLIED MATERIAL']);
function projects(){try{const v=JSON.parse(localStorage.getItem(STORE)||'[]');return Array.isArray(v)?v:[];}catch{return [];}}
function currentProject(){const list=projects(),n=Number(document.querySelector('#project-picker')?.value);return list[Number.isInteger(n)&&n>=0?n:0]||list[0]||null;}
...
function addField(form,collection,name,kind='input',options=[]){
  ...
  control.dataset.recordCollection=collection;control.dataset.recordField=name;control.autocomplete='off';
...
function reject(message,event){event.preventDefault();event.stopImmediatePropagation();alert(message);}
function validateSourceDraft(event){
  ...
  if(cls!=='EXTERNAL GOVERNING SOURCE')return reject('Stage 02 SOURCE_ID records are only for independent External Governing Sources. Preserve User Job Input and Supplied Material in their own project records.',event);
  if(independent!=='TRUE'||relation!=='INDEPENDENT EXTERNAL AUTHORITY')return reject('Confirm that this source is independent external authority and is not the target product, current application, repository, project artifact, User Job Input, or merely Supplied Material.',event);
  if(exactForbiddenReference.test(ref))return reject('Circular source rejected. The current application, repository, implementation files, retained project, and target-product artifacts cannot be Stage 02 governing sources.',event);
}
function validateDownstreamSourceLink(collection,event){
  ...
  if(!validExternalSource(record))reject(`${collection==='requirements'?'Requirement':'Research'} source rejected. SOURCE_ID must resolve to a valid independent External Governing Source established in Stage 02.`,event);
}
...
function wrapPromptEngine(){
  ...
  function wrapped(stage,state){
    ...
    if(Number(stage?.number)===3)prompt=prompt.replace('STAGE-SPECIFIC TASK\n','STAGE-SPECIFIC TASK\nResearch only SOURCE_ID records that were legitimately established as independent External Governing Sources in Stage 02. Do not research the target product, operating application, repository, prior implementation, or project-generated artifacts as requirement authority.\n\n');
    return prompt;
  }
  wrapped.__authorityGuard=REV;core.buildStagePrompt=wrapped;
}
function expose(){globalThis.closedLoopAuthorityGuard={revision:REV,validExternalSource};}
wrapPromptEngine();expose();enhanceForms();
const observer=new MutationObserver(()=>enhanceForms());observer.observe(document.documentElement,{childList:true,subtree:true});
})();
