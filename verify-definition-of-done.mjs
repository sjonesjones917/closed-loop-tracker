...
const artifactIdentityProofs=[['count-equality',engineSource.includes("if(a.length!==d.length)throw new Error('Audited and delivery artifact counts differ.')")],['missing-delivery-rejected',engineSource.includes("if(!right)throw new Error('Missing delivery artifact '")],['filename-equality',engineSource.includes('fields.AUDITED_FILENAME===fields.RELEASE_FILENAME')],['order-independent-regression',completeTestSource.includes('Artifact identity depends on file-selection order.')],['mismatched-bytes-regression',completeTestSource.includes('Mismatched release bytes were authorized.')],['stage28-current-batch',completeTestSource.includes('stage28CurrentBatch:true')],['full-cycle-identity',fullCycleSource.includes('engine.verifyArtifactIdentity(p')]];
