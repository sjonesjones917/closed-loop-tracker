...
export function savePrompt(project,stage,{operation=null,runId=null,contextId=null}={}){
  ...
  const slot=(engine.promptSlotKey?engine.promptSlotKey(record):[Number(record.stage||0),String(record.operation||'COMPLETE'),String(record.scope?.runId||''),String(record.scope?.contextId||'')].join('|'));for(const prior of (project.projectData.generatedPrompts||[]).filter(item=>(engine.promptSlotKey?engine.promptSlotKey(item):[Number(item.stage||0),String(item.operation||'COMPLETE'),String(item.scope?.runId||''),String(item.scope?.contextId||'')].join('|'))===slot&&!item.invalidatedBy))prior.invalidatedBy=`SUPERSEDED-BY-${record.instructionId}`;
