const fs=require('fs');
const path='project-store.js';
const before=fs.readFileSync(path,'utf8');
const old=`function openDatabase(){
...
const replacement=`function indexedDbOpenAttempt(){
...
if(!before.includes(old))throw new Error('Expected openDatabase implementation not found; refusing non-exact repair.');
if((before.match(/function openDatabase\(\)/g)||[]).length!==1)throw new Error('openDatabase owner is not unique; refusing repair.');
