...
const scopeIdentity=value=>{const text=String(value??'').trim();return !text||['NONE','NOT APPLICABLE','UNKNOWN','PENDING','UNASSIGNED'].includes(upper(text))?null:text;};
function currentScope(project){return {inputVersion:scopeIdentity(project.job.CURRENT_INPUT_VERSION),sourceSetVersion:scopeIdentity(project.job.CURRENT_SOURCE_SET_VERSION),requirementsVersion:scopeIdentity(project.job.CURRENT_REQUIREMENTS_VERSION),testSuiteVersion:scopeIdentity(project.job.CURRENT_TEST_SUITE_VERSION),instructionVersion:scopeIdentity(project.job.CURRENT_INSTRUCTION_VERSION),iterationId:scopeIdentity(recordId(latestIteration(project,[10,17,19]),'iterations')||project.job.CURRENT_ITERATION),candidateId:scopeIdentity(recordId(records(project,'candidateFreezes').at(-1),'candidateFreezes')),baselineId:scopeIdentity(recordId(records(project,'baselines').at(-1),'baselines')||project.job.CURRENT_BASELINE_ID),productId:scopeIdentity(recordId(records(project,'products').at(-1),'products')||project.job.CURRENT_PRODUCT_ID)};}
...
function gate(stage,project){
  ...
  switch(stage){
    ...
    case 2:{
      requireAccepted();
      const noSource=upper(project.stages[2]?.agentData?.AUTHORITY_HIERARCHY).startsWith('NO_APPLICABLE_EXTERNAL_SOURCE')&&truth(project.stages[2]?.agentData?.KNOWN_CONTROLLING_SOURCES_EXAMINED);
      if(!collection('sources').length&&!noSource)reasons.push('At least one inspected independent external governing source or an evidence-supported NO_APPLICABLE_EXTERNAL_SOURCE determination is required.');
      if(collection('sources').length&&noSource)reasons.push('NO_APPLICABLE_EXTERNAL_SOURCE cannot coexist with current external source records.');
    ...
    case 3:{
      ...
      if(noSource){if(sourceIds.length)reasons.push('Stage 02 no-source determination conflicts with current source records.');if(collection('research').length)reasons.push('Stage 03 research must remain empty when external source research is not applicable.');break;}
    ...
    case 15:{requireAccepted();const defects=confirmedDefects(project).filter(d=>Number(d.stage)===15||Number(d.stage)===14),regs=collection('regressions'),covered=new Map(regs.map(r=>[String(recordValue(r,'DEFECT_ID')||r.relationships?.DEFECT_ID||''),r]));const missing=defects.filter(d=>!covered.has(recordId(d,'defects'))).map(d=>recordId(d,'defects'));if(missing.length)reasons.push('Permanent regression definitions are missing for: '+missing.join(', ')+'.');const executions=collection('regressionExecutions');for(const reg of regs){const id=recordId(reg,'regressions');if(!executions.some(e=>String(recordValue(e,'REG_ID')||e.relationships?.REG_ID||'')===id&&upper(recordValue(e,'PHASE'))==='PRE_CORRECTION'&&['VIOLATED','FAILED','FAIL'].includes(upper(recordValue(e,'RESULT')))))reasons.push('Regression '+id+' lacks an actual pre-correction failing execution.');}break;}
    ...
    case 19:{requireAccepted();const iteration=latestIteration(project,[19]);const ev=evaluateIteration(project,recordId(iteration,'iterations'),'UNCHANGED_CONFIRMATION');if(!ev.complete)reasons.push(...ev.reasons);const priorCandidate=records(project,'candidateFreezes',{stage:17}).at(-1),currentCandidate=records(project,'candidateFreezes',{stage:19}).at(-1);const identity=c=>hash.sha256Value({versions:recordValue(c,'COMPONENT_VERSIONS')||{},hashes:recordValue(c,'COMPONENT_HASHES')||{}});if(!priorCandidate||!currentCandidate||identity(priorCandidate)!==identity(currentCandidate))reasons.push('Stage 19 candidate versions and hashes are not exactly unchanged from Stage 17.');requireCount('confirmationRecords',1);if(collection('confirmationRecords').some(r=>upper(recordValue(r,'DETERMINATION'))!=='SATISFIED'))reasons.push('Unchanged confirmation is not affirmatively satisfied.');break;}
    ...
    case 21:{requireAccepted();const product=collection('products').at(-1),baseline=all('baselines').at(-1);if(!product)reasons.push('One current application-reserved product execution is required.');if(!baseline)reasons.push('An approved production baseline is required.');if(product&&baseline&&String(recordValue(product,'BASELINE_ID'))!==recordId(baseline,'baselines'))reasons.push('The current product is not bound to the current approved baseline.');const contextId=String(recordValue(product,'PRODUCTION_CONTEXT_ID')||''),context=all('freshContexts').find(r=>recordId(r,'freshContexts')===contextId&&Number(r.stage)===21);if(!context||['','NONE','NOT APPLICABLE','UNKNOWN','PENDING','UNASSIGNED'].includes(upper(recordValue(context,'EXTERNAL_CONTEXT_IDENTIFIER'))))reasons.push('Production does not use one registered fresh external context.');const inventory=(Array.isArray(recordValue(product,'GENERATED_ARTIFACT_INVENTORY'))?recordValue(product,'GENERATED_ARTIFACT_INVENTORY'):String(recordValue(product,'GENERATED_ARTIFACT_INVENTORY')||'').split(/[,|\n]+/)).map(String).map(x=>x.trim()).filter(Boolean),artifacts=all('artifacts').filter(a=>inventory.includes(recordId(a,'artifacts')));if(!inventory.length||artifacts.length!==new Set(inventory).size)reasons.push('Every generated product artifact must resolve to one active canonical artifact.');if(artifacts.some(a=>upper(recordValue(a,'AVAILABILITY'))!=='BYTES_PERSISTED_AND_VERIFIED'||!recordValue(a,'SHA256')||!Number.isFinite(Number(recordValue(a,'BYTE_SIZE')))))reasons.push('Every generated product artifact must have verified stored bytes, byte size, and SHA-256.');break;}
...
function recalculate(project){
  ...
  for(let stage=1;stage<=30;stage++){
    ...
    const derivedData=deriveStageData(project,stage);
