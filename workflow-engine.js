...
const VERSION_BY_STAGE=Object.freeze({
  2:['CURRENT_SOURCE_SET_VERSION','SOURCE-SET','sourceSetVersion'],3:['CURRENT_RESEARCH_VERSION','RESEARCH',null],
  4:['CURRENT_REQUIREMENTS_VERSION','REQUIREMENTS','requirementsVersion'],5:['CURRENT_REQUIREMENTS_VERSION','REQUIREMENTS','requirementsVersion'],6:['CURRENT_TEST_SUITE_VERSION','TEST-SUITE','testSuiteVersion'],
  7:['CURRENT_MUTATION_SUITE_VERSION','MUTATION-SUITE',null],8:['CURRENT_INSTRUCTION_VERSION','INSTRUCTION','instructionVersion']
});
...
function registerStageVersion(project,stage,acceptedChangeId){
  ...
  const [jobField,prefix,scopeKey]=config;
  const acceptedChange=safe(project.projectData.acceptedChanges).find(item=>item.changeId===acceptedChangeId);
  // Stage 05 reviews the requirement set. It creates a new requirements version only when
  // the accepted change actually replaces requirement records; a no-change review must not
  // manufacture a version transition.
  if(stage===5&&!safe(acceptedChange?.canonicalRecordIds).some(id=>records(project,'requirements').some(record=>recordId(record,'requirements')===id)))return null;
  const payload={stage,collections:Object.fromEntries(versionCollections(stage).map(collection=>[collection,records(project,collection).map(record=>({id:recordId(record,collection),fields:recordFields(record),relationships:record.relationships||{},sha256:record.sha256||null}))])),acceptedData:project.stages[stage].agentData};
  ...
  if(latest?.sha256===sha256){project.job[jobField]=latest.version;return {...latest,scopeKey};}
  const version=nextVersion(project.job[jobField],prefix);
  const record={versionId:allocateInfrastructureId(project,'VERSION','artifactVersions'),stage,kind:prefix,version,scopeKey,sha256,createdAt:now(),acceptedChangeId,payload};
  project.projectData.artifactVersions.push(record);
  project.job[jobField]=version;
  return record;
}

const APPLICATION_RECORD_DEFAULTS=Object.freeze({
  requirements:Object.freeze({STATUS:'ACTIVE'}),
  requirementResolutions:Object.freeze({STATUS:'ACTIVE'}),
  tests:Object.freeze({STATUS:'READY'}),
  iterations:Object.freeze({STATUS:'ACTIVE'}),
  candidateFreezes:Object.freeze({STATUS:'RESERVED'}),
  defects:Object.freeze({STATUS:'ACTIVE'}),
...
function initializeCanonicalRecord(project,collection,record){
  ensureShape(project);
  const definition=schema.RECORD_SCHEMAS[collection];
  if(!definition||!record)return record;
  record.fields=record.fields&&typeof record.fields==='object'?record.fields:{};
  record.relationships=record.relationships&&typeof record.relationships==='object'?record.relationships:{};
  for(const [name,value] of Object.entries(APPLICATION_RECORD_DEFAULTS[collection]||{}))if(record.fields[name]===undefined||record.fields[name]===null||record.fields[name]==='')record.fields[name]=typeof value==='function'?value({project,record,collection}):clone(value);
  for(const name of definition.required){
    const fieldDefinition=definition.fieldDefinitions[name];
    if(fieldDefinition?.producer!==schema.PRODUCER.APPLICATION)continue;
    if(record.fields[name]===undefined&&record.relationships[name]!==undefined)record.fields[name]=record.relationships[name];
  }
  Object.assign(record,record.fields);
  return record;
}
function bindAcceptedOutputVersion(project,acceptedChange,versionRecord){
  if(!acceptedChange||!versionRecord?.scopeKey||!versionRecord?.version)return acceptedChange;
  const ids=new Set(safe(acceptedChange.canonicalRecordIds));
  for(const collection of ALL_COLLECTIONS){
    const definition=schema.RECORD_SCHEMAS[collection];
    for(const record of safe(project.projectData[collection])){
      if(!ids.has(recordId(record,collection)))continue;
      record.scope={...(record.scope||{}),[versionRecord.scopeKey]:versionRecord.version};
      if(definition){record.contentSha256=hash.contentRecordSha256(record,definition.idField);record.recordSha256=hash.recordSha256(record);record.sha256=record.recordSha256;}
    }
  }
  acceptedChange.outputScope={...(acceptedChange.outputScope||{}),[versionRecord.scopeKey]:versionRecord.version};
...
function mandatoryRequirements(project){
  return recordsForCurrentScope(project,'requirements').filter(record=>{
...
function confirmedDefects(project){
  return recordsForCurrentScope(project,'defects').filter(record=>{
...
function resultRequirementId(project,record,collection=''){const direct=String(recordValue(record,'REQ_ID')||record.relationships?.REQ_ID||'');if(direct)return direct;if(collection==='deterministicResults'||recordValue(record,'TEST_ID')||record.relationships?.TEST_ID){const testId=String(recordValue(record,'TEST_ID')||record.relationships?.TEST_ID||''),test=records(project,'tests').find(item=>recordId(item,'tests')===testId);return test?testRequirementId(test):'';}return '';}
...
const COLLECTION_SCOPE_KEYS=Object.freeze({
  sources:['inputVersion','sourceSetVersion'],sourceConflicts:['inputVersion','sourceSetVersion'],research:['inputVersion','sourceSetVersion'],candidateRequirements:['inputVersion','sourceSetVersion'],
  requirements:['inputVersion','sourceSetVersion','requirementsVersion'],requirementResolutions:['inputVersion','sourceSetVersion','requirementsVersion'],
  tests:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion'],failureTests:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion'],
  instructions:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion'],instructionTraces:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion'],preflightRecords:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion'],
  iterations:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],candidateFreezes:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],runs:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],verification:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],comparisons:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],defects:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],rootCauses:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],changes:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],
  regressions:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion'],regressionExecutions:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId','baselineId','productId'],
  convergenceRecords:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],confirmationRecords:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId'],
  baselines:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId','baselineId'],products:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId','baselineId','productId'],
  deterministicResults:SCOPE_KEYS,meaningResults:SCOPE_KEYS,adversarialResults:SCOPE_KEYS,representationInspections:SCOPE_KEYS,processAudits:SCOPE_KEYS,productAudits:SCOPE_KEYS,releaseGateReviews:SCOPE_KEYS,releaseRecords:SCOPE_KEYS,artifactIdentities:SCOPE_KEYS,evidenceInvestigations:SCOPE_KEYS,evidenceChains:SCOPE_KEYS,artifacts:SCOPE_KEYS,evidenceRecords:SCOPE_KEYS,blockers:SCOPE_KEYS,freshContexts:['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId','baselineId','productId']
});
function scopeIdentity(value){const text=String(value??'').trim(),sentinel=upper(text);return !text||['NONE','NOT APPLICABLE','UNKNOWN','PENDING','UNASSIGNED'].includes(sentinel)?null:value;}
function currentScope(project){return {inputVersion:project.job.CURRENT_INPUT_VERSION||null,sourceSetVersion:project.job.CURRENT_SOURCE_SET_VERSION||null,requirementsVersion:project.job.CURRENT_REQUIREMENTS_VERSION||null,testSuiteVersion:project.job.CURRENT_TEST_SUITE_VERSION||null,instructionVersion:project.job.CURRENT_INSTRUCTION_VERSION||null,iterationId:scopeIdentity(recordId(latestIteration(project,[10,17,19]),'iterations')||project.job.CURRENT_ITERATION),candidateId:scopeIdentity(recordId(records(project,'candidateFreezes').at(-1),'candidateFreezes')),baselineId:scopeIdentity(recordId(records(project,'baselines').at(-1),'baselines')||project.job.CURRENT_BASELINE_ID),productId:scopeIdentity(recordId(records(project,'products').at(-1),'products')||project.job.CURRENT_PRODUCT_ID)};}
function scopeForCollection(project,collection,sourceScope=null,overrides={}){const source={...currentScope(project),...(sourceScope||{}),...(overrides||{})},keys=COLLECTION_SCOPE_KEYS[collection]||SCOPE_KEYS,identityKeys=new Set(['iterationId','candidateId','baselineId','productId']);return Object.fromEntries(keys.map(key=>[key,identityKeys.has(key)?scopeIdentity(source[key]):source[key]]).filter(([,value])=>value!==undefined&&value!==null));}
...
function verificationMatrix(project,iterationId){const requirements=mandatoryRequirements(project),iteration=records(project,'iterations').find(record=>recordId(record,'iterations')===String(iterationId||'')),candidateId=String(recordValue(iteration,'CANDIDATE_ID')||iteration?.relationships?.CANDIDATE_ID||iteration?.scope?.candidateId||'');const scopeRule={iterationId,...(candidateId?{candidateId}:{})};const runs=recordsForCurrentScope(project,'runs',scopeRule).filter(run=>runIterationId(run)===String(iterationId||''));const verification=recordsForCurrentScope(project,'verification',scopeRule);const expected=[];for(const req of requirements)for(const test of applicableTests(project,req))for(const run of runs)expected.push([requirementId(req),recordId(run,'runs'),recordId(test,'tests')].join('|'));const counts=new Map();for(const record of verification){const key=verificationKey(record);counts.set(key,(counts.get(key)||0)+1);}const missing=expected.filter(key=>!counts.has(key));const duplicates=[...counts].filter(([key,count])=>expected.includes(key)&&count!==1).map(([key,count])=>({key,count}));const invalid=verification.filter(record=>upper(recordValue(record,'INDEPENDENCE_STATUS'))!=='INDEPENDENT'||!String(recordValue(record,'EXACT_EVIDENCE')||'').trim());return {requirements,runs,verification,expected,counts,missing,duplicates,invalid,coverage:expected.length?expected.filter(key=>counts.get(key)===1).length/expected.length:0};}
...
function gate(stage,project){
  ...
  const collection=name=>recordsForCurrentScope(project,name).filter(record=>Number(record.stage)===Number(stage));
  const all=name=>recordsForCurrentScope(project,name);
  ...
  switch(stage){
    ...
    case 2:{
      requireAccepted();
      const sources=collection('sources'),latest=changes.at(-1),stageData=project.stages[2].agentData||{};
      const noApplicable=upper(stageData.KNOWN_CONTROLLING_SOURCES_EXAMINED)==='NO_APPLICABLE_EXTERNAL_SOURCE';
      const supportingEvidence=latest&&recordsForCurrentScope(project,'evidenceRecords').some(record=>record.rawResponseId===latest.rawResponseId&&Number(record.stage)===2);
      if(!sources.length&&!noApplicable)reasons.push('At least one inspected independent external governing source or an explicit NO_APPLICABLE_EXTERNAL_SOURCE determination is required.');
      if(noApplicable&&!supportingEvidence)reasons.push('NO_APPLICABLE_EXTERNAL_SOURCE requires evidence preserved with the accepted response.');
      if(noApplicable&&sources.length)reasons.push('NO_APPLICABLE_EXTERNAL_SOURCE contradicts the current source records.');
    ...
    case 5:
      requireAccepted();
      if(collection('requirementResolutions').some(record=>!String(recordValue(record,'RESOLUTION')||'').trim()))reasons.push('A requirement-set resolution is incomplete.');
      break;
    case 6:{
      requireAccepted();
    ...
    case 9:
      ...
      break;
    case 10:if(records(project,'iterations',{stage:10}).length<1)reasons.push('1 iterations record is required.');if(records(project,'candidateFreezes',{stage:10}).length<1)reasons.push('1 candidateFreezes record is required.');if(!String(project.stages[10].humanData?.FREEZE_OWNER||'').trim())reasons.push('Human candidate-component selection and freeze authorization are required.');break;
    ...
    case 12:{requireAccepted();const iteration=latestIteration(project,[10]);const m=verificationMatrix(project,recordId(iteration,'iterations'));if(m.runs.length!==10)reasons.push('The current verification batch does not contain exactly ten runs.');if(m.expected.length===0||m.coverage!==1)reasons.push('REQ × RUN × TEST verification coverage is not 100%.');if(m.duplicates.length)reasons.push('Duplicate verification triples exist.');if(m.invalid.length)reasons.push('A verification triple lacks independent verification or evidence.');break;}
    case 13:{
      requireAccepted();const reqs=mandatoryRequirements(project),compared=new Set(records(project,'comparisons',{stage:13}).map(record=>String(recordValue(record,'REQ_ID')||record.relationships?.REQ_ID||'')));
    ...
    case 14:{
      requireAccepted();const defects=records(project,'defects',{stage:14}).filter(record=>!['REJECTED','DUPLICATE','NOT A DEFECT','RETIRED'].includes(upper(recordValue(record,'STATUS')||'CONFIRMED'))),analysed=new Set(records(project,'rootCauses',{stage:14}).map(record=>String(recordValue(record,'DEFECT_ID')||record.relationships?.DEFECT_ID||'')));
    ...
    case 15:{requireAccepted();const defects=records(project,'defects',{stage:14}).filter(record=>!['REJECTED','DUPLICATE','NOT A DEFECT','RETIRED'].includes(upper(recordValue(record,'STATUS')||'CONFIRMED'))),regs=records(project,'regressions',{stage:15}).filter(r=>upper(recordValue(r,'ACTIVE_RETIRED_STATE')||'ACTIVE')!=='RETIRED'),covered=new Map(regs.map(r=>[String(recordValue(r,'DEFECT_ID')||r.relationships?.DEFECT_ID||''),r]));const missing=defects.filter(d=>!covered.has(recordId(d,'defects'))).map(d=>recordId(d,'defects'));if(missing.length)reasons.push('Permanent regression definitions are missing for: '+missing.join(', ')+'.');const executions=records(project,'regressionExecutions');for(const reg of regs){const id=recordId(reg,'regressions');if(!executions.some(e=>String(recordValue(e,'REG_ID')||e.relationships?.REG_ID||'')===id&&upper(recordValue(e,'PHASE'))==='PRE_CORRECTION'&&['VIOLATED','FAILED','FAIL'].includes(upper(recordValue(e,'RESULT')))))reasons.push('Regression '+id+' lacks an actual pre-correction failing execution.');}break;}
    case 16:{requireAccepted();const defects=records(project,'defects',{stage:14}).filter(record=>{const status=upper(recordValue(record,'STATUS')||'CONFIRMED');return !['REJECTED','DUPLICATE','NOT A DEFECT','RETIRED'].includes(status);}),changes=records(project,'changes',{stage:16});if(defects.length&&!changes.length)reasons.push('A responsible-layer changeset or blocker is required for confirmed defects.');break;}
    ...
    case 20:requireCount('baselines',1);if(!all('confirmationRecords').some(record=>upper(recordValue(record,'DETERMINATION'))==='SATISFIED'))reasons.push('A successful unchanged confirmation is required.');break;
...
function commandRecord(project,collection,fields,{stage=null,source='APPLICATION_COMMAND',scope=null}={}){
  ...
  const record={id,stage:(stage??Number(project.activeStage||0))||null,createdAt:deviceTimestamp,updatedAt:deviceTimestamp,active:true,scope:scope||scopeForCollection(project,collection),fields:full,...full,source};
...
function reserveRunBatch(project,{stage=project.activeStage,iterationId=null,candidateId=null,count=10}={}){
  ...
  const resolvedCandidate=candidateId||recordId(records(project,'candidateFreezes').at(-1),'candidateFreezes')||null,slotScope=scopeForCollection(project,'runs',null,{iterationId:resolvedIteration,candidateId:resolvedCandidate});const created=[];for(let i=0;i<count;i++){const context=commandRecord(project,'freshContexts',{EXTERNAL_CONTEXT_IDENTIFIER:'UNASSIGNED',ROLE:core.STAGES[Number(stage)-1]?.role||'UNKNOWN',ITERATION_ID:resolvedIteration,RUN_ID:'PENDING',AUTHORIZED_PROJECT_INPUTS:project.job.CURRENT_INPUT_VERSION||'UNKNOWN',AUTHORIZED_EXTERNAL_SOURCE_MATERIAL:project.job.CURRENT_SOURCE_SET_VERSION||'NOT APPLICABLE',FROZEN_ARTIFACT_VERSIONS:'CURRENT AUTHORIZED VERSIONS',TOOL_AVAILABILITY:'UNKNOWN',CONTAMINATION_STATUS:'UNKNOWN',OUTPUT_IDENTITY:'UNKNOWN',DEVIATIONS:'NONE',EVIDENCE:'Application-reserved context slot.',USABILITY_DETERMINATION:'PENDING'},{stage,scope:scopeForCollection(project,'freshContexts',slotScope),source:'APPLICATION_RESERVATION'});const run=commandRecord(project,'runs',{ITERATION_ID:resolvedIteration,CANDIDATE_ID:resolvedCandidate||'PENDING',CONTEXT_ID:context.id,FRESH_CONTEXT_RECORD:context.id,CONTAMINATION_CHECK:'PENDING',TOOL_CONFIGURATION:'PENDING',EXECUTION_STATUS:'RESERVED',COMPLETE_OUTPUT:''},{stage,scope:slotScope,source:'APPLICATION_RESERVATION'});context.fields.RUN_ID=context.RUN_ID=run.id;created.push({runId:run.id,contextId:context.id});}
...
function freezeCandidate(project,{stage=project.activeStage,artifactIds=[],operatorLabel='HUMAN_OPERATOR',purpose='TEST_CANDIDATE',toolConfiguration='CURRENT AUTHORIZED CONFIGURATION',settings='CURRENT AUTHORIZED SETTINGS',permissions='CURRENT AUTHORIZED PERMISSIONS',limitations='RECORDED LIMITATIONS',roleDistribution='WORKFLOW ROLE MAP',batchChangeRule='ANY MATERIAL CHANGE REQUIRES A NEW CANDIDATE'}={}){
  ensureShape(project);if(Number(stage)===10)recordHumanDecision(project,{stage:10,field:'FREEZE_OWNER',value:operatorLabel,operatorLabel,reason:'Human selected and authorized the candidate component set for freezing.'});const artifacts=selectedArtifacts(project,artifactIds);if(!artifacts.length)throw new Error('At least one verified artifact is required to freeze a candidate.');const iterationId=allocateId(project,'iterations'),candidateId=allocateId(project,'candidateFreezes'),manifest=artifacts.map(a=>({artifactId:recordId(a,'artifacts'),filename:recordValue(a,'FILENAME'),byteSize:Number(recordValue(a,'BYTE_SIZE')),sha256:recordValue(a,'SHA256'),storageReference:recordValue(a,'STORAGE_REFERENCE')})),componentHashes=Object.fromEntries(manifest.map(a=>[a.artifactId,a.sha256])),createdAt=now();
  const iterationFields={ITERATION_ID:iterationId,CANDIDATE_ID:candidateId,PREVIOUS_ITERATION_ID:recordId(recordsForCurrentScope(project,'iterations').at(-1),'iterations')||'',CHANGESET_ID:recordId(recordsForCurrentScope(project,'changes').at(-1),'changes')||'',PURPOSE:purpose,STATUS:'FROZEN',LINEAGE:hash.sha256Value(manifest),EVIDENCE:`Application freeze from ${manifest.length} verified artifact(s).`};const iteration={id:iterationId,stage:Number(stage),createdAt,active:true,scope:scopeForCollection(project,'iterations',null,{iterationId,candidateId}),fields:iterationFields,...iterationFields,source:'APPLICATION_DERIVATION'};iteration.contentSha256=hash.contentRecordSha256(iteration,'ITERATION_ID');iteration.recordSha256=hash.recordSha256(iteration);iteration.sha256=iteration.recordSha256;
  const candidateFields={CANDIDATE_ID:candidateId,ITERATION_ID:iterationId,COMPONENT_MANIFEST:manifest,COMPONENT_VERSIONS:Object.fromEntries(manifest.map(a=>[a.artifactId,'APPLICATION-CONTROLLED'])),COMPONENT_HASHES:componentHashes,ROLE_DISTRIBUTION:roleDistribution,IMMUTABLE_LOCATIONS:manifest.map(a=>a.storageReference),TOOL_CONFIGURATION:toolConfiguration,SETTINGS:settings,PERMISSIONS:permissions,LIMITATIONS:limitations,BATCH_CHANGE_RULE:batchChangeRule,STATUS:'FROZEN',EVIDENCE:hash.sha256Value({manifest,operatorLabel})};const candidate={id:candidateId,stage:Number(stage),createdAt,active:true,scope:scopeForCollection(project,'candidateFreezes',null,{iterationId,candidateId}),fields:candidateFields,...candidateFields,source:'APPLICATION_DERIVATION'};candidate.contentSha256=hash.contentRecordSha256(candidate,'CANDIDATE_ID');candidate.recordSha256=hash.recordSha256(candidate);candidate.sha256=candidate.recordSha256;project.projectData.iterations.push(iteration);project.projectData.candidateFreezes.push(candidate);project.job.CURRENT_ITERATION=iterationId;addHistory(project,'CANDIDATE_FROZEN',{stage:Number(stage),iterationId,candidateId,artifactIds:manifest.map(a=>a.artifactId),operatorLabel,identityAssurance:'SELF_ASSERTED'});recalculate(project);return {iteration,candidate};
}
function beginConfirmationIteration(project,{stage=19,candidateId=null,purpose='UNCHANGED_CONFIRMATION'}={}){ensureShape(project);const candidate=records(project,'candidateFreezes').find(r=>recordId(r,'candidateFreezes')===String(candidateId||''))||records(project,'candidateFreezes').at(-1);if(!candidate)throw new Error('A frozen candidate is required before unchanged confirmation.');const resolvedCandidate=recordId(candidate,'candidateFreezes'),previous=latestIteration(project,[10,17]),iterationId=allocateId(project,'iterations'),createdAt=now(),fields={ITERATION_ID:iterationId,CANDIDATE_ID:resolvedCandidate,PREVIOUS_ITERATION_ID:recordId(previous,'iterations')||'',CHANGESET_ID:'NONE',PURPOSE:purpose,STATUS:'FROZEN_UNCHANGED',LINEAGE:recordValue(candidate,'EVIDENCE')||candidate.sha256||hash.sha256Value(recordFields(candidate)),EVIDENCE:'Application-created unchanged-confirmation iteration using the exact frozen candidate identity.'};const record={id:iterationId,stage:Number(stage),createdAt,active:true,scope:scopeForCollection(project,'iterations',null,{iterationId,candidateId:resolvedCandidate}),fields,...fields,source:'APPLICATION_DERIVATION'};record.contentSha256=hash.contentRecordSha256(record,'ITERATION_ID');record.recordSha256=hash.recordSha256(record);record.sha256=record.recordSha256;project.projectData.iterations.push(record);project.job.CURRENT_ITERATION=iterationId;addHistory(project,'CONFIRMATION_ITERATION_CREATED',{stage:Number(stage),iterationId,candidateId:resolvedCandidate});recalculate(project);return record;}
function freezeBaseline(project,{artifactIds=[],operatorLabel='HUMAN_OPERATOR',authorization='AUTHORIZED'}={}){
  ensureShape(project);recordHumanDecision(project,{stage:20,field:'BASELINE_OWNER',value:operatorLabel,operatorLabel,reason:'Human authorized the exact unchanged candidate and artifacts as the production baseline.'});const confirmation=recordsForCurrentScope(project,'confirmationRecords').filter(r=>upper(recordValue(r,'DETERMINATION'))==='SATISFIED').at(-1);if(!confirmation)throw new Error('A current successful unchanged confirmation is required before baseline freeze.');const artifacts=selectedArtifacts(project,artifactIds);if(!artifacts.length)throw new Error('At least one verified artifact is required to freeze a baseline.');const baselineId=allocateId(project,'baselines'),createdAt=now(),approvedVersions={inputVersion:project.job.CURRENT_INPUT_VERSION,sourceSetVersion:project.job.CURRENT_SOURCE_SET_VERSION,requirementsVersion:project.job.CURRENT_REQUIREMENTS_VERSION,testSuiteVersion:project.job.CURRENT_TEST_SUITE_VERSION,instructionVersion:project.job.CURRENT_INSTRUCTION_VERSION,iterationId:project.job.CURRENT_ITERATION,candidateId:recordId(recordsForCurrentScope(project,'candidateFreezes').at(-1),'candidateFreezes'),hashes:Object.fromEntries(artifacts.map(a=>[recordId(a,'artifacts'),recordValue(a,'SHA256')]))};const fields={BASELINE_ID:baselineId,SUPPORTING_CONFIRMATION_ID:recordId(confirmation,'confirmationRecords'),APPROVED_VERSIONS:approvedVersions,IMMUTABLE_ARTIFACT_RECORDS:artifacts.map(a=>recordId(a,'artifacts')),HASHES:approvedVersions.hashes,HUMAN_AUTHORIZATION:authorization,AUTHORIZED_RECIPIENT_ROLES:'CURRENT AUTHORIZED ROLES',CONTROLLED_STORAGE:'INDEXEDDB VERIFIED BYTES',STATUS:'FROZEN',EVIDENCE:hash.sha256Value({approvedVersions,operatorLabel})};const record={id:baselineId,stage:20,createdAt,active:true,scope:{...currentScope(project),baselineId},fields,...fields,source:'APPLICATION_DERIVATION'};record.contentSha256=hash.contentRecordSha256(record,'BASELINE_ID');record.recordSha256=hash.recordSha256(record);record.sha256=record.recordSha256;project.projectData.baselines.push(record);project.job.CURRENT_BASELINE_ID=baselineId;addHistory(project,'BASELINE_FROZEN',{stage:20,baselineId,artifactIds:fields.IMMUTABLE_ARTIFACT_RECORDS,operatorLabel,identityAssurance:'SELF_ASSERTED'});recalculate(project);return record;
}
function reserveProductExecution(project,{operatorLabel='HUMAN_OPERATOR'}={}){ensureShape(project);const baseline=recordsForCurrentScope(project,'baselines').at(-1);if(!baseline)throw new Error('A current frozen baseline is required.');const contexts=recordsForCurrentScope(project,'freshContexts').filter(r=>['','NONE','NOT APPLICABLE','UNKNOWN'].includes(upper(recordValue(r,'RUN_ID')||''))),context=contexts.at(-1);if(!context)throw new Error('A current fresh production context is required.');const productId=allocateId(project,'products'),executionId=allocateInfrastructureId(project,'EXECUTION','products'),createdAt=now(),fields={PRODUCT_ID:productId,PRODUCT_VERSION:'PRODUCT-v001',BASELINE_ID:recordId(baseline,'baselines'),EXECUTION_ID:executionId,PRODUCTION_CONTEXT_ID:recordId(context,'freshContexts'),INSTRUCTION_VERSION:project.job.CURRENT_INSTRUCTION_VERSION,BASELINE_MATERIALS:recordValue(baseline,'IMMUTABLE_ARTIFACT_RECORDS')||[],EXECUTION_TIMESTAMPS:{reservedAt:createdAt},TOOL_CONFIGURATION:'PENDING AGENT COMPLETION',DEVIATIONS:'NONE RECORDED',FAILURES:'NONE RECORDED',GENERATED_ARTIFACT_INVENTORY:[],STATUS:'RESERVED'};const record={id:productId,stage:21,createdAt,active:true,reserved:true,completionState:'AWAITING_AGENT_COMPLETION',scope:{...currentScope(project),baselineId:fields.BASELINE_ID,productId},fields,...fields,source:'APPLICATION_RESERVED'};record.contentSha256=hash.contentRecordSha256(record,'PRODUCT_ID');record.recordSha256=hash.recordSha256(record);record.sha256=record.recordSha256;project.projectData.products.push(record);project.job.CURRENT_PRODUCT_ID=productId;addHistory(project,'PRODUCT_EXECUTION_RESERVED',{stage:21,productId,executionId,contextId:fields.PRODUCTION_CONTEXT_ID,operatorLabel,identityAssurance:'SELF_ASSERTED'});recalculate(project);return record;}
...
function operationalMetrics(project){
  ensureShape(project);
  const data=project.projectData||{},history=safe(data.history),dispositions=safe(data.responseDispositions),validations=safe(data.responseValidations),raw=safe(data.rawResponses);
  const historyCount=pattern=>history.filter(event=>pattern.test(String(event.type||''))).length;
  return Object.freeze({
    rawResponses:raw.length,
    validationFailures:validations.filter(item=>item.valid===false||upper(item.status)==='VALIDATION_FAILED_RESPONSE').length||dispositions.filter(item=>item.type==='VALIDATION_FAILED_RESPONSE').length,
    staleResponses:raw.filter(item=>/STALE/.test(upper(item.status))).length+validations.filter(item=>/STALE/.test(upper(item.status))).length,
    rejectedProposals:dispositions.filter(item=>item.type==='REJECTED_RESPONSE').length,
    acceptedDataChanges:safe(data.acceptedChanges).filter(item=>item.status==='COMMITTED'&&!item.invalidatedBy).length,
    clarificationCycles:safe(data.humanInputAnswers).length,
    controlledCorrections:records(project,'changes').length+historyCount(/CORRECT|INVALIDAT|SUPERSED/i),
    storageFailures:historyCount(/STORAGE.*FAIL|PERSIST.*FAIL|TRANSACTION.*FAIL/i),
    gateRegressions:historyCount(/GATE.*REGRESS|DOWNSTREAM_INVALIDATED/i),
    releaseRejections:records(project,'releaseRecords').filter(item=>upper(recordValue(item,'DETERMINATION'))==='REJECTED').length,
    releaseBlocks:records(project,'releaseRecords').filter(item=>upper(recordValue(item,'DETERMINATION'))==='BLOCKED').length
  });
}
...
globalThis.closedLoopWorkflowEngine=Object.freeze({recordMigratedAcceptedChange,
  ...
  clone,now,safe,upper,truth,falsey,numeric,recordFields,recordValue,recordId,isActiveRecord,records,scopeForCollection,registerGeneratedPrompt,createHumanBlocker,registerFreshContext,recordHumanDecision,invalidateAcceptedResponse,reserveRunBatch,registerArtifactBytes,freezeCandidate,beginConfirmationIteration,freezeBaseline,reserveProductExecution,createNewJobReset,
  ensureShape,addHistory,allocateId,allocateInfrastructureId,nextVersion,registerStageVersion,initializeCanonicalRecord,bindAcceptedOutputVersion,
  ...
  recordHumanInputVersion,recordStageConfirmation,recordReleaseDetermination,acceptedControlEvents,constructEvidenceChains,verifyArtifactIdentity,operationalMetrics
});
})();
