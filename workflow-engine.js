...
function acceptedChangeOperation(project,change){if(change?.operation)return String(change.operation);const proposal=safe(project?.projectData?.responseProposals).find(item=>item.proposalId===change?.proposalId);return String(proposal?.envelope?.operation||'COMPLETE');}
...
function nextPromptOperation(project,stage){ensureShape(project);const number=Number(stage),contract=schema.STAGE_CONTRACTS[number],operations=contract?.operations||['COMPLETE'];if(![17,19].includes(number))return {operation:operations[0],scope:{}};const changes=acceptedChanges(project,number),opOf=change=>acceptedChangeOperation(project,change),first=operations[0];if(!changes.some(change=>opOf(change)===first))return {operation:first,scope:{}};const iteration=latestIteration(project,[number]),iterationId=recordId(iteration,'iterations'),candidateId=iterationCandidateId(project,iterationId);if(!iterationId||!candidateId)return {operation:'EXECUTE_RUN',scope:{},blockedReason:number===17?'Freeze the corrected candidate before generating execution prompts.':'Start the unchanged-confirmation iteration from the exact converged candidate before generating execution prompts.'};const iterationScope={iterationId,candidateId},runs=recordsForCurrentScope(project,'runs',iterationScope).filter(run=>runIterationId(run)===iterationId);if(runs.length!==10)return {operation:'EXECUTE_RUN',scope:iterationScope,blockedReason:'Reserve exactly ten run/context slots for the current iteration before generating execution prompts.'};const executions=changes.filter(change=>opOf(change)==='EXECUTE_RUN'&&String(acceptedChangeScope(project,change).iterationId||'')===iterationId&&String(acceptedChangeScope(project,change).candidateId||'')===candidateId),done=new Set(executions.map(change=>String(acceptedChangeScope(project,change).runId||'')));const nextRun=runs.find(run=>!done.has(recordId(run,'runs')));if(nextRun){const runId=recordId(nextRun,'runs'),contextId=String(recordValue(nextRun,'CONTEXT_ID')||nextRun.relationships?.CONTEXT_ID||nextRun.scope?.contextId||'');return {operation:'EXECUTE_RUN',scope:{...iterationScope,runId,contextId}};}for(const operation of operations.slice(2)){const accepted=changes.some(change=>opOf(change)===operation&&String(acceptedChangeScope(project,change).iterationId||iterationId)===iterationId);if(!accepted)return {operation,scope:iterationScope};}return {operation:operations.at(-1),scope:iterationScope,complete:true};}
...
function verificationMatrix(project,iterationId){const candidateId=iterationCandidateId(project,iterationId);const iterationScope={iterationId,...(candidateId?{candidateId}:{})};const requirements=mandatoryRequirements(project);const runs=recordsForCurrentScope(project,'runs',iterationScope).filter(run=>runIterationId(run)===String(iterationId||''));const verification=recordsForCurrentScope(project,'verification',iterationScope);const expected=[];for(const req of requirements)for(const test of applicableTests(project,req))for(const run of runs)expected.push([requirementId(req),recordId(run,'runs'),recordId(test,'tests')].join('|'));const counts=new Map();for(const record of verification){const key=verificationKey(record);counts.set(key,(counts.get(key)||0)+1);}const missing=expected.filter(key=>!counts.has(key));const duplicates=[...counts].filter(([key,count])=>expected.includes(key)&&count!==1).map(([key,count])=>({key,count}));const invalid=verification.filter(record=>upper(recordValue(record,'INDEPENDENCE_STATUS'))!=='INDEPENDENT'||!String(recordValue(record,'EXACT_EVIDENCE')||'').trim());return {requirements,runs,verification,expected,counts,missing,duplicates,invalid,coverage:expected.length?expected.filter(key=>counts.get(key)===1).length/expected.length:0};}
function currentRegressionExecutions(project,iterationId){const candidateId=iterationCandidateId(project,iterationId);const iterationScope={iterationId,...(candidateId?{candidateId}:{})};return recordsForCurrentScope(project,'regressionExecutions',iterationScope).filter(record=>!iterationId||String(recordValue(record,'ITERATION_ID')||record.relationships?.ITERATION_ID||record.scope?.iterationId||'')===String(iterationId));}
function evaluateIteration(project,iterationId,mode='INITIAL'){const candidateId=iterationCandidateId(project,iterationId);const iterationScope={iterationId,...(candidateId?{candidateId}:{})};const matrix=verificationMatrix(project,iterationId);const runs=matrix.runs;const contextRecords=recordsForCurrentScope(project,'freshContexts',iterationScope);const contextById=new Map(contextRecords.map(record=>[recordId(record,'freshContexts'),record]));const placeholder=new Set(['','UNASSIGNED','UNKNOWN','PENDING','NOT APPLICABLE','NONE']);const resolvedContexts=runs.map(run=>{const id=String(recordValue(run,'CONTEXT_ID')||run.relationships?.CONTEXT_ID||run.scope?.contextId||'');const record=contextById.get(id);const externalIdentifier=String(recordValue(record,'EXTERNAL_CONTEXT_IDENTIFIER')||'').trim();return {id,record,externalIdentifier};});const contexts=new Set(resolvedContexts.filter(item=>item.record&&!placeholder.has(upper(item.externalIdentifier))).map(item=>upper(item.externalIdentifier)));const candidates=new Set(runs.map(r=>String(recordValue(r,'CANDIDATE_ID')||r.relationships?.CANDIDATE_ID||r.scope?.candidateId||'')));const candidateHashes=new Set(runs.map(r=>hash.sha256Value(recordValue(r,'OUTPUT_HASHES')||recordValue(r,'CANDIDATE_HASHES')||r.scope?.candidateId||'')).filter(Boolean));const contaminated=runs.filter(r=>!['NONE','FALSE','CLEAN','NOT CONTAMINATED'].includes(upper(recordValue(r,'CONTAMINATION_CHECK'))));const stage=Number(runs[0]?.stage||0),expectedExecutionOperation=stage===11?'COMPLETE':'EXECUTE_RUN';const executionChanges=acceptedChanges(project,stage).filter(change=>acceptedChangeOperation(project,change)===expectedExecutionOperation&&String(acceptedChangeScope(project,change).iterationId||'')===String(iterationId||'')&&String(acceptedChangeScope(project,change).candidateId||candidateId||'')===String(candidateId||''));const missingExecutionProof=runs.flatMap(run=>{const runId=recordId(run,'runs'),contextId=String(recordValue(run,'CONTEXT_ID')||run.relationships?.CONTEXT_ID||run.scope?.contextId||''),matches=executionChanges.filter(change=>String(acceptedChangeScope(project,change).runId||'')===runId&&String(acceptedChangeScope(project,change).contextId||'')===contextId);if(matches.length!==1)return [runId];const change=matches[0],raw=safe(project.projectData.rawResponses).find(item=>item.rawResponseId===change.rawResponseId&&!item.invalidatedBy),receipt=safe(project.projectData.outputReceipts).find(item=>(item.acceptedCanonicalChangeId===change.changeId||item.rawResponseId===change.rawResponseId)&&!item.invalidatedBy);return raw&&String(raw.promptScope?.runId||'')===runId&&String(raw.promptScope?.contextId||'')===contextId&&receipt&&String(receipt.runId||'')===runId&&String(receipt.contextId||'')===contextId?[]:[runId];});const comparisons=recordsForCurrentScope(project,'comparisons',iterationScope);const defects=recordsForCurrentScope(project,'defects',iterationScope);const rca=recordsForCurrentScope(project,'rootCauses',iterationScope);const regExec=currentRegressionExecutions(project,iterationId);const unexplained=comparisons.filter(r=>truth(recordValue(r,'CORRECTNESS_AFFECTING_VARIANCE'))&&!truth(recordValue(r,'AUTHORIZED_VARIANCE')));const reasons=[];if(runs.length!==10)reasons.push('Exactly ten current runs are required.');if(resolvedContexts.some(item=>!item.record||placeholder.has(upper(item.externalIdentifier)))||contexts.size!==10)reasons.push('Ten distinct, actually registered external fresh contexts are required.');if(candidates.size!==1||candidates.has(''))reasons.push('Exactly one current candidate identity is required.');if(candidateHashes.size>1)reasons.push('Candidate hashes differ across current runs.');if(contaminated.length)reasons.push('A current run is contaminated or contamination is unknown.');if(missingExecutionProof.length)reasons.push('Every current run requires exactly one accepted execution response with matching raw-response and receipt run/context identity.');if(mode!=='INITIAL'&&(matrix.expected.length===0||matrix.missing.length||matrix.duplicates.length||matrix.invalid.length))reasons.push('The current REQ × RUN × TEST matrix is incomplete or invalid.');if(mode!=='INITIAL'&&!regExec.length&&records(project,'regressions').length)reasons.push('Current regression executions are required.');if(unexplained.length)reasons.push('Unexplained correctness-affecting variance remains.');return {mode,iterationId,runs:runs.map(r=>recordId(r,'runs')),contextCount:contexts.size,contextRecordIds:resolvedContexts.map(item=>item.id),externalContextIdentifiers:resolvedContexts.map(item=>item.externalIdentifier),candidateIds:[...candidates],matrix,comparisonCount:comparisons.length,defectCount:defects.length,rootCauseCount:rca.length,regressionExecutionCount:regExec.length,missingExecutionProof,unexplainedVarianceCount:unexplained.length,complete:reasons.length===0,reasons};}
...
function gate(stage,project){
  ...
  switch(stage){
    ...
    case 17:{requireAccepted();const missingOps=schema.STAGE_CONTRACTS[17].operations.filter(operation=>!changes.some(change=>acceptedChangeOperation(project,change)===operation));if(missingOps.length)reasons.push('Required Stage 17 operation responses are missing: '+missingOps.join(', ')+'.');const iteration=latestIteration(project,[17]);const ev=evaluateIteration(project,recordId(iteration,'iterations'),'CORRECTED');if(!ev.complete)reasons.push(...ev.reasons);break;}
    case 18:{
    ...
    case 19:{requireAccepted();const missingOps=schema.STAGE_CONTRACTS[19].operations.filter(operation=>!changes.some(change=>acceptedChangeOperation(project,change)===operation));if(missingOps.length)reasons.push('Required Stage 19 operation responses are missing: '+missingOps.join(', ')+'.');const iteration=latestIteration(project,[19]);const ev=evaluateIteration(project,recordId(iteration,'iterations'),'UNCHANGED_CONFIRMATION');if(!ev.complete)reasons.push(...ev.reasons);requireCount('confirmationRecords',1);if(collection('confirmationRecords').some(r=>upper(recordValue(r,'DETERMINATION'))!=='SATISFIED'))reasons.push('Unchanged confirmation is not affirmatively satisfied.');break;}
...
function invalidateAcceptedResponse(project,{stage=project.activeStage,rawResponseId,reason='Corrected response required.',operatorLabel='HUMAN_OPERATOR'}={}){
  ...
  if([17,19].includes(number)&&selectedIndex===0){for(const collection of ['iterations','runs','freshContexts',...(number===17?['candidateFreezes']:[])])for(const record of records(project,collection,{stage:number,active:false}))if(!record.invalidatedBy){record.active=false;record.validity='INVALIDATED';record.invalidatedBy=event.eventId;}const prior=latestIteration(project,[number===17?10:17]);project.job.CURRENT_ITERATION=recordId(prior,'iterations')||'';}
...
function reserveRunBatch(project,{stage=project.activeStage,iterationId=null,candidateId=null,count=10}={}){
  ...
  const created=[];for(let i=0;i<count;i++){const context=commandRecord(project,'freshContexts',{EXTERNAL_CONTEXT_IDENTIFIER:'UNASSIGNED',ROLE:core.STAGES[Number(stage)-1]?.role||'UNKNOWN',ITERATION_ID:resolvedIteration,RUN_ID:'PENDING',AUTHORIZED_PROJECT_INPUTS:project.job.CURRENT_INPUT_VERSION||'UNKNOWN',AUTHORIZED_EXTERNAL_SOURCE_MATERIAL:project.job.CURRENT_SOURCE_SET_VERSION||'NOT APPLICABLE',FROZEN_ARTIFACT_VERSIONS:'CURRENT AUTHORIZED VERSIONS',TOOL_AVAILABILITY:'UNKNOWN',CONTAMINATION_STATUS:'UNKNOWN',OUTPUT_IDENTITY:'UNKNOWN',DEVIATIONS:'NONE',EVIDENCE:'Application-reserved context slot.',USABILITY_DETERMINATION:'PENDING'},{stage,source:'APPLICATION_RESERVATION',scope});const run=commandRecord(project,'runs',{ITERATION_ID:resolvedIteration,CANDIDATE_ID:resolvedCandidate,CONTEXT_ID:context.id,FRESH_CONTEXT_RECORD:context.id,CONTAMINATION_CHECK:'PENDING',TOOL_CONFIGURATION:'PENDING',EXECUTION_STATUS:'RESERVED',COMPLETE_OUTPUT:''},{stage,source:'APPLICATION_RESERVATION',scope});context.fields.RUN_ID=context.RUN_ID=run.id;created.push({runId:run.id,contextId:context.id});}
...
globalThis.closedLoopWorkflowEngine=Object.freeze({recordMigratedAcceptedChange,
  ...
  clone,now,safe,upper,truth,falsey,numeric,recordFields,recordValue,recordId,isActiveRecord,records,registerGeneratedPrompt,createHumanBlocker,registerFreshContext,recordHumanDecision,invalidateAcceptedResponse,reserveRunBatch,registerArtifactBytes,freezeCandidate,freezeBaseline,reserveProductExecution,createNewJobReset,
  ensureShape,addHistory,allocateId,allocateInfrastructureId,nextVersion,registerStageVersion,
  unresolvedHumanRequests,openBlockers,acceptedChanges,acceptedChangeOperation,acceptedChangeScope,nextPromptOperation,hasStageActivity,mandatoryRequirements,confirmedDefects,unresolvedMaterialDefects,
  currentScope,recordsForCurrentScope,verificationMatrix,evaluateIteration,DERIVATIONS,coverageMetrics,convergenceMetrics,releaseMetrics,gate,deriveStageData,recalculate,invalidateDownstream,applicationInitialFields,
