...
if (!html.includes(proofMarker)) {
  ...
  const block = `<div class="card" ${proofMarker}>
...
const statusElement=()=>document.getElementById('retainedProjectStatus');
...
const fingerprint=project=>JSON.stringify({schema:project&&project.schema,projectId:project&&project.projectId,name:project&&project.name,updatedAt:project&&project.updatedAt,objective:project&&project.job&&project.job.exactUserObjective,deliverables:project&&project.job&&project.job.exactDeliverables,stages:Array.isArray(project&&project.stages)?project.stages.map(stage=>[stage.number,stage.name,stage.status,String(stage.completionEvidence||'').length]):[],releaseHash:project&&project.legacyProjectMetadata&&project.legacyProjectMetadata.releaseHash});
...
const bootstrapProject=async()=>{try{setStatus('Loading the completed project created with this application…');const response=await fetch(PROJECT_PATH,{cache:'no-store'});if(!response.ok)throw new Error('Project retrieval failed: '+response.status);retainedProject=validateProject(await response.json());const changed=storeProject(retainedProject);const reloadToken=String(retainedProject.projectId)+'|'+String(retainedProject.updatedAt||'');if(changed&&sessionStorage.getItem(RELOAD_KEY)!==reloadToken){sessionStorage.setItem(RELOAD_KEY,reloadToken);location.reload();return}let card=await waitForCard(retainedProject);if(!card)card=await importThroughVisibleControl(retainedProject);if(!card)throw new Error('The completed application project was stored but was not rendered by the Projects view.');sessionStorage.removeItem(RELOAD_KEY);protectCard(card,retainedProject);const view=document.getElementById('projectsView');if(view){new MutationObserver(()=>{const current=projectCard(retainedProject);if(current)protectCard(current,retainedProject)}).observe(view,{childList:true,subtree:true})}setStatus('Loaded: '+String(retainedProject.name||'completed application project')+' · '+completedCount(retainedProject)+'/31 stages complete.')}catch(error){console.error(error);setStatus('Completed application project load failed: '+String(error&&error.message||error),true)}};
document.addEventListener('click',event=>{const trigger=event.target&&event.target.closest&&event.target.closest('#showRetainedProjectBtn');if(!trigger)return;event.preventDefault();if(retainedProject)openProject(retainedProject);else bootstrapProject()});
window.addEventListener('DOMContentLoaded',bootstrapProject,{once:true});
})();
</script>`;
