...
function assertRequiredPromptScope(stage,operation,scope){const required=schema.operationContract(stage,operation)?.scopeRequirements||[],missing=required.filter(key=>key==='projectRevision'?scope?.[key]===undefined||scope?.[key]===null||!Number.isFinite(Number(scope[key])):scopePlaceholder(scope?.[key]));if(missing.length){const error=new Error(`Controlling instruction cannot be created until application scope exists for: ${missing.join(', ')}.`);error.code='MISSING_REQUIRED_PROMPT_SCOPE';error.missingScope=missing;throw error;}return scope;}"""
...
new="if(!object(envelope.scope))issues.push(issue('INVALID_SCOPE','/scope','scope must be an object.'));else{unknownKeys(envelope.scope,RESPONSE_SCOPE_KEYS,'/scope',issues);const expected=currentScope(project,promptRecord),placeholder=value=>['','UNKNOWN','NONE','NOT APPLICABLE','UNASSIGNED','PENDING'].includes(String(value??'').trim().toUpperCase());for(const key of operationContract?.scopeRequirements||[]){const missing=key==='projectRevision'?envelope.scope[key]===undefined||envelope.scope[key]===null||!Number.isFinite(Number(envelope.scope[key])):placeholder(envelope.scope[key]);if(missing)issues.push(issue('MISSING_REQUIRED_SCOPE',`/scope/${key}`,`Required operation scope ${key} is missing or unresolved.`));const expectedMissing=key==='projectRevision'?expected[key]===undefined||expected[key]===null||!Number.isFinite(Number(expected[key])):placeholder(expected[key]);if(expectedMissing)issues.push(issue('INVALID_CONTROLLING_PROMPT_SCOPE',`/scope/${key}`,`The saved controlling prompt is missing required operation scope ${key}.`));}for(const key of RESPONSE_SCOPE_KEYS)if(JSON.stringify(envelope.scope[key]??null)!==JSON.stringify(expected[key]??null))issues.push(issue('STALE_SCOPE',`/scope/${key}`,`Scope ${key} does not match the controlling prompt.`));}"
...
test="""
// Required operation scope must fail closed before a controlling prompt can exist.
{
