...
function addRegression(p,{result='VIOLATED',withExecution=true,withEvidence=true}={}){
  ...
  if(withEvidence){evidence=record('evidenceRecords',15,{KIND:'REGRESSION_EXECUTION',AUTHORITY_TYPE:'APPLICATION',DESCRIPTION:'Stipulated synthetic pre-correction regression execution observation.',CONTENT:'The preserved fixture executed against the pre-correction candidate and reproduced the controlled failure.',STATUS:'PRESERVED'},'EVIDENCE-STAGE18-PRE');p.projectData.evidenceRecords.push(evidence);}
...
// Stage 15 component: a stipulated linked failing-execution record satisfies the record gate.
// This fixture does not run the defective product or establish the claimed observation.
{
...
console.log(JSON.stringify({controllerStage:'18',applicationStages:[14,15],rootCauseAndRegressions:'PASS',currentScopeRcaEnforced:true,staleRcaRejected:true,completeRcaFieldsEnforced:true,duplicateRcaRejected:true,regressionAppendOnlyPolicyDeclared:true,preCorrectionExecutionRecordRequired:true,actualPreCorrectionProductExecutionEstablished:false,actualOperatorJourney:false,evidenceClass:'gate components with stipulated canonical records',canonicalPreCorrectionEvidenceRequired:true,preCorrectionFailureRequired:true,postCorrectionSuccessNotClaimedAtStage15:true,isolatedDisposableProject:true}));
