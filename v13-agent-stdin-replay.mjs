...
const BASE_NEW = "const prompt=promptB64?Buffer.from(promptB64,'base64').toString('utf8'):fs.readFileSync(0,'utf8');";
...
function ensureExact(text, oldValue, newValue, label) {
  if (text.includes(newValue)) return text;
  if (!text.includes(oldValue)) throw new Error(`${label} is neither the supported old form nor the required current form`);
  return text.replace(oldValue, newValue);
}

let base = fs.readFileSync('self-e2e-agent-base.mjs', 'utf8');
base = ensureExact(base, BASE_OLD, BASE_NEW, 'base agent prompt transport');
fs.writeFileSync('self-e2e-agent-base.mjs', base);

let wrapper = fs.readFileSync('self-e2e-agent.mjs', 'utf8');
wrapper = ensureExact(wrapper, WRAPPER_OLD, WRAPPER_NEW, 'wrapper-to-base prompt transport');
...
if (!fs.readFileSync('self-e2e-agent-base.mjs', 'utf8').includes(BASE_NEW)) throw new Error('base agent stdin transport not installed');
