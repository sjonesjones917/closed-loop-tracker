...
project.stages.forEach((stage, i) => {
  if (stage.number !== i + 1) fail(`Stage ${i + 1} has the wrong number.`);
  if (stage.status !== 'COMPLETE') fail(`Stage ${i + 1} is not COMPLETE.`);
});
for (const name of exactStages) {
  if (!html.includes(`[\"${name}\"`) && !html.includes(`["${name}"`)) fail(`Application is missing stage: ${name}`);
}
...
console.log(JSON.stringify({status:'PASS',selfProject:true,aboutApplicationItself:true,stages:31,appSha256,projectSha256}, null, 2));
