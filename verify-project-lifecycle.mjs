...
for(const token of ['currentScopeSelectorCoverage','exactReqRunTestCoverage','applicableCurrentRegressionSuccess','mandatoryEvidenceChainStructuralCoverage','mandatoryEvidenceSufficiencyCoverage','releaseArtifactIdentityCoverage'])assert(pages.includes(token),`Acceptance reduction lost required invariant ${token}.`);
for(const token of ['unauthorizedFieldMutationsAccepted:0','canonicalMutationsBeforeAcceptance:0','partialCommitsAfterInjectedFailure:0','staleProposalsAccepted:0','crossProjectRelationshipsAccepted:0','historicalScopeSatisfyingCurrentGates:0','unmatchedDeliveryFilesAuthorized:0','appendOnlyHistoryRewritesAccepted:0','unsupportedTestIrTreatedAsExecutable:0','externalAssertionsOverridingApplicationProof:0','nativeExecutionReceiptsFabricatedExternally:0','releaseAcceptedWithContradiction:0'])assert(pages.includes(token),`Acceptance reduction lost required zero-valued failure invariant ${token}.`);
