...
assert(store.includes("embeddedSchema==='closed-loop-project/2'")&&store.includes("embeddedSchema!==schemaApi?.PROJECT_SCHEMA")&&store.includes('embeddedProjectSha256'),'Complete package import must accept the current project schema and deterministically migrate verified /2 package state without hashing the migrated state as if it were the original package bytes.');
assert(pages.includes('node verify-definition-of-done.mjs')&&pages.includes("const definition=JSON.parse(fs.readFileSync('/tmp/verify-definition-of-done.out','utf8'))")&&pages.includes('...definition'),'Acceptance reduction must consume the executed definition-of-done proof rather than duplicate its invariant names in workflow text.');
assert(pages.includes('node verify-semantic-invariant.mjs')&&pages.includes("const semantic=Object.assign({},...jsonLines('/tmp/verify-semantic-invariant.out'))")&&pages.includes('semanticFalseAcceptanceInvariant'),'Acceptance reduction must consume the executed semantic false-acceptance proof.');
...
console.log(JSON.stringify({projectLifecycleControls:true,compactHeader:true,mobileProjectActionsVisible:true,dangerHiddenByDefault:true,transactionalDeleteRetained:true,lifecycleMetadataDeleteAtomic:true,durableAttemptAbandonment:true,canonicalBlobReverification:true,applicationCustodyBlocking:true,custodyFailureRecoveryBehavior:true,staleDeliveryAuthorizationNotResurrected:true,perProjectBackupState:true,currentPackageRoundTripAndLegacyMigration:true,zeroLossAcceptanceReduction:true,unsafeOverrides:0}));
