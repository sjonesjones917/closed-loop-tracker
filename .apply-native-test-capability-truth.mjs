...
const readLines=path=>fs.readFileSync(path,'utf8').split('\n');
const writeLines=(path,lines)=>fs.writeFileSync(path,lines.join('\n'));
function uniqueIndex(lines,needle,path){const matches=[];for(let i=0;i<lines.length;i++)if(lines[i].includes(needle))matches.push(i);if(matches.length!==1)throw new Error(`${path}: expected one line containing ${needle}, found ${matches.length}`);return matches[0];}
function replaceLine(path,needle,replacement){const lines=readLines(path),i=uniqueIndex(lines,needle,path);lines.splice(i,1,...(Array.isArray(replacement)?replacement:[replacement]));writeLines(path,lines);}
function insertBeforeLine(path,needle,newLines){const lines=readLines(path),i=uniqueIndex(lines,needle,path);lines.splice(i,0,...newLines);writeLines(path,lines);}
function insertAfterLine(path,needle,newLines){const lines=readLines(path),i=uniqueIndex(lines,needle,path);lines.splice(i+1,0,...newLines);writeLines(path,lines);}
function replaceText(path,from,to){const text=fs.readFileSync(path,'utf8'),first=text.indexOf(from),last=text.lastIndexOf(from);if(first<0||first!==last)throw new Error(`${path}: expected one exact text anchor`);fs.writeFileSync(path,text.slice(0,first)+to+text.slice(first+from.length));}

// Fail closed unless APPLICATION_DETERMINISTIC names an actually registered per-test browser executor.
insertBeforeLine('workflow-engine.js','const TEST_EXECUTION_ACTIONS=Object.freeze({',[
...
replaceLine('workflow-engine.js',"APPLICATION_DETERMINISTIC:'No operator execution is required now.","  APPLICATION_DETERMINISTIC:'No operator execution is required only when the exact REQUIRED_CAPABILITY names a registered application-native test executor. Otherwise request a corrected test definition; the browser must not pretend it can run the test.',");
replaceLine('workflow-engine.js',"return {testId:recordId(test,'tests'),requirementId:testRequirementId(test),testType:upper(recordValue(test,'TEST_TYPE'))",[
...
insertAfterLine('workflow-engine.js',"const unavailableTestIds=items.filter(item=>item.executionMode==='UNAVAILABLE')",[
...
replaceLine('workflow-engine.js','return {total:items.length,counts,incompleteTestIds,unavailableTestIds,items};',"  return {total:items.length,counts,incompleteTestIds,unavailableTestIds,unsupportedApplicationTestIds,items};");
insertAfterLine('workflow-engine.js','if(unavailable.length)reasons.push(`',[
...
// First-class semantic contradictions and a deterministic Stage 06 gate regression.
replaceLine('verify-prompt-semantics.mjs','TEST_DEFINITION_ARTIFACT_BOUNDARY_MISSING',[
...
insertBeforeLine('verify-prompt-semantics.mjs','let checked=0;',[
  '{',
...
insertBeforeLine('verify-complete.mjs','// Invalid canonical relationship is rejected before mutation.',[
