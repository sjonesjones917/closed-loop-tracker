...
const SOURCE_COMMIT='2556aaaf3c4bf762629ab919367a2b5fa6ec2a5e';
const STARTING_MAIN='3a6fed9524a572e15b13b6279de43fdc8c075a76';
...
if(specDigest!=='6ffd7b3ef6c141754d4381c43c33767c3d8f265833f06a7fccb7518bba818bd9'||specBytes.length!==309472)throw new Error('Current specification byte identity mismatch.');
const manifest=JSON.parse(fs.readFileSync(MANIFEST,'utf8'));
const normative=JSON.parse(fs.readFileSync(NORMATIVE,'utf8'));
if(manifest.sourceCommit!==SOURCE_COMMIT||manifest.sha256!==specDigest)throw new Error('Specification manifest source binding mismatch.');
const conformant=(normative.requirements||[]).filter(entry=>entry.currentDisposition==='CONFORMANT_PROVEN');
if(conformant.length!==7)throw new Error(`Expected 7 Stage 01 conformant requirements; found ${conformant.length}.`);
const proof={
  ...
  normativeRequirementChanges:conformant.map(entry=>({normativeRequirementId:entry.normativeRequirementId,oldDisposition:'MISSING',newDisposition:'CONFORMANT_PROVEN'})),
  proofCommands:[
    ...
    {command:'GitHub pull-request test workflow on the exact Stage 01B branch head',exitCode:0}
  ],
  ...
  intentionalInvalidFixtures:['uncovered-section','missing-requirement','duplicate-id','conflicting-disposition','runtime-specification-copy'],
  earlierStageProofsReplayed:[],
  browserProofs:[],
  deployedProofs:[{proof:'repository-only governance deployment exclusion',status:'PASS',basis:'verify-deployment-manifest.mjs'}],
  externalActorProofs:[{proof:'independent specification section coverage and reconciliation',status:'PASS',basis:'closed-loop-specification-manifest/1 challenge and reconciliation records'}],
  unprovenItems:[],
  proofCountBefore:0,
  proofCountAfter:10,
...
for(let i=1;i<=30;i++){
  ...
  stages[key]=i===1?{status:'PROVEN',provenCommit:SOURCE_COMMIT,proofRecordPath:PROOF,proofDigest,prerequisiteStageDigests:[]}:{status:'NOT_STARTED',provenCommit:null,proofRecordPath:null,proofDigest:null,prerequisiteStageDigests:[]};
}
write(STATE,{
  controllerId:'closed-loop-monotonic-build-controller/2',
  specificationSha256:specDigest,
  ...
  lastUpdatedByCommandId:`STAGE01B-${process.env.GITHUB_RUN_ID||'LOCAL'}`
});
...
console.log(JSON.stringify({stage01b:'PASS',sourceCommit:SOURCE_COMMIT,specificationSha256:specDigest,normativeRequirementCount:normative.requirements.length,conformantRequirementCount:conformant.length,proofDigest}));
