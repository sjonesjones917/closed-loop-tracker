...
prompt=replaceOnce(prompt,"const PROMPT_ENGINE_VERSION='closed-loop-prompt-engine/14';","const PROMPT_ENGINE_VERSION='closed-loop-prompt-engine/15';",'prompt engine version');
prompt=replaceOnce(
  prompt,
...
app=replaceOnce(
  ...
  'parsed-response rejection state'
);
...
semantics=replaceOnce(
  ...
  'replacement response semantic check'
);
semantics=replaceOnce(
  semantics,
...
browser=replaceOnce(
  ...
  "await fill(cdp,'#stage-output','{\"schema\":}');await click(cdp,'#parse-output');await waitExpr(cdp,`document.body.innerText.includes('Previous parsed response was rejected.')`);assert(await evalValue(cdp,`(()=>{const v=document.querySelector('#validation-report'),o=document.querySelector('#stage-output');return Boolean(v&&o&&(v.compareDocumentPosition(o)&Node.DOCUMENT_POSITION_FOLLOWING));})()`),'Validation status must appear before the response box so a phone user can see what the currently rejected response means.');retained=await activeProject(cdp);assert(retained.projectData.sources.length===0,'Malformed response mutated canonical sources.');assert(retained.projectData.rawResponses.length>=1&&retained.projectData.responseValidations.at(-1).valid===false,'Malformed raw response/validation not preserved.');await fill(cdp,'#stage-output','{\"replacement\":true}');await waitExpr(cdp,`document.body.innerText.includes('Replacement response not evaluated.')`);assert(await evalValue(cdp,`document.querySelector('#validation-report')?.classList.contains('warn')`),'Replacement response did not replace the stale rejection state with an unparsed warning.');",
  'replacement response browser state'
);
browser=replaceOnce(
  browser,
  "await click(cdp,'#new-project');await waitExpr(cdp,`document.body.innerText.includes('Save User Job Input')`);const afterNew=await allProjects(cdp);",
  "await click(cdp,'#new-project');await waitExpr(cdp,`document.body.innerText.includes('Save User Job Input')`);await click(cdp,'details.record-card summary');let projectHelp=(await snapshot(cdp)).text;for(const token of ['What belongs here?','You do not need to know the final file format in advance','A specification substitute requires human confirmation'])assert(projectHelp.includes(token),`Collapsed Project intake help missing ${token}.`);const afterNew=await allProjects(cdp);",
  'collapsed Project help browser check'
);
write('verify-browser.mjs',browser);

console.log('Applied exact residual human-stage prompt and operator UX changes.');
