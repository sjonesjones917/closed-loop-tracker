...
function validateValue(definition,value,path,issues,{required=false,maxTextFieldLength=schema.DEFAULT_RESOURCE_LIMITS.maxTextFieldLength}={}){if(value===null){if(!definition.nullable)issues.push(issue('PROHIBITED_NULL',path,'Null is not permitted.'));return;}const t=definition.valueType;const emptyString=typeof value==='string'&&!value.trim();if(required&&emptyString)issues.push(issue('EMPTY_REQUIRED_STRING',path,'Required string is empty.'));if(typeof value==='string'&&value.trim()==='<value>')issues.push(issue('PLACEHOLDER_VALUE',path,'Prompt placeholder <value> cannot be accepted as data.'));if(t==='STRING'&&typeof value!=='string')issues.push(issue('WRONG_VALUE_TYPE',path,'Expected STRING.'));else if(t==='INTEGER'&&(!Number.isInteger(value)||!Number.isFinite(value)))issues.push(issue('WRONG_VALUE_TYPE',path,'Expected finite INTEGER.'));else if(t==='NUMBER'&&(typeof value!=='number'||!Number.isFinite(value)))issues.push(issue('WRONG_VALUE_TYPE',path,'Expected finite NUMBER.'));else if(t==='BOOLEAN'&&typeof value!=='boolean')issues.push(issue('WRONG_VALUE_TYPE',path,'Expected BOOLEAN.'));else if((t==='STRING_ARRAY'||t==='REFERENCE_ARRAY')&&!Array.isArray(value))issues.push(issue('WRONG_VALUE_TYPE',path,`Expected ${t}.`));else if(t==='STRING_ARRAY'&&Array.isArray(value)&&value.some(v=>typeof v!=='string'))issues.push(issue('WRONG_VALUE_TYPE',path,'STRING_ARRAY items must be strings.'));else if(t==='OBJECT'&&!object(value))issues.push(issue('WRONG_VALUE_TYPE',path,'Expected OBJECT.'));if(required&&Array.isArray(value)&&!value.length)issues.push(issue('EMPTY_REQUIRED_ARRAY',path,'Required array is empty.'));if(definition.enumValues?.length&&!definition.enumValues.includes(value))issues.push(issue('INVALID_ENUM_VALUE',path,`Value must be one of ${definition.enumValues.join(', ')}.`));if(typeof value==='string'&&value.length>maxTextFieldLength)issues.push(issue('TEXT_FIELD_TOO_LARGE',path,'Text field exceeds the configured maximum length.'));if(t==='OBJECT'&&object(value)&&definition.closedProperties)for(const key of Object.keys(value))if(!definition.closedProperties.includes(key))issues.push(issue('UNKNOWN_NESTED_PROPERTY',`${path}/${pointerEscape(key)}`,`Unknown nested property ${key}.`));}
function currentScope(project,promptRecord){return clone(promptRecord?.scope||{});}
const PROMPT_LANE_SCOPE_KEYS=Object.freeze(['iterationId','candidateId','runId','contextId','baselineId','productId']);
function samePromptLane(left,right){if(!left||!right||Number(left.stage)!==Number(right.stage)||String(left.operation||'')!==String(right.operation||''))return false;return PROMPT_LANE_SCOPE_KEYS.every(key=>JSON.stringify(left.scope?.[key]??null)===JSON.stringify(right.scope?.[key]??null));}
function latestPromptForLane(project,promptRecord){return safe(project?.projectData?.generatedPrompts).filter(record=>!record.invalidatedBy&&samePromptLane(record,promptRecord)).at(-1)||null;}
...
function validateEnvelope(project,envelope,{stage,promptRecord,rawSha256,suppliedFiles=[]}={}){
  workflow.ensureShape(project);
  const issues=[];
  const stageNumber=Number(stage);
  const stageContract=schema.STAGE_CONTRACTS[stageNumber];
  if(!stageContract)issues.push(issue('INVALID_STAGE','/stage',`Stage ${stage} is outside the 30-stage workflow.`));
  ...
  const expectedOperation=promptRecord?.operation||stageContract?.operations?.[0];const contract=schema.operationContract(stageNumber,expectedOperation);if(!contract)issues.push(issue('INVALID_OPERATION_CONTRACT','/operation',`Operation ${expectedOperation||'UNKNOWN'} has no Stage ${stageNumber} contract.`));if(contract&&!contract.responseRequired)issues.push(issue('APPLICATION_CONTROLLED_OPERATION','/operation',`Stage ${stageNumber} operation ${expectedOperation} does not accept an external-agent response.`));if(String(envelope.operation||'')!==String(expectedOperation||''))issues.push(issue('WRONG_OPERATION','/operation',`Expected operation ${expectedOperation||'UNKNOWN'}.`));
  if(!object(envelope.scope))issues.push(issue('INVALID_SCOPE','/scope','scope must be an object.'));else{unknownKeys(envelope.scope,['projectRevision','inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId','runId','contextId','baselineId','productId'],'/scope',issues);const expected=currentScope(project,promptRecord);const requiredScope=new Set(['projectRevision',...(contract?.scopeRequirements||[])]);for(const key of requiredScope)if(JSON.stringify(envelope.scope[key]??null)!==JSON.stringify(expected[key]??null))issues.push(issue('STALE_SCOPE',`/scope/${key}`,`Scope ${key} does not match the controlling prompt.`));}
  ...
  if(!object(envelope.promptIdentity))issues.push(issue('MISSING_PROMPT_IDENTITY','/promptIdentity','promptIdentity must be an object.'));
    ...
    const latest=latestPromptForLane(project,promptRecord);
  ...
  if(object(envelope.stageData)){
    for(const [name,value] of Object.entries(envelope.stageData)){
      ...
      validateValue(definition,value,path,issues,{maxTextFieldLength:stageContract?.resourceLimits?.maxTextFieldLength||schema.DEFAULT_RESOURCE_LIMITS.maxTextFieldLength});
  ...
  const allowedCollections=new Set(contract?.agentWritableCollections||[]);
  for(const [collection,list] of Object.entries(envelope.records||{}))if(Array.isArray(list)&&list.length>(stageContract?.resourceLimits?.maxRecordsPerCollection||250))issues.push(issue('RESOURCE_LIMIT_EXCEEDED',`/records/${pointerEscape(collection)}`,'Too many records in collection.'));
  if(safe(envelope.evidence).length>(stageContract?.resourceLimits?.maxEvidenceRecords||500))issues.push(issue('RESOURCE_LIMIT_EXCEEDED','/evidence','Too many evidence records.'));if(safe(envelope.attachments).length>(stageContract?.resourceLimits?.maxAttachments||25))issues.push(issue('RESOURCE_LIMIT_EXCEEDED','/attachments','Too many attachments.'));
  ...
  if(object(envelope.records)){
    for(const [collection,list] of Object.entries(envelope.records)){
      ...
      list.forEach((record,index)=>{
        ...
        const identityCount=Number(Boolean(record.tempKey))+Number(Boolean(record.targetId));if(identityCount!==1)issues.push(issue('INVALID_RECORD_IDENTITY',path,'Provide exactly one of tempKey or targetId.'));if(definition.commitPolicy===schema.COLLECTION_POLICIES.UPDATE_RESERVED&&record.tempKey)issues.push(issue('RESERVED_TARGET_REQUIRED',`${path}/tempKey`,`${collection} is application-reserved and must be completed through targetId.`));
        ...
        if(record.targetId){
          const target=workflow.records(project,collection,{active:true}).find(existing=>workflow.recordId(existing,collection)===String(record.targetId));
          if(!target)issues.push(issue('INVALID_RESERVED_TARGET',`${path}/targetId`,`Target ${record.targetId} does not exist as an active ${collection} record.`));
          else{
            if(Number(target.stage)!==stageNumber)issues.push(issue('WRONG_RESERVED_TARGET_STAGE',`${path}/targetId`,`Target ${record.targetId} belongs to Stage ${target.stage??'UNKNOWN'}, not Stage ${stageNumber}.`));
            if(!['RESERVED','PENDING_AGENT','OPEN'].includes(upper(workflow.recordValue(target,'STATUS')||target.status||'RESERVED')))issues.push(issue('INVALID_RESERVED_TARGET',`${path}/targetId`,'Target record is not in an agent-completable reserved state.'));
            const expectedScope=currentScope(project,promptRecord),targetScope=target.scope||{};for(const key of ['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId','baselineId','productId'])if(targetScope[key]!=null&&JSON.stringify(targetScope[key])!==JSON.stringify(expectedScope[key]??null))issues.push(issue('WRONG_RESERVED_TARGET_SCOPE',`${path}/targetId`,`Target ${record.targetId} has stale ${key}.`));
            if(collection==='runs'&&expectedScope.runId&&String(record.targetId)!==String(expectedScope.runId))issues.push(issue('WRONG_RUN_TARGET',`${path}/targetId`,`Run target must equal prompt scope runId ${expectedScope.runId}.`));
            if(collection==='runs'&&expectedScope.contextId&&String(workflow.recordValue(target,'CONTEXT_ID')||target.relationships?.CONTEXT_ID||'')!==String(expectedScope.contextId))issues.push(issue('WRONG_CONTEXT_TARGET',`${path}/targetId`,`Run target does not belong to prompt scope contextId ${expectedScope.contextId}.`));
            if(collection==='products'&&expectedScope.productId&&String(record.targetId)!==String(expectedScope.productId))issues.push(issue('WRONG_PRODUCT_TARGET',`${path}/targetId`,`Product target must equal prompt scope productId ${expectedScope.productId}.`));
          }
        }
        ...
        for(const [name,value] of Object.entries(record.fields)){
          ...
          validateValue(fieldDefinition,value,fieldPath,issues,{required:definition.required.includes(name),maxTextFieldLength:stageContract?.resourceLimits?.maxTextFieldLength||schema.DEFAULT_RESOURCE_LIMITS.maxTextFieldLength});
        ...
        for(const required of definition.required){
          ...
          if(def?.producer===schema.PRODUCER.AGENT){const value=record.fields[required];if(value===undefined||value===null||(typeof value==='string'&&!value.trim())||(Array.isArray(value)&&!value.length))issues.push(issue('MISSING_REQUIRED_FIELD',`${path}/fields/${pointerEscape(required)}`,`${required} is required.`));continue;}
          if(def?.producer===schema.PRODUCER.APPLICATION&&def.valueType==='REFERENCE'&&!record.targetId&&!record.relationships?.[required])issues.push(issue('MISSING_REQUIRED_RELATIONSHIP',`${path}/relationships/${pointerEscape(required)}`,`${required} must be resolved by an explicit canonical relationship.`));
        }
  ...
  if(object(envelope.records))for(const [collection,list] of Object.entries(envelope.records))if(Array.isArray(list))list.forEach((record,index)=>{
    ...
    if(object(record?.relationships))for(const [name,reference] of Object.entries(record.relationships)){
      ...
      if(reference.tempKey){
        const target=expectedCollection==='evidenceRecords'?(evidenceIndex.has(String(reference.tempKey))?{collection:'evidenceRecords'}:null):responseRecordIndex.get(String(reference.tempKey));
  ...
  if(Array.isArray(envelope.attachments))envelope.attachments.forEach((attachment,index)=>{
    ...
    if(!String(attachment.mediaType||'').trim())issues.push(issue('MISSING_ATTACHMENT_MEDIA_TYPE',`${path}/mediaType`,'mediaType is required.'));
    if(!Number.isInteger(attachment.byteSize)||attachment.byteSize<0)issues.push(issue('INVALID_ATTACHMENT_BYTE_SIZE',`${path}/byteSize`,'byteSize must be a non-negative integer.'));
    if(!/^[a-f0-9]{64}$/i.test(String(attachment.sha256||'')))issues.push(issue('INVALID_ATTACHMENT_SHA256',`${path}/sha256`,'sha256 must be a 64-character hexadecimal SHA-256.'));
    const candidates=safe(suppliedFiles).filter(file=>String(file.filename||file.name||'')===String(attachment.filename||''));
    const exact=candidates.find(file=>String(file.mediaType||file.type||'application/octet-stream')===String(attachment.mediaType||'')&&Number(file.byteSize??file.size)===Number(attachment.byteSize)&&String(file.sha256||'').toLowerCase()===String(attachment.sha256||'').toLowerCase());
    if(!exact&&attachment.required!==false){
      if(!candidates.length)issues.push(issue('MISSING_REQUIRED_ATTACHMENT',path,`Required attachment ${attachment.filename||'UNKNOWN'} was not supplied to the application.`));
      else{if(!candidates.some(file=>String(file.mediaType||file.type||'application/octet-stream')===String(attachment.mediaType||'')))issues.push(issue('ATTACHMENT_MEDIA_TYPE_MISMATCH',`${path}/mediaType`,'Declared media type does not match the supplied file.'));if(!candidates.some(file=>Number(file.byteSize??file.size)===Number(attachment.byteSize)))issues.push(issue('ATTACHMENT_BYTE_SIZE_MISMATCH',`${path}/byteSize`,'Declared byte size does not match the supplied file bytes.'));if(!candidates.some(file=>String(file.sha256||'').toLowerCase()===String(attachment.sha256||'').toLowerCase()))issues.push(issue('ATTACHMENT_SHA256_MISMATCH',`${path}/sha256`,'Declared SHA-256 does not match the application-computed file hash.'));}
    }
    if(exact){const artifactId=exact.artifactId||exact.id;if(!artifactId||!workflow.records(project,'artifacts',{active:true}).some(record=>workflow.recordId(record,'artifacts')===String(artifactId)&&String(workflow.recordValue(record,'SHA256')||record.sha256||'').toLowerCase()===String(attachment.sha256||'').toLowerCase()))issues.push(issue('UNVERIFIED_ATTACHMENT_BYTES',path,'The supplied file has no matching current canonical artifact bytes record.'));}
  });
...
function planProposal(project,envelope,{rawRecord,promptRecord,validationRecord,expectedProjectRevision=Number(project.revision||0)}){
  ...
  for(const attachment of safe(envelope.attachments)){
    const match=safe(rawRecord.files).find(file=>String(file.filename||file.name||'')===String(attachment.filename||'')&&String(file.mediaType||file.type||'application/octet-stream')===String(attachment.mediaType||'')&&Number(file.byteSize??file.size)===Number(attachment.byteSize)&&String(file.sha256||'').toLowerCase()===String(attachment.sha256||'').toLowerCase());
    if(match?.artifactId||match?.id)tempToCanonical[attachment.temporaryKey]={collection:'artifacts',id:String(match.artifactId||match.id)};
  }
  const evidence=[];
  ...
  return {
    ...
    bodySha256:promptRecord.bodySha256||promptRecord.sha256,promptSha256:promptRecord.bodySha256||promptRecord.sha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature,scopeSha256:promptRecord.scopeSha256,jobId:envelope.jobId,stage:Number(envelope.stage),operation:envelope.operation,responseSchemaVersion:envelope.schema,responseType:envelope.responseType,
...
function createReceipt(project,{stage,promptRecord,rawRecord,validationRecord,proposal}){
  ...
  const receipt={
    receiptId,jobId:project.job.JOB_ID,stage:Number(stage),role:globalThis.closedLoopCore?.STAGES?.[Number(stage)-1]?.role||'UNKNOWN',contextId:promptRecord.scope?.contextId||'NOT APPLICABLE',
    iteration:project.job.CURRENT_ITERATION||'NOT APPLICABLE',runId:promptRecord.scope?.runId||'NOT APPLICABLE',requestDateTime:promptRecord.generatedAt||'UNKNOWN',responseDateTime:rawRecord.createdAt,
...
function prepareCaptured(project,{rawResponseId,promptRecord=null,expectedCommittedRevision=null}={}){
  ...
  let validation;if(parseError)validation={valid:false,issues:[issue(parseError.code||'MALFORMED_JSON','/',parseError.message)],errorCount:1,warningCount:0,checkedAt:now(),responseSchema:null,responseType:null};else validation=validateEnvelope(next,envelope,{stage:stageNumber,promptRecord:prompt,rawSha256:rawRecord.sha256,suppliedFiles:rawRecord.files});
