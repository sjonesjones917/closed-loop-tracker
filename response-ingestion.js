...
function ensureProposalCurrent(project,proposal){
  if(!proposal)throw new Error('Response proposal does not exist.');if(proposal.status==='ACCEPTED'||proposal.status==='QUESTIONS_CREATED'||proposal.status==='BLOCKER_ACCEPTED'||proposal.status==='EXECUTION_FAILURE_ACCEPTED')return {idempotent:true};if(['REJECTED','CORRECTION_REQUESTED'].includes(proposal.status)){const e=new Error(`Response proposal is ${proposal.status} and cannot be accepted.`);e.code='PROPOSAL_REJECTED';throw e;}if(proposal.status!=='PENDING_OPERATOR_REVIEW')throw new Error(`Response proposal is ${proposal.status}, not pending review.`);
