...
if(!html.includes('data-generate-production-instruction')){
  ...
  html=replaceOnce(html,"9:['preflightReviews']","9:['productionInstructions','preflightReviews']",'Stage 9 record groups');
...
function buildProductionPrompt(project,{preflighted=false}={}){
  ...
  return [
    ...
    preflighted?'PREFLIGHT CONDITION\\nThis version supersedes the prior production prompt only after Stage 9 records the review. Preserve all substantive requirements; corrections may remove ambiguity or omissions but may not silently change user intent or external authority.':''
  ].filter(Boolean).join('\\n\\n');
}
...
`;
  ...
  html=replaceOnce(html,"if(target.id==='saveJobBtn'){saveJob();return}if(target.dataset.createRuns)","if(target.id==='saveJobBtn'){saveJob();return}if(target.hasAttribute('data-generate-production-instruction')){generateProductionInstruction();return}if(target.hasAttribute('data-generate-preflighted-instruction')){generatePreflightedInstruction();return}if(target.dataset.createRuns)",'prompt generation click handlers');
}

if(!html.includes("Stage 8 requires an actual generated production prompt.")){
  html=replaceOnce(html,"case 8:requireRecords('productionInstructions');break;","case 8:{const instructions=requireRecords('productionInstructions');const generated=instructions.filter(r=>Number(r.stageNumber)===8&&String(r.promptText||'').trim());if(!generated.length)throw Error('Stage 8 requires an actual generated production prompt. Use Generate production prompt, then review or edit it before completion.');break}",'Stage 8 prompt completion gate');
  html=replaceOnce(html,"case 9:{const reviews=requireRecords('preflightReviews');if(reviews.some(r=>!['RESOLVED','NONE_FOUND'].includes(r.status)))throw Error('All preflight records must be RESOLVED or NONE_FOUND.');break}","case 9:{const reviews=requireRecords('preflightReviews');if(reviews.some(r=>!['RESOLVED','NONE_FOUND'].includes(r.status)))throw Error('All preflight records must be RESOLVED or NONE_FOUND.');const prompts=currentRecords(project,'productionInstructions',9).filter(r=>String(r.promptText||'').trim());if(!prompts.length)throw Error('Stage 9 requires a separately versioned preflighted production prompt. Use Generate preflighted prompt before completion.');break}",'Stage 9 prompt completion gate');
}
...
console.log(JSON.stringify({status:'PASS',promptGeneration:true,stage8:true,stage9:true,completionGates:true,htmlBytes:manifest.htmlBytes,htmlSha256:manifest.htmlSha256,compressedBytes:manifest.compressedBytes,parts:manifest.partCount},null,2));
