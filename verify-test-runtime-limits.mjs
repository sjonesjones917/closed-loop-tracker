...
const workerSource=fs.readFileSync(new URL('./test-worker.js',import.meta.url)),workerBytes=new Uint8Array(workerSource),workerArrayBuffer=workerBytes.buffer.slice(workerBytes.byteOffset,workerBytes.byteOffset+workerBytes.byteLength);
const attestedFetch=async url=>({ok:true,status:200,redirected:false,url:String(url),arrayBuffer:async()=>workerArrayBuffer.slice(0)});
...
const sharedBuffer=new ArrayBuffer(Math.floor(runtime.LIMITS.maxTotalInputBytes/2)+1),leftView=new Uint8Array(sharedBuffer),rightView=new Uint8Array(sharedBuffer);
await rejectsCode(runtime.execute({spec:spec([{op:'LOAD_ARTIFACT',binding:'LEFT'},{op:'READ_BYTES'},{op:'BYTE_COMPARE',binding:'RIGHT'},{op:'ASSERT_EQ',value:true}]),artifacts:{LEFT:binding('ART-LEFT',leftView),RIGHT:binding('ART-RIGHT',rightView)},metadata:metadata(['LEFT','RIGHT'])}),'INPUT_BYTE_LIMIT');
...
const regexCanonical=canonicalInput('TEXT',regexInput);
await rejectsCode(runtime.execute({spec:spec([{op:'LOAD_ARTIFACT',binding:'TEXT'},{op:'ASSERT_MATCH',pattern:'a+'}]),canonicalBindings:{TEXT:regexCanonical.payload},metadata:{bindings:{TEXT:regexCanonical.descriptor}}}),'REGEX_INPUT_LIMIT');

const csvCells=runtime.LIMITS.maxCsvCells+1;const csvText=Array(csvCells).fill('x').join(',');
await rejectsCode(runtime.execute({spec:spec([{op:'LOAD_ARTIFACT',binding:'PRODUCT'},{op:'READ_BYTES'},{op:'DECODE_UTF8'},{op:'PARSE_CSV',delimiter:',',header:false,quote:'"',newline:'AUTO',encoding:'UTF-8'},{op:'COUNT'},{op:'ASSERT_EQ',value:1}]),artifacts:{PRODUCT:binding('ART-PRODUCT',encoder.encode(csvText))},metadata:metadata(['PRODUCT'])}),'CSV_CELL_LIMIT');

const xmlText='<root>'+Array(runtime.LIMITS.maxXmlNodes+1).fill('<n/>').join('')+'</root>';
await rejectsCode(runtime.execute({spec:spec([{op:'LOAD_ARTIFACT',binding:'PRODUCT'},{op:'READ_BYTES'},{op:'DECODE_UTF8'},{op:'PARSE_XML'},{op:'SELECT_XML',path:'/root/n'},{op:'COUNT'},{op:'ASSERT_EQ',value:1}]),artifacts:{PRODUCT:binding('ART-PRODUCT',encoder.encode(xmlText))},metadata:metadata(['PRODUCT'])}),'XML_NODE_LIMIT');
...
const hashResult=await runtime.execute({spec:spec([{op:'LOAD_ARTIFACT',binding:'PRODUCT'},{op:'READ_BYTES'},{op:'HASH_SHA256'},{op:'ASSERT_EQ',value:createHash('sha256').update(bytes).digest('hex')}]),artifacts:{PRODUCT:binding('ART-PRODUCT',bytes)},metadata:metadata(['PRODUCT'])});
...
const timeout=await runtime.executeTest(timeoutTest,{PRODUCT:binding('ART-TIMEOUT',encoder.encode('x'))},{},{Worker:SilentWorker,fetch:attestedFetch,timeoutMs:5,workerUrl:'test-worker.js'});
