...
function preparePromptPrerequisites(p,targetStage){
  if(targetStage<=1)return p;
  prepareStage4Upstream(p);
  for(let prior=4;prior<targetStage;prior++){p.stages[prior].status='COMPLETE';p.stages[prior].gate={complete:true,blocked:false,reasons:[]};}
  return p;
}
...
function validEnvelope(p,stage,promptRecord){
  ...
  const stageData={};
  const writableStageField=stageFields.find(name=>schema.STAGE_FIELDS[stage]?.[name]?.producer===schema.PRODUCER.AGENT);if(writableStageField)stageData[writableStageField]=safeValue(writableStageField);
  ...
  if(!Object.keys(stageData).length||stage===4){
    ...
    const fields={};
    for(const name of def.required){if(def.fieldDefinitions[name]?.producer===schema.PRODUCER.AGENT)fields[name]=def.fieldDefinitions[name]?.enumValues?.length?def.fieldDefinitions[name].enumValues[0]:safeValue(name);}
    ...
    if(stage===6&&collection==='tests'){fields.TEST_TYPE='MEANING';fields.EXECUTION_MODE='INDEPENDENT_AGENT_REVIEW';fields.REQUIRED_CAPABILITY='INDEPENDENT_AI_REVIEW';fields.ARTIFACT_REQUIREMENTS='NONE';fields.EXECUTABLE_KIND='NONE';fields.EXECUTABLE_SPEC={};fields.EXECUTABLE_INPUT_BINDINGS={};}
    records[collection]=[{tempKey:'record-1',fields,relationships:{},evidenceRefs:['evidence-1']}];
    if(stage===4&&collection==='requirements'){const obligationManifest=promptRecord.contextManifest.obligationManifest;records.requirements=(obligationManifest.items||[]).map((item,index)=>{const requirementFields={};for(const name of def.required){if(def.fieldDefinitions[name]?.producer===schema.PRODUCER.AGENT)requirementFields[name]=def.fieldDefinitions[name]?.enumValues?.length?def.fieldDefinitions[name].enumValues[0]:safeValue(name);}requirementFields.USER_INPUT_RELATIONSHIP=item.obligationId;return {tempKey:'requirement-'+String(index+1),fields:requirementFields,relationships:{},evidenceRefs:['evidence-1']};});}
  }
  return {
...
// Raw capture audit scope must be controlled by the persisted prompt, not a caller-supplied context hint.
{
  ...
  preparePromptPrerequisites(p,17);
...
{
  const p=project('JOB-UNPERSISTED-PROMPT'),stage=2;preparePromptPrerequisites(p,stage);const pr=prompts.buildPromptRecord(stage,p),e=validEnvelope(p,stage,pr);let rejected=false;
...
// Final boundary: naming a required executable/input artifact is not possession of its bytes.
{
  ...
  fields.TEST_TYPE='MEANING';fields.EXECUTION_MODE='INDEPENDENT_AGENT_REVIEW';fields.REQUIRED_CAPABILITY='INDEPENDENT_AI_REVIEW';fields.EXECUTABLE_KIND='NONE';fields.EXECUTABLE_SPEC={};fields.EXECUTABLE_INPUT_BINDINGS={};fields.ARTIFACT_REQUIREMENTS='fixture.js';
