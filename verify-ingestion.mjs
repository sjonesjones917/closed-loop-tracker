...
function fixturePromptOptions(stage,operation){
  const scope={};
  for(const key of schema.operationContract(stage,operation)?.scopeRequirements||[]){
    if(key==='runId')scope.runId=`RUN-FIXTURE-${stage}`;
    if(key==='contextId')scope.contextId=`CONTEXT-FIXTURE-${stage}`;
  }
...
function savePrompt(p,stage){
  ...
  const operation=stage===19?'COMPARE':'COMPLETE',options={...fixturePromptOptions(stage,operation),...(stage===19?{operation}: {})};
