...
{
 let p=project('JOB-INGEST-INDEPENDENCE');p.job.CURRENT_ITERATION='ITERATION-INGEST';const iteration={id:'ITERATION-INGEST',stage:10,active:true,scope:{iterationId:'ITERATION-INGEST',candidateId:'CANDIDATE-INGEST'},fields:{ITERATION_ID:'ITERATION-INGEST',CANDIDATE_ID:'CANDIDATE-INGEST'}};p.projectData.iterations.push(iteration);const run={id:'RUN-INGEST',stage:11,active:true,scope:{iterationId:'ITERATION-INGEST',candidateId:'CANDIDATE-INGEST'},fields:{RUN_ID:'RUN-INGEST',ITERATION_ID:'ITERATION-INGEST',CANDIDATE_ID:'CANDIDATE-INGEST',CONTEXT_ID:'CTX-GENERATOR'}};p.projectData.runs.push(run);const pr=savePrompt(p,12);const e=validEnvelope(p,12,pr);e.records={verification:[{tempKey:'verification-1',fields:{VERIFIER:'reviewer',VERIFIER_CONTEXT_ID:'CTX-GENERATOR',INDEPENDENCE_STATUS:'INDEPENDENT',INPUTS:'run',PROCEDURE:'verify',EXPECTED_RESULT:'pass',OBSERVED_RESULT:'pass',EXACT_EVIDENCE:'evidence',DETERMINATION:'SATISFIED'},relationships:{REQ_ID:{recordId:'REQ-DOES-NOT-MATTER'},RUN_ID:{recordId:'RUN-INGEST'},TEST_ID:{recordId:'TEST-DOES-NOT-MATTER'}},evidenceRefs:['evidence-1']}]};e.evidence=[{temporaryKey:'evidence-1',kind:'WORKFLOW_EVIDENCE',description:'independence fixture',location:'fixture',content:'evidence'}];const prepared=ingestion.prepare(p,{stage:12,text:JSON.stringify(e),promptRecord:pr});if(!prepared.validation.issues.some(x=>x.code==='VERIFIER_CONTEXT_REUSE'))throw new Error('Observable verifier/generator context reuse was not rejected.');if(prepared.project.projectData.verification.length!==0)throw new Error('Rejected verifier-context reuse partially mutated canonical verification.');
}
