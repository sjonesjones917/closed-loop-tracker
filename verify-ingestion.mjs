...
function prepareStage4Upstream(p){
  ...
  p.job.CURRENT_SOURCE_SET_VERSION='SOURCE-SET-v001';p.stages[2].status='COMPLETE';p.stages[2].gate={complete:true,blocked:false,reasons:[]};p.stages[2].agentData.SOURCE_APPLICABILITY_DETERMINATION='APPLICABLE_SOURCES_ESTABLISHED';
  if(!p.projectData.sources.some(record=>engine.recordId(record,'sources')==='SOURCE-INGESTION-UPSTREAM'))p.projectData.sources.push({id:'SOURCE-INGESTION-UPSTREAM',active:true,stage:2,scope:{inputVersion:p.job.CURRENT_INPUT_VERSION,sourceSetVersion:p.job.CURRENT_SOURCE_SET_VERSION},fields:{SOURCE_ID:'SOURCE-INGESTION-UPSTREAM',TITLE:'Independent ingestion fixture source'}});
  if(!p.projectData.research.some(record=>engine.recordId(record,'research')==='RESEARCH-INGESTION-UPSTREAM'))p.projectData.research.push({id:'RESEARCH-INGESTION-UPSTREAM',active:true,stage:3,scope:{inputVersion:p.job.CURRENT_INPUT_VERSION,sourceSetVersion:p.job.CURRENT_SOURCE_SET_VERSION},fields:{RESEARCH_ID:'RESEARCH-INGESTION-UPSTREAM',SOURCE_ID:'SOURCE-INGESTION-UPSTREAM',PASS_NUMBER:2,EXACT_PORTION_EXAMINED:'Complete fixture source',FINDING_CLASSIFICATION:'COMPLETE',SOURCE_EVIDENCE:'Fixture evidence',SATURATION_STATUS:'SATURATED'},relationships:{SOURCE_ID:'SOURCE-INGESTION-UPSTREAM'}});
...
let fixtureContextSequence=0;
function fixtureBuildPrompt(stage,p,options){
  preparePromptPrerequisites(p,stage);
  const selected={...(options||{})},operation=selected.operation||fixturePromptOperation(stage),independent=[9,12,23,24].includes(Number(stage))||([17,19].includes(Number(stage))&&operation==='VERIFY'),context=engine.registerFreshContext(p,{stage,externalContextIdentifier:`INGESTION-FIXTURE-${stage}-${operation}-${++fixtureContextSequence}`,operatorLabel:'INGESTION_VERIFIER',purpose:independent?'REVIEWER':'GENERAL'}),contextId=engine.recordId(context,'freshContexts');
...
function savePrompt(p,stage){
  ...
  Object.assign(p.projectData.generatedPrompts.find(item=>item.instructionId===record.instructionId),{generatedAt:record.generatedAt,iteration:record.iteration});
  return record;
}
function safeValue(name,definition=null){
  ...
  if(definition?.valueType==='INTEGER'||definition?.valueType==='NUMBER')return 1;
  if(definition?.valueType==='BOOLEAN')return true;
  if(definition?.valueType==='OBJECT')return {};
  if(definition?.valueType==='OBJECT_ARRAY')return [{}];
  if(definition?.valueType==='STRING_ARRAY'||definition?.valueType==='REFERENCE_ARRAY')return ['verified'];
...
function responseBinding(promptRecord){const binding=promptRecord.operationBinding;return {packageId:binding?.packageId||null,operationReservationId:binding?.operationReservationId||null,challengeNonce:binding?.challengeNonce||null,targetSlot:binding?.targetSlot||null};}
function validEnvelope(p,stage,promptRecord){
  ...
  if(agentStageFields.length)stageData[agentStageFields[0]]=safeValue(agentStageFields[0],schema.stageFieldDefinition(stage,agentStageFields[0]));
  ...
  if(!Object.keys(stageData).length||stage===4){
    ...
    const fields={};
    for(const name of def.required){if(def.fieldDefinitions[name]?.producer===schema.PRODUCER.AGENT)fields[name]=safeValue(name,def.fieldDefinitions[name]);}
    if(!Object.keys(fields).length){const agentField=schema.recordAgentFields(collection)[0];if(agentField)fields[agentField]=safeValue(agentField,def.fieldDefinitions[agentField]);}
    records[collection]=[{tempKey:'record-1',fields,relationships:{},evidenceRefs:['evidence-1']}];
    if(stage===4&&collection==='requirements'){const obligationManifest=promptRecord.contextManifest.obligationManifest;records.requirements=(obligationManifest.items||[]).map((item,index)=>{const requirementFields={};for(const name of def.required){if(def.fieldDefinitions[name]?.producer===schema.PRODUCER.AGENT)requirementFields[name]=safeValue(name,def.fieldDefinitions[name]);}requirementFields.USER_INPUT_RELATIONSHIP=item.obligationId;return {tempKey:'requirement-'+String(index+1),fields:requirementFields,relationships:{},evidenceRefs:['evidence-1']};});}
  }
  return {
    schema:schema.RESPONSE_SCHEMA,
    jobId:p.job.JOB_ID,
    stage,
    ...responseBinding(promptRecord),
...
function blockedEnvelope(p,stage,promptRecord){return {schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,...responseBinding(promptRecord),operation:promptRecord.operation,promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:{...promptRecord.scope},responseType:'BLOCKED',humanInputRequests:[],stageData:{},records:{},evidence:[],unresolved:[{temporaryKey:'u-1',kind:'MISSING_CAPABILITY',description:'Controlled blocked fixture',whyBlocking:'Scope identity validation fixture.',affectedStageFields:[],affectedRecords:[],blocking:true}],warnings:[],attachments:[]};}
...
function addConditionalActivationFixture(envelope){
  const requirements=envelope.records?.requirements||[];
  if(requirements.length<2)throw new Error('Stage 04 conditional-activation fixture requires at least two current obligation-backed requirements.');
  requirements[0].fields.MANDATORY_OPTIONAL_STATUS='CONDITIONAL';requirements[1].fields.MANDATORY_OPTIONAL_STATUS='MANDATORY';
  const definition=schema.RECORD_SCHEMAS.propositions,fields=label=>Object.fromEntries(definition.required.filter(name=>definition.fieldDefinitions[name]?.producer===schema.PRODUCER.AGENT).map(name=>[name,`${label} ${name.toLowerCase().replaceAll('_',' ')}`]));
  envelope.records.propositions=[
    {tempKey:'conditional-target',fields:fields('Conditional target'),relationships:{REQUIREMENT_ID:{tempKey:requirements[0].tempKey},ACTIVATION_PROPOSITION_ID:{tempKey:'activation-source'}},evidenceRefs:['evidence-1']},
    {tempKey:'activation-source',fields:fields('Activation condition'),relationships:{REQUIREMENT_ID:{tempKey:requirements[1].tempKey}},evidenceRefs:['evidence-1']}
  ];
  return envelope;
}

// Conditional activation is a same-response relationship proposal; the application alone resolves canonical IDs.
{
  let p=project('JOB-STAGE4-ACTIVATION-BINDING'),stage=4,promptRecord=savePrompt(p,stage),envelope=addConditionalActivationFixture(validEnvelope(p,stage,promptRecord));
  const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(envelope),promptRecord});
  if(!prepared.validation.valid)throw new Error(`Valid Stage 04 activation relationship rejected: ${JSON.stringify(prepared.validation.issues)}`);
  const target=prepared.proposal.canonicalRecords.propositions.find(record=>record.temporaryKey==='conditional-target'),activation=prepared.proposal.canonicalRecords.propositions.find(record=>record.temporaryKey==='activation-source');
  if(!target||!activation||target.fields.ACTIVATION_PROPOSITION_ID!==activation.id||target.relationships.ACTIVATION_PROPOSITION_ID!==activation.id)throw new Error('Stage 04 activation tempKey was not application-resolved to the exact allocated proposition ID.');
  ...
  const canonicalTarget=engine.recordsForCurrentScope(p,'propositions').find(record=>engine.recordId(record,'propositions')===target.id),entry=committed.manifest.entries.find(item=>item.jsonPointer==='/records/propositions/0/relationships/ACTIVATION_PROPOSITION_ID');
  if(engine.recordValue(canonicalTarget,'ACTIVATION_PROPOSITION_ID')!==activation.id||!entry||entry.origin!=='APPLICATION_RELATIONSHIP_RESOLUTION'||entry.relationshipTargetId!==activation.id)throw new Error('Accepted Stage 04 activation relationship lost application authority or exact extraction provenance.');
}
negativeAt('conditional proposition missing activation',4,e=>{addConditionalActivationFixture(e);delete e.records.propositions[0].relationships.ACTIVATION_PROPOSITION_ID;},'MISSING_ACTIVATION_PROPOSITION_BINDING');
negativeAt('conditional proposition self activation',4,e=>{addConditionalActivationFixture(e);e.records.propositions[0].relationships.ACTIVATION_PROPOSITION_ID={tempKey:'conditional-target'};},'SELF_ACTIVATION_PROPOSITION');
negativeAt('conditional proposition canonical activation id',4,e=>{addConditionalActivationFixture(e);e.records.propositions[0].relationships.ACTIVATION_PROPOSITION_ID={recordId:'PROPOSITION-FABRICATED'};},'INVALID_ACTIVATION_PROPOSITION_BINDING');
negativeAt('nonconditional proposition activation',4,e=>{addConditionalActivationFixture(e);e.records.propositions[1].relationships.ACTIVATION_PROPOSITION_ID={tempKey:'conditional-target'};},'UNEXPECTED_ACTIVATION_PROPOSITION_BINDING');

// Smart/curly JSON delimiters fail closed while the exact raw response remains preserved for audit.
{
  ...
  if(prepared.validation.valid||!prepared.validation.issues.some(issue=>issue.code==='UNSAFE_JSON_QUOTES'))throw new Error(`Unsafe smart-quoted JSON was not rejected: ${JSON.stringify(prepared.validation.issues)}`);
...
// HUMAN_INPUT_REQUIRED must contain actual blocking human-authority work.
{
 ...
 const envelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,...responseBinding(promptRecord),operation:promptRecord.operation,promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'question-optional',question:'Optional preference?',whyRequired:'This preference is not required to continue.',affectedStageFields:[],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:false}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};
 const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(envelope),promptRecord});
...
// Clarification loop: structured question -> accepted question record -> human answer -> INPUT version increments.
{
  ...
  const envelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,...responseBinding(promptRecord),operation:promptRecord.operation,promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'question-1',question:'Which jurisdiction controls the requested release?',whyRequired:'The operator must establish jurisdictional scope.',affectedStageFields:['EXACT_DELIVERABLE_REQUESTED'],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:true}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};
  const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(envelope),promptRecord});
...
for(const [name,definition,value,code] of [
  ['non-finite number',{valueType:'NUMBER',enumValues:[],nullable:false},Infinity,'UNSAFE_JSON_NUMBER'],
  ['integer required',{valueType:'INTEGER',enumValues:[],nullable:false},1.5,'UNSAFE_JSON_NUMBER'],
...
{let p=project('JOB-PR3-STALE-PROMPT-ENGINE'),stage=2,pr=savePrompt(p,stage),e=validEnvelope(p,stage,pr);p.projectData.generatedPrompts.find(x=>(x.instructionId||x.promptId)===pr.instructionId).promptEngineVersion='closed-loop-prompt-engine/obsolete';const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(e),promptRecord:pr});if(prepared.validation.valid||!prepared.validation.issues.some(i=>i.code==='STALE_PROMPT_ENGINE_VERSION'))throw new Error('Response to an obsolete prompt-engine instruction was accepted.');if(prepared.project.projectData.acceptedChanges.length)throw new Error('Obsolete prompt-engine response mutated canonical state.');negativeCount++;}
...
// Raw capture audit scope must be controlled by the persisted prompt, not a caller-supplied context hint.
{
  ...
  if(captured.rawRecord.runId!==prompt.scope.runId||captured.rawRecord.contextId!==prompt.scope.contextId||captured.rawRecord.iteration!==prompt.scope.iterationId)throw new Error('Raw-response audit identity is not bound to the exact persisted controlling prompt scope.');
}
...
{
  const p=project('JOB-UNPERSISTED-PROMPT'),stage=2,pr=fixtureBuildPrompt(stage,p),e=validEnvelope(p,stage,pr);p.projectData.generatedPrompts=p.projectData.generatedPrompts.filter(item=>String(item.instructionId||item.promptId)!==pr.instructionId);let rejected=false;
...
// demonstrated-smart-quote-rejection-and-stageData-provenance-regression-v2
{
  ...
  const rejected=ingestion.prepare(p,{stage:1,text:smart,promptRecord:pr});
  if(rejected.validation.valid||!rejected.validation.issues.some(x=>x.code==='UNSAFE_JSON_QUOTES'))throw new Error('Unsafe smart-quote delimiters did not fail closed.');
  if(rejected.rawRecord.completeRawResponse!==smart||rejected.project.projectData.acceptedChanges.length)throw new Error('Smart-quote rejection did not preserve exact raw bytes with no canonical mutation.');
  const prepared=ingestion.prepare(p,{stage:1,text:standard,promptRecord:pr});
  if(!prepared.validation.valid)throw new Error('ASCII JSON StageData provenance fixture was rejected: '+JSON.stringify(prepared.validation.issues));
