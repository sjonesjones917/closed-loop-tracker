...
function fixtureBuildPrompt(stage,p,options){
  ...
  const operation=options?.operation||fixturePromptOperation(stage),nextRevision=Number(p.revision||0)+1,preview=structuredClone(p);preview.revision=nextRevision;
  const unbound=prompts.buildPromptRecord(stage,preview,{...(options||{}),operation}),scope=unbound.scope;
  if(scope.runId&&!engine.records(p,'runs').some(record=>engine.recordId(record,'runs')===scope.runId))p.projectData.runs.push({id:scope.runId,stage,active:true,status:'RESERVED',scope:{iterationId:scope.iterationId,candidateId:scope.candidateId,runId:scope.runId},fields:{RUN_ID:scope.runId,ITERATION_ID:scope.iterationId||'NOT APPLICABLE',CANDIDATE_ID:scope.candidateId||'NOT APPLICABLE',CONTEXT_ID:scope.contextId||'NOT APPLICABLE'},RUN_ID:scope.runId,ITERATION_ID:scope.iterationId||'NOT APPLICABLE',CANDIDATE_ID:scope.candidateId||'NOT APPLICABLE',CONTEXT_ID:scope.contextId||'NOT APPLICABLE'});
  if(scope.contextId&&!engine.records(p,'freshContexts').some(record=>engine.recordId(record,'freshContexts')===scope.contextId))p.projectData.freshContexts.push({id:scope.contextId,stage,active:true,status:'RESERVED',scope:{iterationId:scope.iterationId,candidateId:scope.candidateId,runId:scope.runId,contextId:scope.contextId},fields:{CONTEXT_ID:scope.contextId,RUN_ID:scope.runId||'NOT APPLICABLE',ITERATION_ID:scope.iterationId||'NOT APPLICABLE',STATUS:'RESERVED'},CONTEXT_ID:scope.contextId,RUN_ID:scope.runId||'NOT APPLICABLE',ITERATION_ID:scope.iterationId||'NOT APPLICABLE',STATUS:'RESERVED'});
  const contract=schema.operationContract(stage,operation),scopeKey=globalThis.closedLoopHash.sha256Value(scope).slice(0,20).toUpperCase(),packageId=contract?.packageIdPolicy==='REQUIRED'?`PACKAGE-${stage}-${scopeKey}`:null,targetSlot=`STAGE-${String(stage).padStart(2,'0')}:${operation}:${scopeKey}`,reservation=engine.reserveOperation(p,{stage,operation,targetSlot,packageId,promptId:'',scope,owningTabInstance:'VERIFY-INGESTION',payload:{purpose:'BOUND_FIXTURE_PROMPT',scopeKey}}),reservationId=engine.recordId(reservation,'operationReservations');
  const boundState=structuredClone(p);boundState.revision=nextRevision;
  const record={...prompts.buildPromptRecord(stage,boundState,{...(options||{}),operation,operationReservation:reservationId}),generatedAt:new Date().toISOString(),iteration:p.job.CURRENT_ITERATION||'NOT APPLICABLE'};
  engine.registerGeneratedPrompt(p,record);engine.bindOperationReservation(p,{reservationId,promptId:record.instructionId,packageId});p.revision=nextRevision;return record;
}
...
function savePrompt(p,stage){
  ...
  return fixtureBuildPrompt(stage,p,options);
}
function responseBinding(promptRecord){return {packageId:promptRecord.operationReservation?.packageId??null,operationReservationId:promptRecord.operationReservation?.operationReservationId||'',challengeNonce:promptRecord.operationReservation?.challengeNonce||''};}
...
function validEnvelope(p,stage,promptRecord){
  ...
  const requiredAgentStageFields=agentStageFields.filter(name=>Number(schema.stageFieldDefinition(stage,name).requiredAtStage)===Number(stage));
  for(const name of requiredAgentStageFields)stageData[name]=valueForDefinition(schema.stageFieldDefinition(stage,name));
  if(!requiredAgentStageFields.length&&agentStageFields.length)stageData[agentStageFields[0]]=valueForDefinition(schema.stageFieldDefinition(stage,agentStageFields[0]));
  if(stage===1){const m=promptRecord.contextManifest.intakeCoverageManifest;stageData.EXACT_DELIVERABLE_REQUESTED='Verified deliverable';stageData.ASSUMPTIONS='NONE';stageData.UNKNOWN_INFORMATION='NONE';stageData.INPUT_SET_CONTENTS=JSON.stringify({schema:'closed-loop-stage01-capture/1',inputVersion:m.inputVersion,manifestSha256:m.manifestSha256,units:m.units.map((u,i)=>({sourceUnitId:u.unitId,sourceRawValueSha256:u.rawValueSha256,...(u.kind==='SUPPLIED_MATERIAL'?{artifactInspection:{artifactId:u.artifactId,artifactSha256:u.artifactSha256,inspectedActualBytes:true}}:{}),disposition:'EXTRACTED_RELEVANT_INFORMATION',reason:'',extractedStatements:[{statementKey:'S'+String(i+1),text:u.rawValueText||u.label,statementClass:'FACT',sourceLocations:[{kind:'OTHER',value:u.sourceLocation||u.unitId}]}]}))});}
  const records={};
  ...
  return {
    ...
    jobId:p.job.JOB_ID,
    stage,
    operation:promptRecord.operation,...responseBinding(promptRecord),promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,
...
function blockedEnvelope(p,stage,promptRecord){return {schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,operation:promptRecord.operation,...responseBinding(promptRecord),promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:{...promptRecord.scope},responseType:'BLOCKED',humanInputRequests:[],stageData:{},records:{},evidence:[],unresolved:[{temporaryKey:'u-1',kind:'MISSING_CAPABILITY',description:'Controlled blocked fixture',whyBlocking:'Scope identity validation fixture.',affectedStageFields:[],affectedRecords:[],blocking:true}],warnings:[],attachments:[]};}
...
for(let stage=1;stage<=30;stage++){
  ...
  if(!envelope){
    const prohibited={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,operation:promptRecord.operation,...responseBinding(promptRecord),promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,responseType:'DATA_PROPOSAL',humanInputRequests:[],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};
...
// Typographic quote delimiters are unsafe JSON and must fail closed without repair.
{
  ...
  if(prepared.validation.valid||!prepared.validation.issues.some(issue=>issue.code==='UNSAFE_JSON_QUOTATION'))throw new Error(`Smart-quoted JSON delimiter output was not rejected: ${JSON.stringify(prepared.validation.issues)}`);
  ...
  if(prepared.proposal||prepared.project.projectData.acceptedChanges.length)throw new Error('Smart-quoted response created a proposal or changed canonical state.');
  negativeCount++;
}
...
negativeAt('missing operation-required agent stage field',1,(e)=>{delete e.stageData.EXACT_DELIVERABLE_REQUESTED;},'MISSING_REQUIRED_STAGE_FIELD');
...
// Attachment declarations are claims; only application-hashed supplied bytes may satisfy them.
{
  ...
  for(const [name,files,mutate,code] of [
    ...
    ['unknown attachment reference property',[exactFile],e=>{e.evidence[0].attachmentRef.unexpected='forbidden';},'UNKNOWN_PROPERTY'],
    ['ambiguous duplicate attachment',[exactFile,{...exactFile,artifactId:'ARTIFACT-ATTACHMENT-2'}],()=>{},'AMBIGUOUS_ATTACHMENT_MATCH']
...
// HUMAN_INPUT_REQUIRED must contain actual blocking human-authority work.
{
 ...
 const envelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,operation:promptRecord.operation,...responseBinding(promptRecord),promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'question-optional',question:'Optional preference?',whyRequired:'This preference is not required to continue.',affectedStageFields:[],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:false}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};
 const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(envelope),promptRecord});
...
// Clarification loop: structured question -> accepted question record -> human answer -> INPUT version increments.
{
  ...
  const envelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,operation:promptRecord.operation,...responseBinding(promptRecord),promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'question-1',question:'Which jurisdiction controls the requested release?',whyRequired:'The operator must establish jurisdictional scope.',affectedStageFields:['EXACT_DELIVERABLE_REQUESTED'],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:true}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};
  const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(envelope),promptRecord});
...
{let p=project('JOB-PR3-STALE-PROMPT-ENGINE'),stage=2,pr=savePrompt(p,stage),e=validEnvelope(p,stage,pr);const stored=p.projectData.generatedPrompts.find(record=>(record.instructionId||record.promptId)===pr.instructionId);stored.promptEngineVersion='closed-loop-prompt-engine/obsolete';const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(e),promptRecord:stored});if(prepared.validation.valid||!prepared.validation.issues.some(i=>i.code==='STALE_PROMPT_ENGINE_VERSION'))throw new Error('Response to an obsolete prompt-engine instruction was accepted.');if(prepared.project.projectData.acceptedChanges.length)throw new Error('Obsolete prompt-engine response mutated canonical state.');negativeCount++;}
...
{let p=project('JOB-REFINEMENT-LOOP'),stage=2,pr=savePrompt(p,stage),e=validEnvelope(p,stage,pr),prepared=ingestion.prepare(p,{stage,text:JSON.stringify(e),promptRecord:pr});preparePromptPrerequisites(prepared.project,stage);const result=ingestion.reject(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY',reason:'Add the missing controlling-source rationale and make the result more complete.',requestCorrection:true});p=result.project;const old=p.projectData.generatedPrompts.find(x=>(x.instructionId||x.promptId)===pr.instructionId),active=p.projectData.generatedPrompts.filter(x=>Number(x.stage)===stage&&!x.invalidatedBy),reservation=p.projectData.operationReservations.find(x=>engine.recordId(x,'operationReservations')===e.operationReservationId);if(!old?.invalidatedBy||active.length||engine.recordValue(reservation,'STATUS')!=='SUPERSEDED'||!result.newPromptRequired)throw new Error('Correction request did not invalidate the old prompt and supersede its reservation before replacement.');}
// EXECUTION_FAILED is fail-closed and a successful exact-scope replacement resolves it.
{let p=project('JOB-EXECUTION-FAIL-CLOSED'),stage=1,pr=savePrompt(p,stage);const fail={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,operation:pr.operation,...responseBinding(pr),promptIdentity:{instructionId:pr.instructionId,bodySha256:pr.bodySha256,contractSha256:pr.contractSha256,contextSignature:pr.contextSignature},scope:pr.scope,responseType:'EXECUTION_FAILED',humanInputRequests:[],stageData:{},records:{},evidence:[],unresolved:[{temporaryKey:'failure-1',kind:'TOOL_FAILURE',description:'Required tool failed.',whyBlocking:'The operation could not be executed.',affectedStageFields:[],affectedRecords:[],blocking:true}],warnings:[],attachments:[]};let prepared=ingestion.prepare(p,{stage,text:JSON.stringify(fail),promptRecord:pr});p=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY'}).project;if(p.stages[stage].status!=='BLOCKED'||!engine.gate(stage,p).reasons.some(x=>x.includes('execution failure')))throw new Error('Accepted execution failure did not fail closed.');pr=savePrompt(p,stage);const replacement=validEnvelope(p,stage,pr);prepared=ingestion.prepare(p,{stage,text:JSON.stringify(replacement),promptRecord:pr});p=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY'}).project;if(p.projectData.executionFailures.some(x=>Number(x.stage)===stage&&!x.resolvedBy&&!x.invalidatedBy))throw new Error('Successful replacement did not resolve execution failure.');}

{let p=project('JOB-PARALLEL-PROMPT-VALIDATION'),stage=17;p.revision=0;const a=fixtureBuildPrompt(stage,p,{operation:'EXECUTE_RUN',scope:{runId:'RUN-A',contextId:'CTX-A'}}),b=fixtureBuildPrompt(stage,p,{operation:'EXECUTE_RUN',scope:{runId:'RUN-B',contextId:'CTX-B'}});const q={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,operation:a.operation,...responseBinding(a),promptIdentity:{instructionId:a.instructionId,bodySha256:a.bodySha256,contractSha256:a.contractSha256,contextSignature:a.contextSignature},scope:a.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'parallel-q',question:'Provide the missing run-specific value.',whyRequired:'The selected run cannot continue without it.',affectedStageFields:[],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:true}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(q),promptRecord:a});if(!prepared.validation.valid)throw new Error('Unrelated newer run prompt incorrectly staled the controlling run prompt: '+JSON.stringify(prepared.validation.issues));if(!b.operationReservationBound)throw new Error('The second lane prompt was not reservation-bound.');}
{let p=project('JOB-SCOPED-CLARIFICATION'),stage=17;p.revision=0;const pr=fixtureBuildPrompt(stage,p,{operation:'EXECUTE_RUN',scope:{runId:'RUN-CLARIFY',contextId:'CTX-CLARIFY'}}),q={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,operation:pr.operation,...responseBinding(pr),promptIdentity:{instructionId:pr.instructionId,bodySha256:pr.bodySha256,contractSha256:pr.contractSha256,contextSignature:pr.contextSignature},scope:pr.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'scoped-q',question:'Provide the run-specific missing input.',whyRequired:'This exact run is missing required human authority.',affectedStageFields:[],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:true}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};let prepared=ingestion.prepare(p,{stage,text:JSON.stringify(q),promptRecord:pr});if(!prepared.validation.valid)throw new Error('Scoped clarification response invalid: '+JSON.stringify(prepared.validation.issues));p=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY'}).project;const request=p.projectData.humanInputRequests.at(-1);preparePromptPrerequisites(p,stage);const answered=ingestion.answerHumanInput(p,{[request.requestId]:'Exact run-specific answer'},{operator:'VERIFY'});p=answered.project;const target=answered.replacementPromptTargets[0];if(!target||target.operation!=='EXECUTE_RUN'||target.scope?.runId!=='RUN-CLARIFY'||target.scope?.contextId!=='CTX-CLARIFY')throw new Error('Scoped clarification lost the exact operation/run replacement target.');if(p.projectData.generatedPrompts.some(x=>Number(x.stage)===stage&&!x.invalidatedBy))throw new Error('Scoped clarification created an unbound prompt before reservation.');const replacement=fixtureBuildPrompt(stage,p,{operation:target.operation,scope:target.scope});if(!replacement.operationReservationBound)throw new Error('Scoped clarification replacement was not reservation-bound.');}
...
{
  const p=project('JOB-BLOCKED-SEMANTICS'),stage=1,pr=fixtureBuildPrompt(stage,p);
...
{
  const p=project('JOB-UNPERSISTED-PROMPT'),stage=2;preparePromptPrerequisites(p,stage);const preview=structuredClone(p);preview.revision=Number(p.revision||0)+1;const pr=prompts.buildPromptRecord(stage,preview,{operation:fixturePromptOperation(stage)}),e=validEnvelope(p,stage,pr);let rejected=false;
...
// Required Stage 01 data remains provenance-bound on strict ASCII JSON acceptance.
{
  let p=project('JOB-STAGE-DATA-PROVENANCE'),pr=savePrompt(p,1),e=validEnvelope(p,1,pr);
  ...
  const standard=JSON.stringify(e),prepared=ingestion.prepare(p,{stage:1,text:standard,promptRecord:pr});
  if(!prepared.validation.valid)throw new Error('Strict Stage 01 JSON was rejected: '+JSON.stringify(prepared.validation.issues));
  const committed=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'STAGE_DATA_PROVENANCE_REGRESSION'});const stageEntries=committed.manifest.entries.filter(x=>x.canonicalCollection==='stageData');
...
// Stage 05 applicability reviews keep reviewer-lane identity as provenance while
// the canonical semantic record remains current under the requirements scope.
function stage5ApplicabilityFixture(jobId,{reuseCompilerContext=false}={}){
  const p=project(jobId);preparePromptPrerequisites(p,5);p.job.CURRENT_REQUIREMENTS_VERSION='REQUIREMENTS-v001';engine.recalculate(p);
  const canonicalScope=engine.currentScope(p),requirementId='REQ-APPLICABILITY-001',propositionId='PROPOSITION-APPLICABILITY-001';
  const append=(collection,id,stage,fields)=>{const definition=schema.RECORD_SCHEMAS[collection],record={id,stage,active:true,scope:{...canonicalScope},fields:{...fields},...fields,source:'APPLICATION_DERIVED'};record.fields[definition.idField]=id;record[definition.idField]=id;engine.refreshRecordHashes(record,collection);p.projectData[collection].push(record);return record;};
  append('requirements',requirementId,4,{REQ_ID:requirementId,OBLIGATION:'The current product must satisfy the applicability fixture.',MANDATORY_OPTIONAL_STATUS:'MANDATORY',STATUS:'ACTIVE'});
  append('propositions',propositionId,4,{PROPOSITION_ID:propositionId,REQUIREMENT_ID:requirementId,PROPOSITION_TEXT:'The applicability fixture applies to the current product.',SUBJECT_AND_SCOPE_DESCRIPTION:'Current controlled product',SATISFACTION_MEANING:'The fixture applies.',FAILURE_MEANING:'The fixture does not apply.',CURRENT_SCOPE:canonicalScope,CONTENT_SHA256:'',STATUS:'CURRENT'});
  const compiler=engine.registerFreshContext(p,{stage:5,externalContextIdentifier:`${jobId}-COMPILER`,operatorLabel:'VERIFY',purpose:'GENERAL'}),reviewer=reuseCompilerContext?compiler:engine.registerFreshContext(p,{stage:5,externalContextIdentifier:`${jobId}-REVIEWER`,operatorLabel:'VERIFY',purpose:'REVIEWER'}),compilerContextId=engine.recordId(compiler,'freshContexts'),reviewerContextId=engine.recordId(reviewer,'freshContexts');
  const seedAcceptedCompiler=(stage,contextId,eventSequence)=>{const scope={...engine.currentScope(p),contextId},rawResponseId=`RAW-${jobId}-${stage}-COMPILER`,proposalId=`PROPOSAL-${jobId}-${stage}-COMPILER`;p.projectData.acceptedChanges.push({changeId:`CHANGE-${jobId}-${stage}-COMPILER`,rawResponseId,proposalId,validationId:`VALIDATION-${jobId}-${stage}-COMPILER`,jobId:p.job.JOB_ID,stage,responseType:'DATA_PROPOSAL',status:'COMMITTED',operation:'COMPLETE',scope,eventSequence,canonicalRecordIds:[],stageFields:[]});p.projectData.responseProposals.push({proposalId,rawResponseId,stage,responseType:'DATA_PROPOSAL',status:'ACCEPTED',envelope:{operation:'COMPLETE',scope},scope,proposedStageData:{},canonicalRecords:{},evidence:[]});};
  seedAcceptedCompiler(4,`CONTEXT-${jobId}-STAGE4-COMPILER`,1);seedAcceptedCompiler(5,compilerContextId,2);
  const pr=fixtureBuildPrompt(5,p,{operation:'APPLICABILITY_REVIEW',scope:{contextId:reviewerContextId}}),envelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage:5,operation:'APPLICABILITY_REVIEW',...responseBinding(pr),promptIdentity:{instructionId:pr.instructionId,bodySha256:pr.bodySha256,contractSha256:pr.contractSha256,contextSignature:pr.contextSignature},scope:pr.scope,responseType:'DATA_PROPOSAL',humanInputRequests:[],stageData:{},records:{applicabilityRecords:[{tempKey:'applicability-review',fields:{PROPOSED_APPLICABILITY:'APPLICABLE',REASONING:'The current mandatory proposition applies to the current controlled product.'},relationships:{SUBJECT_ID:{recordId:propositionId}},evidenceRefs:['applicability-evidence']}]},evidence:[{temporaryKey:'applicability-evidence',kind:'APPLICABILITY_REVIEW',description:'Current independent applicability review.',location:'Stage 05 reviewer context',content:'The proposition applies to the current controlled product.'}],unresolved:[],warnings:[],attachments:[]};
  return{p,pr,envelope,reviewerContextId,compilerContextId,propositionId};
}
{
  const fixture=stage5ApplicabilityFixture('JOB-APPLICABILITY-SCOPE-CURRENT'),prepared=ingestion.prepare(fixture.p,{stage:5,text:JSON.stringify(fixture.envelope),promptRecord:fixture.pr});
  if(!prepared.validation.valid)throw new Error('A correctly bound fresh-context applicability review was rejected: '+JSON.stringify(prepared.validation.issues));
  const committed=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'APPLICABILITY_SCOPE_REGRESSION'}),rows=engine.recordsForCurrentScope(committed.project,'applicabilityRecords'),record=rows.find(item=>String(engine.recordValue(item,'SUBJECT_ID'))===fixture.propositionId);
  if(!record||engine.recordValue(record,'SELECTED_APPLICABILITY')!=='APPLICABLE')throw new Error('The accepted applicability review did not remain current and application-selected after ingestion.');
  if(Object.prototype.hasOwnProperty.call(record.scope||{},'contextId'))throw new Error('Reviewer context identity leaked into canonical applicability current-scope dimensions.');
  if(record.reviewerContextId!==fixture.reviewerContextId||record.provenanceScope?.contextId!==fixture.reviewerContextId)throw new Error('The accepted applicability review lost its exact reviewer-context provenance.');
}
{
  const fixture=stage5ApplicabilityFixture('JOB-APPLICABILITY-SCOPE-WRONG'),envelope=structuredClone(fixture.envelope);envelope.scope.contextId='CONTEXT-WRONG';const prepared=ingestion.prepare(fixture.p,{stage:5,text:JSON.stringify(envelope),promptRecord:fixture.pr});
  if(prepared.validation.valid||!prepared.validation.issues.some(item=>item.code==='STALE_SCOPE'))throw new Error('An applicability review with the wrong bound reviewer context was accepted.');negativeCount++;
}
{
  const fixture=stage5ApplicabilityFixture('JOB-APPLICABILITY-SCOPE-REUSED',{reuseCompilerContext:true}),prepared=ingestion.prepare(fixture.p,{stage:5,text:JSON.stringify(fixture.envelope),promptRecord:fixture.pr});
  if(prepared.validation.valid||!prepared.validation.issues.some(item=>item.code==='REUSED_OR_INVALID_REVIEW_CONTEXT'))throw new Error('An applicability review reusing its compiler context was accepted.');
  if(prepared.project.projectData.applicabilityRecords.length)throw new Error('A reused-context applicability review partially mutated canonical semantic state.');negativeCount++;
}
