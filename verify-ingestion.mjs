...
// HUMAN_INPUT_REQUIRED must contain actual blocking human-authority work.
{
 ...
 const envelope={schema:schema.RESPONSE_SCHEMA,contractProfileId:schema.CONTRACT_PROFILE_ID,jobId:p.job.JOB_ID,stage,operation:promptRecord.operation,promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'question-optional',question:'Optional preference?',whyRequired:'This preference is not required to continue.',affectedStageFields:[],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:false}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};
 const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(envelope),promptRecord});
...
// Clarification loop: structured question -> accepted question record -> human answer -> INPUT version increments.
{
  ...
  const envelope={schema:schema.RESPONSE_SCHEMA,contractProfileId:schema.CONTRACT_PROFILE_ID,jobId:p.job.JOB_ID,stage,operation:promptRecord.operation,promptIdentity:{instructionId:promptRecord.instructionId,bodySha256:promptRecord.bodySha256,contractSha256:promptRecord.contractSha256,contextSignature:promptRecord.contextSignature},scope:promptRecord.scope,responseType:'HUMAN_INPUT_REQUIRED',humanInputRequests:[{temporaryKey:'question-1',question:'Which jurisdiction controls the requested release?',whyRequired:'The operator must establish jurisdictional scope.',affectedStageFields:['EXACT_DELIVERABLE_REQUESTED'],affectedRecords:[],answerType:'TEXT',allowedValues:[],blocking:true}],stageData:{},records:{},evidence:[],unresolved:[],warnings:[],attachments:[]};
  const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(envelope),promptRecord});
...
// demonstrated-smart-quote-rejection-and-stageData-provenance-regression-v2
{
  ...
  if(rejected.validation.valid||!rejected.validation.issues.some(x=>x.code==='UNSAFE_SMART_QUOTES'))throw new Error('Smart-quote authoritative JSON was not rejected fail-closed.');
  if(rejected.rawRecord.completeRawResponse!==smart)throw new Error('Smart-quote rejection changed the preserved raw response.');
  if(rejected.project.projectData.acceptedChanges.length)throw new Error('Smart-quote rejection mutated canonical state.');
  const prepared=ingestion.prepare(p,{stage:1,text:standard,promptRecord:pr});
  if(!prepared.validation.valid)throw new Error('Valid ASCII JSON failed provenance preparation: '+JSON.stringify(prepared.validation.issues));
  const committed=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'STAGEDATA_PROVENANCE_REGRESSION'});const stageEntries=committed.manifest.entries.filter(x=>x.canonicalCollection==='stageData');
