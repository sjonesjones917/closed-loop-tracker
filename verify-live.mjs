...
for(const token of ['mobile-closed-loop/30','closed-loop-project/3','closed-loop-stage-response/3','closed-loop-test-spec/1','closed-loop-verification-package/1','PENDING_OPERATOR_REVIEW'])if(!active.includes(token))throw new Error(`Required deployed contract identity/control is missing: ${token}`);
if(!active.includes('closedLoopTestRuntime')||!active.includes('test-worker.js'))throw new Error('Application-owned deterministic Test IR runtime is missing from deployed source.');
console.log(JSON.stringify({liveSourceIdentity:true,filesCompared:deployed.length,retainedJobId:project.jobId,currentStage:2,stage1:'COMPLETE',workflow:'mobile-closed-loop/30',projectSchema:'closed-loop-project/3',responseSchema:'closed-loop-stage-response/3',testIrSchema:'closed-loop-test-spec/1',packageSchema:'closed-loop-verification-package/1',oneApplication:true},null,2));
