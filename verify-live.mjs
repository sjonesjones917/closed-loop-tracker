...
const fetched={};
for(const file of ['index.html','app.js','workbook.js','TEST_PROJECT.json','AUTHORIZED_OPERATION_01.txt']){
...
for(const token of ['stageRecordText','stageOutputText','stageFieldsMarkup','stageRecordFromFields','appendixFieldsMarkup','saveStageWork','savePromptRecord','recordOutputRecord','retainedSpecRevision'])if(!fetched['app.js'].includes(token))throw new Error(`Live application repair missing ${token}.`);
...
for(const token of ['Complete project record','Generated instructions','Generated outputs','Original project input','Add supporting record','PRESERVE THE COMPLETE EVIDENCE CHAIN','PRESERVE FAILURES PERMANENTLY'])if(!source.includes(token))throw new Error(`Live human-facing project control missing: ${token}`);
console.log(JSON.stringify({live:true,stages:30,title:project.title,jobId:project.jobId,currentStage:project.currentStage,state:project.currentState,prompts:project.generatedPrompts.length,outputs:project.generatedOutputs.length,receipts:project.outputReceipts.length,structuredStageRenderer:true,contextualAppendices:true},null,2));
