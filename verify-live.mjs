...
const deployed=['index.html','app-core.js','hash.js','workflow-schema.js','workflow-engine.js','prompt-engine.js','response-ingestion.js','runtime-fixes.js','project-store.js','workbook.js','TEST_PROJECT.json','AUTHORIZED_OPERATION_01.txt'];
...
if(!active.includes('closed-loop-runtime-fixes/1')||!active.includes('MALFORMED_JSON_SMART_QUOTES')||!active.includes('STRICT MACHINE-RESPONSE SERIALIZATION'))throw new Error('JSON response repair runtime is missing from deployed source.');
console.log(JSON.stringify({liveSourceIdentity:true,filesCompared:deployed.length,retainedJobId:project.jobId,currentStage:2,stage1:'COMPLETE',responseSchema:'closed-loop-stage-response/2',runtimeFix:'closed-loop-runtime-fixes/1',oneApplication:true},null,2));
