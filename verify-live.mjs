...
if(mode==='workflow'){
  ...
  const result=await evalJS(`(()=>({count:document.querySelectorAll('.workflow-row').length,text:document.body.innerText,scroll:document.documentElement.scrollWidth-innerWidth,buttons:[...document.querySelectorAll('.header-actions button,.project-tabs button')].map(b=>({t:b.innerText.trim(),h:b.getBoundingClientRect().height,w:b.getBoundingClientRect().width})),schema:JSON.parse(localStorage.getItem('mobile-closed-loop-agent')).projectData.schema}))()`);
  ...
  for(const b of result.buttons.filter(x=>x.t)){must(b.h>=28&&b.h<=48,`compact control height outside usable range: ${b.t} ${b.h}`)}
  ...
  await evalJS(`document.querySelector('[data-view="history"]')?.click()`);await sleep(100);const t=String(await evalJS(`document.body.innerText`)).toLowerCase();must(t.includes('complete project history'),'history view missing');must(t.includes('stage states'),'stage history missing');must(t.includes('output receipts'),'output receipts missing');must(t.includes('preserved artifacts'),'artifact history missing');console.log('live history verified');
}else if(mode==='reset'){
