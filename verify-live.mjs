...
for(let i=0;i<100;i++){await sleep(100);const ready=await evalJS(`!![...document.querySelectorAll('#project-picker option')].find(o=>o.textContent.includes('GEN-042 maintenance handoff'))`);if(ready)break;if(i===99)throw new Error('Application did not load the retained GEN-042 project')}
await evalJS(`(()=>{const s=document.getElementById('project-picker');const o=[...s.options].find(x=>x.textContent.includes('GEN-042 maintenance handoff'));if(!o)return false;s.value=o.value;s.dispatchEvent(new Event('change',{bubbles:true}));return true})()`);await sleep(150);
