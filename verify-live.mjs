...
const fetched={};
for(const file of ['index.html','app.js','app-core.js','prompt-engine.js','workbook.js','experience.js','TEST_PROJECT.json','AUTHORIZED_OPERATION_01.txt']){
...
for(const token of ['COPY BLOCK — STAGE 01 — INITIALIZE THE JOB','JOB_ID: JOB-20260823144121','AUTHORIZED INPUTS','AUTHORIZED STAGE RECORD','REQUIRED OUTPUT','UNIVERSAL OPERATING RULES'])if(!historicalPrompt.includes(token))throw new Error(`Live historical Stage 01 instruction is missing ${token}.`);
...
const loader=fetched['app.js'],app=fetched['app-core.js'],prompts=fetched['prompt-engine.js'];
for(const token of ['prompt-engine.js','app-core.js'])if(!loader.includes(token))throw new Error(`Live application loader is missing ${token}.`);
if(!fetched['index.html'].includes('closed-loop-runtime-20260823-2037-r2'))throw new Error('Live repaired cache identity is missing.');
if(!fetched['index.html'].includes('experience.js?v=closed-loop-runtime-20260823-2103-r3'))throw new Error('Live cache-busted human-facing experience asset is missing.');
...
for(const token of ['stageRecordText','stageOutputText','stageFieldsMarkup','stageRecordFromFields','appendixFieldsMarkup','saveStageWork','savePromptRecord','recordOutputRecord','retainedSpecRevision','recordSchemas','structuredRecords','addStructuredRecord','data-add-record','createUniqueJobId','invalidateDownstream','blockingRecord=openBlockers().find'])if(!app.includes(token))throw new Error(`Live application core repair missing ${token}.`);
for(const token of ['core.buildStagePrompt=build','const procedures={','const inputCollections={','CONTROLLING PROJECT INPUT','AUTHORIZED INPUTS FOR THIS STAGE','STAGE-SPECIFIC TASK','PROJECT-SCOPE BOUNDARY','Do not instruct the operator to reuse this prompt for other jobs','genuinely independent external authorities','Research only the legitimate Stage 02 external governing source set','2026-08-24-r2'])if(!prompts.includes(token))throw new Error(`Live prompt engine missing ${token}.`);
for(let n=1;n<=30;n++)if(!prompts.includes(`${n}:'`))throw new Error(`Live prompt engine has no stage-specific procedure for Stage ${n}.`);
for(const token of ['Mobile closed-loop control','Current work','Completed work','Continue current stage','View complete record','Project identity','Authorized job input','Workflow control','Work for this stage','Instruction to run','Returned output','Stage decision and evidence','Completion controls','Supporting records','Find project information','deleteCurrentProject','enforceConsistentPageWidth'])if(!fetched['experience.js'].includes(token))throw new Error(`Live human-facing experience missing ${token}.`);
if(app.includes('id="stage-record"')||app.includes("draftRecord:r.record||r.evidenceRecord"))throw new Error('Live raw stage-record defect remains.');
if(/async\s+async\s+function/.test(app)||/async\s+function\s+createUniqueJobId/.test(app))throw new Error('Live new-job implementation contains an invalid async declaration.');
if(!app.includes("while(projects.some(p=>p.job?.JOB_ID===id))"))throw new Error('Live new-job collision protection is missing.');
if(!app.includes("previousState=current.job.CURRENT_STATE,previousStage=current.job.CURRENT_STAGE"))throw new Error('Live project-save state preservation is missing.');
for(const collection of ['sources','sourceConflicts','research','candidateRequirements','requirements','tests','failureTests','preflightRecords','candidateFreezes','runs','verification','comparisons','defects','rootCauses','regressions','changes','baselines','products','deterministicResults','meaningResults','adversarialResults','representationInspections','processAudits','productAudits','releaseRecords','artifactIdentities','evidenceChains'])if(!app.includes(`${collection}:{title:`))throw new Error(`Live structured record definition missing: ${collection}`);
const source=fetched['index.html']+loader+app+prompts+fetched['workbook.js']+fetched['experience.js'];
...
console.log(JSON.stringify({live:true,stages:30,title:project.title,jobId:project.jobId,currentStage:project.currentStage,state:project.currentState,historicalPrompts:project.generatedPrompts.length,promptEngineStages:30,promptEngineRevision:'2026-08-24-r2',projectScopedPrompts:true,outputs:project.generatedOutputs.length,receipts:project.outputReceipts.length,structuredStageRenderer:true,structuredRepeatingRecords:true,contextualAppendices:true,blockerGate:true,uniqueNewJobs:true,humanFacingExperience:true,storageResetRemoved:true},null,2));