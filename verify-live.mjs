...
const deployed=['index.html','app-core.js','hash.js','workflow-schema.js','test-runtime.js','workflow-engine.js','prompt-engine.js','response-ingestion.js','project-store.js','workbook.js','test-runtime.js','test-worker.js','TEST_PROJECT.json','AUTHORIZED_OPERATION_01.txt'];
...
if(!active.includes('closed-loop-stage-response/3')||!active.includes('PENDING_OPERATOR_REVIEW')||!active.includes('ACCEPTED_DATA_CHANGE'))throw new Error('Structured response ingestion controls are missing from deployed source.');
console.log(JSON.stringify({liveSourceIdentity:true,filesCompared:deployed.length,retainedJobId:project.jobId,currentStage:2,stage1:'COMPLETE',responseSchema:'closed-loop-stage-response/3',oneApplication:true},null,2));
