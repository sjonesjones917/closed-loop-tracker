...
try{
  ...
  const result=await evalJs(`(async()=>{
    ...
    for(let stage=1;stage<=30;stage++)for(const operation of schema.STAGE_CONTRACTS[stage].operations){
      ...
      if(!text.includes('RESPONSE CONTRACT DEFINITIONS')||!text.includes('closed-loop-stage-response/3'))throw new Error('Stage '+stage+' '+operation+' omitted its response contract.');
