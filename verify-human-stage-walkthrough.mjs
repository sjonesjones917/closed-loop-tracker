...
try{
  ...
  const result=await evalJs(`(async()=>{
    ...
    const seedAcceptedOperation=(stage,operation,eventSequence,stageData={})=>{const scope=prompts.scopeFor(stage,state),proposalId='PROPOSAL-HUMAN-WALK-'+stage+'-'+operation,rawResponseId='RAW-HUMAN-WALK-'+stage+'-'+operation;state.projectData.rawResponses.push({rawResponseId,stage,sha256:'RAW-HASH-'+stage+'-'+operation,status:'ACCEPTED_DATA_CHANGE'});state.projectData.responseProposals.push({proposalId,rawResponseId,stage,status:'ACCEPTED',scope,proposedStageData:stageData,canonicalRecords:{},evidence:[],canonicalEnvelopeSha256:'ENVELOPE-HASH-'+stage+'-'+operation});state.projectData.acceptedChanges.push({changeId:'CHANGE-HUMAN-WALK-'+stage+'-'+operation,proposalId,rawResponseId,stage,operation,eventSequence,status:'COMMITTED',responseType:'DATA_PROPOSAL',scope,promptId:'PROMPT-HUMAN-WALK-'+stage+'-'+operation,canonicalEnvelopeSha256:'ENVELOPE-HASH-'+stage+'-'+operation});};
    ...
    if(!compact.includes('.expandable-prompt { height: 280px; max-height: 280px;'))throw new Error('Prompt preview height changed from the restored baseline.');
    if(!compact.includes('.expandable-prompt.expanded { height: auto; max-height: none;'))throw new Error('Expanded prompt preview remains constrained by the collapsed baseline.');
    if(compact.includes('max-height: 88px'))throw new Error('Obsolete 88px prompt height returned.');
