...
try{
  ...
  const result=await evalJs(`(async()=>{
    ...
    const capture={schema:'closed-loop-stage01-capture/1',inputVersion:intake.inputVersion,manifestSha256:intake.manifestSha256,units:intake.units.map((unit,index)=>({sourceUnitId:unit.unitId,sourceRawValueSha256:unit.rawValueSha256,disposition:'retained as context',reason:'Browser prompt walkthrough preserves current human authority.',extractedStatements:[{statementKey:'walk-'+(index+1),text:unit.rawValueText||unit.label||unit.unitId,statementClass:'CONTEXT'}]}))};
    state.stages[1].agentData={EXACT_DELIVERABLE_REQUESTED:'Sequential subject-neutral walkthrough deliverable.',ASSUMPTIONS:'NONE',UNKNOWN_INFORMATION:'NONE',INPUT_SET_CONTENTS:JSON.stringify(capture)};
    ...
    for(let completed=1;completed<30;completed++){state.stages[completed].status='COMPLETE';state.stages[completed].gate={complete:true,blocked:false,reasons:[]};}
    if(!engine.evaluateIntakeAccounting(state).complete)throw new Error('Browser walkthrough failed to establish Stage 01 intake accounting.');
