import {clickVisibleControl,fillVisibleControl} from './browser-interactions.mjs';
...
export async function createOperatorBrowser({url=process.env.PAGE_URL||'http://127.0.0.1:4173/',directory,width=393,height=852}={}){
  ...
  const target=await json(`/json/new?${encodeURIComponent(url)}`,{method:'PUT'}),page=new Connection(target.webSocketDebuggerUrl);await page.ready;await page.send('Runtime.enable');await page.send('Page.enable');await page.send('Log.enable');
  ...
  async function idle(){await until(()=>evaluate(`document.readyState==='complete'&&globalThis.closedLoopAppReady===true&&document.querySelector('#app')?.getAttribute('aria-busy')!=='true'`),'The application did not finish its visible action');}
  async function exists(selector){return evaluate(`Boolean(document.querySelector(${JSON.stringify(selector)}))`);}
  async function click(selector){await idle();await clickVisibleControl(page,selector);await idle();events.push({operation:'click',selector,transport:'CHROMIUM_POINTER'});}
  async function fill(selector,value){await idle();await fillVisibleControl(page,selector,value);await idle();events.push({operation:'fill',selector,transport:'VISIBLE_FIELD_VALUE_EVENTS'});}
  ...
  async function reload(){await page.send('Page.reload',{ignoreCache:true});await delay(100);await idle();events.push({operation:'reload'});}
  async function waitForSavedVersion(versionId){await until(()=>evaluate(`document.querySelector('#project-history-controls')?.dataset.savedVersion===${JSON.stringify(versionId)}`),'The selected project version was not restored');await idle();}
  async function close(){try{page.close();root.close();}finally{child.kill('SIGKILL');}}
  await idle();return {click,fill,selectFiles,download,project,inspect,reload,waitForSavedVersion,exists,evaluate,events,directory,close,exceptions:()=>page.events.filter(event=>event.method==='Runtime.exceptionThrown'||event.method==='Page.javascriptDialogOpening'||event.method==='Log.entryAdded'&&event.params.entry.level==='error')};
}
