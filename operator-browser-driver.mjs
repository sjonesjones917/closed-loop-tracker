...
export async function createOperatorBrowser({url=process.env.PAGE_URL||'http://127.0.0.1:4173/',directory,width=393,height=852}={}){
  ...
  async function click(selector){const started=performance.now();await idle();const clicked=await evaluate(`(()=>{const node=document.querySelector(${JSON.stringify(selector)});if(!node||node.disabled)return false;for(const box of [...(function*(n){for(let p=n.parentElement;p;p=p.parentElement)if(p.tagName==='DETAILS'&&!p.open)yield p;})(node)].reverse())box.querySelector(':scope > summary')?.click();const rect=node.getBoundingClientRect();if(rect.top<0||rect.left<0||rect.bottom>innerHeight||rect.right>innerWidth)node.scrollIntoView({block:'nearest',inline:'nearest'});node.click();return true;})()`);assert.equal(clicked,true,`Missing or disabled control: ${selector}`);await idle();events.push({operation:'click',selector,elapsedMs:performance.now()-started});}
