...
function migrateState(p){
  ...
  if(!['closed-loop-project/2','human-project/30'].includes(p.schema))throw new Error(`Unsupported project schema: ${p.schema||'MISSING'}`);
  const migrated=JSON.parse(JSON.stringify(p));
