...
assert(appCore.includes("if([2,3,4].includes(n))return `<div class=\"panel\"><h2 class=\"section-title\">Original intent file prohibited"),'Stages 02 through 04 must not render generic original-intent upload controls after canonical capture.');

console.log(JSON.stringify({sha256Vectors:true,canonicalOrdering:true,ambiguousValuesRejected:17,runtimeBuildIdentity,directRuntimeScripts:directRuntimeFiles.length,workerBoundToSameIdentity:true,stage04RepeatAttachmentControlAbsent:true}));