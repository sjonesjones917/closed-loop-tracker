...
assert(/function artifactControlMarkup\(n,locked\)\{\s*if\(n===19\)/.test(appCore),'Stage 04 artifact controls must retain the established visual rendering; repeat-input prevention belongs to canonical data flow, not UI suppression.');
assert(!/function artifactControlMarkup\(n,locked\)\{\s*if\(n===4\)return ['"]{2};/.test(appCore),'Stage 04 visual controls must not be hidden as a substitute for canonical intent reuse.');
