...
const rejectCall=(name,fn)=>{let ok=false;try{fn();}catch(e){ok=e instanceof TypeError;}assert(ok,`${name} must be rejected.`);};
...
assert(h.stableStringify({'2':'two','10':'ten'})==='{"10":"ten","2":"two"}','integer-like keys were reordered by host JSON serialization');
const protoValue={};Object.defineProperty(protoValue,'__proto__',{value:'owned',enumerable:true});assert(h.stableStringify(protoValue)==='{"__proto__":"owned"}','an own __proto__ member was lost during canonicalization');assert(h.sha256Value(protoValue)!==h.sha256Value({}),'an own __proto__ member collided with an empty object');
...
assert(testRuntime.includes("url.searchParams.set('v',RUNTIME_BUILD_ID)"),'Test IR worker URL must bind the exact shared build/cache identity.');
assert(testRuntime.includes("new URL(candidate||'test-worker.js',base)"),'Test IR worker must remain the same-origin registered worker entry.');
assert(testRuntime.includes("cache:'no-store'")&&testRuntime.includes("redirect:'error'"),'Test IR worker bytes must be retrieved with cache-safe, fail-closed semantics before execution.');
...
console.log(JSON.stringify({sha256Vectors:true,canonicalOrdering:true,unicodeScalarOrdering:true,safeIntegerBoundaries:true,ambiguousValuesRejected:29,sharedBuildIdentity,runtimeScriptCount:runtimeFiles.length,workerSharesBuildIdentity:true,stage04RepeatAttachmentControlAbsent:true}));
