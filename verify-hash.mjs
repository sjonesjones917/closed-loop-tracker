...
assert(/function\s+artifactControlMarkup\s*\(n,locked\)\s*\{\s*if\s*\(n===19\)/.test(appCore),'Stage 04 artifact controls must retain the established visual rendering; repeat-input prevention belongs to canonical data flow, not UI suppression.');
assert(!/function\s+artifactControlMarkup\s*\(n,locked\)\s*\{\s*if\s*\(n===4\)\s*return\s+['"]{2}/.test(appCore),'Stage 04 visual controls must not be hidden as a substitute for canonical intent reuse.');

console.log(JSON.stringify({sha256Vectors:true,canonicalOrdering:true,ambiguousValuesRejected:17,sharedBuildIdentity,runtimeScriptCount:runtimeFiles.length,workerSharesBuildIdentity:true,stage04VisualSuppressionAbsent:true}));