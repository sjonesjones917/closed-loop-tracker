...
// Artifact identity is independent of file-selection order.
{
  ...
  let rejected=false;try{engine.verifyArtifactIdentity(p,audited.map((x)=>({...x,bytesVerified:true,actualSize:x.size,actualSha256:x.sha256})),delivery.map((x)=>({...x,bytesVerified:true,actualSize:x.size,actualSha256:x.sha256})));}catch(error){rejected=/current bound Stage 27/.test(String(error.message));}assert(rejected,'Fabricated ACCEPTED release bypassed current Stage 27 binding.');
}
...
// Release identity is prohibited before ACCEPTED and exact mismatches remain unauthorized.
{
  ...
  let staleRejected=false;try{engine.verifyArtifactIdentity(p,[{artifactId:'A',name:'x.bin',size:3,sha256:'aaa',bytesVerified:true,actualSize:3,actualSha256:'aaa'}],[{artifactId:'A',name:'x.bin',size:4,sha256:'bbb',bytesVerified:true,actualSize:4,actualSha256:'bbb'}]);}catch(error){staleRejected=/current bound Stage 27/.test(String(error.message));}assert(staleRejected&&p.release.authorization==='NOT AUTHORIZED','Stale fabricated release was accepted.');
}
...
{
 const p=project('JOB-IDENTITY-RECOVERY');p.projectData.releaseRecords.push(record('releaseRecords',27,{DETERMINATION:'ACCEPTED'},'RELEASE-IDENTITY-RECOVERY'));deliveryCandidate(p,['A'],['a.bin']);const audited=[{artifactId:'A',name:'a.bin',size:3,sha256:'aaa',bytesVerified:true,actualSize:3,actualSha256:'aaa'}],bad=[{artifactId:'A',name:'a.bin',size:4,sha256:'bbb',bytesVerified:true,actualSize:4,actualSha256:'bbb'}],good=[{artifactId:'A',name:'a.bin',size:3,sha256:'aaa',bytesVerified:true,actualSize:3,actualSha256:'aaa'}];let rejected=false;try{engine.verifyArtifactIdentity(p,audited,bad);}catch(error){rejected=/current bound Stage 27/.test(String(error.message));}assert(rejected,'A stale Stage 27 release was accepted during identity recovery.');
}
