...
// Exact unchanged-confirmed candidate artifact identity controls Stage 20 baseline bytes.
{
  ...
  let rejected=false;try{engine.freezeBaseline(p,{artifactIds:['ARTIFACT-DIFFERENT'],operatorLabel:'VERIFY'});}catch(error){rejected=/exact artifact set|successful unchanged confirmation/i.test(String(error.message));}
  assert(rejected,'Stage 20 accepted a baseline artifact set different from the unchanged-confirmed candidate or bypassed semantic confirmation adjudication.');
  // Positive baseline derivation is exercised by verify-full-cycle.mjs using a complete application-established unchanged-confirmation iteration.
}
...
{
 const p=project('JOB-EVIDENCE-V2'),t=record('tests',6,{REQ_ID:'REQ-E',TEST_TYPE:'DETERMINISTIC',EXECUTION_MODE:'EXTERNAL_AGENT_TOOL',REQUIRED_CAPABILITY:'sha256',ARTIFACT_REQUIREMENTS:'exact bytes',EVIDENCE_TO_PRESERVE:'byte hash',STATUS:'READY'},'TEST-E'),r=record('deterministicResults',22,{TEST_ID:'TEST-E',ACTUAL_RESULT:'same',DETERMINATION:'SATISFIED',EVIDENCE:'agent says same'},'RESULT-E');assert(!engine.evaluateEvidenceSufficiency(p,{test:t,result:r}).sufficient,'Prose satisfied a byte test.');const a=record('artifacts',22,{FILENAME:'x.bin',TYPE:'application/octet-stream',BYTE_SIZE:1,SHA256:'b'.repeat(64),AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED'},'ART-E'),e=record('evidenceRecords',22,{KIND:'TOOL_OUTPUT',DESCRIPTION:'hash',AUTHORITY_TYPE:'EXTERNAL_TOOL',LOCATION:'tool',CONTENT:'sha256 output',ATTACHMENT_ID:'ART-E',STATUS:'PRESERVED'},'EVIDENCE-E');p.projectData.artifacts.push(a);p.projectData.evidenceRecords.push(e);r.evidenceRefs=['EVIDENCE-E'];assert(engine.evaluateEvidenceSufficiency(p,{test:t,result:r}).sufficient,'Verified byte-backed evidence was not sufficient.');
}
{
 const p=project('JOB-CONTRADICTION-V2');p.job.CURRENT_REQUIREMENTS_VERSION='R1';p.job.CURRENT_TEST_SUITE_VERSION='T1';const scope=engine.currentScope(p),td=record('tests',6,{REQ_ID:'REQ-C',TEST_TYPE:'DETERMINISTIC',EXECUTION_MODE:'EXTERNAL_AGENT_TOOL',REQUIRED_CAPABILITY:'TEST_TOOL',ARTIFACT_REQUIREMENTS:'NONE',STATUS:'READY'},'TEST-C-DET'),tm=record('tests',6,{REQ_ID:'REQ-C',TEST_TYPE:'MEANING',EXECUTION_MODE:'INDEPENDENT_AGENT_REVIEW',REQUIRED_CAPABILITY:'MEANING_REVIEW',ARTIFACT_REQUIREMENTS:'NONE',STATUS:'READY'},'TEST-C-MEAN'),e=record('evidenceRecords',22,{KIND:'TOOL_OUTPUT',DESCRIPTION:'controlled',AUTHORITY_TYPE:'EXTERNAL_TOOL',LOCATION:'exec-1',CONTENT:'SATISFIED',STATUS:'PRESERVED'},'EVIDENCE-C');for(const x of [td,tm,e])x.scope={...scope};p.projectData.tests.push(td,tm);p.projectData.evidenceRecords.push(e);const d=record('deterministicResults',22,{REQ_ID:'REQ-C',PRODUCT_ID:'PRODUCT-C',TEST_ID:'TEST-C-DET',TOOL_AND_VERSION:'fixture',ACTUAL_RESULT:'SATISFIED',DETERMINATION:'SATISFIED'},'DET-C'),m=record('meaningResults',23,{REQ_ID:'REQ-C',PRODUCT_ID:'PRODUCT-C',TEST_ID:'TEST-C-MEAN',PRODUCT_LOCATION:'product',REQUIRED_MEANING:'A',OBSERVED_MEANING:'B',EVIDENCE_BASED_COMPARISON:'MISMATCH',DETERMINATION:'VIOLATED'},'MEAN-C');d.scope={...scope};m.scope={...scope};d.evidenceRefs=['EVIDENCE-C'];p.projectData.deterministicResults.push(d);p.projectData.meaningResults.push(m);assert(engine.detectCurrentContradictions(p).some(x=>x.type==='DETERMINISTIC_MEANING_CONFLICT'),'Cross-method contradiction was not detected.');
}
...
// application-result-adjudication-v1: favorable external claims never override contradictory observations or missing proof.
{
 const p=project('JOB-RESULT-ADJUDICATION'),test=record('tests',6,{REQ_ID:'REQ-ADJ',TEST_TYPE:'DETERMINISTIC',EXECUTION_MODE:'EXTERNAL_AGENT_TOOL',REQUIRED_CAPABILITY:'controlled verifier',ARTIFACT_REQUIREMENTS:'NONE',INPUTS:'controlled input',TOOLS:'controlled verifier',PROCEDURE:'execute and observe',EXPECTED_RESULT:'SATISFIED',FAILURE_CONDITION:'observed failure',EVIDENCE_TO_PRESERVE:'typed execution evidence',STATUS:'READY'},'TEST-ADJ');
 const contradictory=record('deterministicResults',22,{PRODUCT_ID:'PRODUCT-ADJ',TEST_ID:'TEST-ADJ',TOOL_AND_VERSION:'verifier 1',PROCEDURE:'execute and observe',EXPECTED_RESULT:'SATISFIED',ACTUAL_RESULT:'VIOLATED',DETERMINATION:'SATISFIED',EVIDENCE:'claimed pass'},'RESULT-ADJ');
 p.projectData.tests.push(test);p.projectData.deterministicResults.push(contradictory);
 const verdict=engine.evaluateResultConsistency(p,'deterministicResults',contradictory,test);assert(verdict.determination==='VIOLATED','Contradictory deterministic observation was not adjudicated as VIOLATED.');
 const weak=record('evidenceRecords',22,{KIND:'NARRATIVE',DESCRIPTION:'bare claim',AUTHORITY_TYPE:'',LOCATION:'',CONTENT:'x',STATUS:'PRESERVED'},'EVIDENCE-WEAK');p.projectData.evidenceRecords.push(weak);contradictory.evidenceRefs=['EVIDENCE-WEAK'];assert(!engine.evaluateEvidenceContract(p,{collection:'deterministicResults',test,result:contradictory}).sufficient,'Bare narrative was accepted as establishing evidence.');
}
