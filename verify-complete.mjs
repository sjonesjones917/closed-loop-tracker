...
function promptForContext(p,stage,operation,contextId,scopeOverrides={}){
  const scope=prompts.scopeFor(stage,{...p,revision:Number(p.revision||0)+1},{...scopeOverrides,contextId}),prepared=engine.prepareCurrentOperationReservation(p,{stage,operation,contextId,scope,owningTabInstance:'VERIFY_COMPLETE'}),preview=engine.clone(p);
  preview.revision=prepared.expectedRevision;
  const r={...prompts.buildPromptRecord(stage,preview,{operation,scope:prepared.scope,operationReservation:prepared}),generatedAt:new Date().toISOString()};
  engine.registerGeneratedPrompt(p,r);engine.reserveOperation(p,{preparedReservation:prepared,promptId:r.instructionId});p.revision=prepared.expectedRevision;engine.recalculate(p);return r;
}
function prompt(p,stage,operation='COMPLETE',purpose='GENERAL'){
  const context=engine.registerFreshContext(p,{stage,externalContextIdentifier:`VERIFY-COMPLETE-${stage}-${++promptContextSequence}`,operatorLabel:'VERIFY_COMPLETE',purpose}),contextId=engine.recordId(context,'freshContexts');return promptForContext(p,stage,operation,contextId);
}
const responseBinding=promptRecord=>({packageId:promptRecord.operationBinding.packageId||null,operationReservationId:promptRecord.operationBinding.operationReservationId,challengeNonce:promptRecord.operationBinding.challengeNonce,targetSlot:promptRecord.operationBinding.targetSlot});
function acceptStage1Fixture(p){
  ...
  engine.recordStageConfirmation(next,1,true,'Establish the accepted Stage 01 prerequisite for this verifier.','VERIFY',{acceptedChangeId:acceptedChange.changeId,inputVersion:next.job.CURRENT_INPUT_VERSION,instructionId:acceptedChange.promptId,contextSignature:acceptedChange.contextSignature,operatorLabel:'VERIFY'});
...
function establishNoSourceStage2(p){
  const authorPrompt=prompt(p,2),authorEnvelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage:2,...responseBinding(authorPrompt),operation:authorPrompt.operation,promptIdentity:{instructionId:authorPrompt.instructionId,bodySha256:authorPrompt.bodySha256,contractSha256:authorPrompt.contractSha256,contextSignature:authorPrompt.contextSignature},scope:authorPrompt.scope,responseType:'DATA_PROPOSAL',humanInputRequests:[],stageData:{AUTHORITY_HIERARCHY:'No external authority applies inside the declared bounded verifier scope.',SOURCE_APPLICABILITY_DETERMINATION:'NO_APPLICABLE_EXTERNAL_SOURCE',KNOWN_CONTROLLING_SOURCES_EXAMINED:'The declared bounded search was executed.',SEARCH_UNIVERSE:{projectScope:'verify-complete controlled relationship fixture',authorityClasses:['external standards','governing specifications','authoritative registries']},SEARCH_PROCEDURE:'Inspect every declared location with every declared strategy and preserve exact execution evidence.',SEARCH_LOCATIONS:['Controlled verifier authority registry'],SEARCH_QUERIES_OR_STRATEGIES:['Exact deliverable governing-authority search'],SEARCH_CUTOFF:'Current controlled fixture revision',SEARCH_LIMITATIONS:'Bounded fixture registry only; no universal-absence claim.',SEARCH_EXECUTION_EVIDENCE:['author-search-evidence'],DISCOVERY_RISK:'NONMATERIAL'},records:{},evidence:[{temporaryKey:'author-search-evidence',kind:'WORKFLOW_EVIDENCE',description:'Bounded source-search execution evidence',authorityType:'AGENT_CLAIM',location:'verify-complete.mjs',content:'The exact declared location and strategy were executed for this controlled fixture.'}],unresolved:[],warnings:[],attachments:[]};
  let prepared=ingestion.prepare(p,{stage:2,text:JSON.stringify(authorEnvelope),promptRecord:authorPrompt});assert(prepared.validation.valid,`Stage 02 bounded-search author fixture rejected: ${JSON.stringify(prepared.validation.issues)}`);p=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY'}).project;
  assert(!engine.gate(2,p).complete&&engine.gate(2,p).reasons.some(reason=>/adequacy review/i.test(reason)),'Stage 02 closed before independent search-adequacy review.');
  const reviewPrompt=prompt(p,2,'SEARCH_ADEQUACY_REVIEW','REVIEWER'),reviewEnvelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage:2,...responseBinding(reviewPrompt),operation:reviewPrompt.operation,promptIdentity:{instructionId:reviewPrompt.instructionId,bodySha256:reviewPrompt.bodySha256,contractSha256:reviewPrompt.contractSha256,contextSignature:reviewPrompt.contextSignature},scope:reviewPrompt.scope,responseType:'DATA_PROPOSAL',humanInputRequests:[],stageData:{},records:{sourceSearchAdequacyReviews:[{tempKey:'search-review',fields:{ADEQUACY_FINDING:'ADEQUATE',REASONING:'A distinct reviewer checked the exact bounded universe, procedure, locations, strategies, execution evidence, limitations, empty source set, and nonmaterial residual discovery risk.'},relationships:{},evidenceRefs:['review-evidence']}]},evidence:[{temporaryKey:'review-evidence',kind:'WORKFLOW_EVIDENCE',description:'Independent bounded-search adequacy review',authorityType:'AGENT_CLAIM',location:'verify-complete.mjs',content:'Target-only independent assessment of the current source-search contract.'}],unresolved:[],warnings:[],attachments:[]};
  prepared=ingestion.prepare(p,{stage:2,text:JSON.stringify(reviewEnvelope),promptRecord:reviewPrompt});assert(prepared.validation.valid,`Stage 02 independent adequacy-review fixture rejected: ${JSON.stringify(prepared.validation.issues)}`);p=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY'}).project;
  assert(engine.gate(2,p).complete,'Stage 02 did not close after the distinct accepted adequacy review.');return p;
}
...
// Invalid canonical relationship is rejected before mutation.
{
  ...
  assert(prepared.project.projectData.research.length===0&&prepared.project.projectData.acceptedChanges.length===acceptedBefore,'Invalid relationship partially mutated canonical state.');
}
...
{
  let p=acceptStage1Fixture(project('JOB-REFINEMENT-CANONICAL'));const stage=2,authorPrompt=prompt(p,stage),authorEnvelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage,...responseBinding(authorPrompt),operation:authorPrompt.operation,promptIdentity:{instructionId:authorPrompt.instructionId,bodySha256:authorPrompt.bodySha256,contractSha256:authorPrompt.contractSha256,contextSignature:authorPrompt.contextSignature},scope:authorPrompt.scope,responseType:'DATA_PROPOSAL',humanInputRequests:[],stageData:{AUTHORITY_HIERARCHY:'The identified primary external authority controls the verifier fixture.',SOURCE_APPLICABILITY_DETERMINATION:'APPLICABLE_SOURCES_ESTABLISHED',KNOWN_CONTROLLING_SOURCES_EXAMINED:'The one declared primary source was inspected.'},records:{sources:[{tempKey:'source-refine',fields:{TITLE:'Controlled External Standard',ISSUING_ORGANIZATION_OR_AUTHOR:'Independent Standards Body',SOURCE_TYPE:'OFFICIAL_STANDARD',PUBLICATION_ORIGIN:'Independent Standards Body',URL_REFERENCE:'https://example.invalid/controlled-standard',AUTHORITY_LEVEL:'PRIMARY',AUTHORITY_ROLE:'GOVERNING',RELEVANCE:'Controls the accepted-response refinement fixture.',APPLICABLE_PORTIONS:'Complete controlled fixture section.',INSPECTION_STATUS:'INSPECTED',CURRENCY_STATUS:'CURRENT',SUPERSESSION_STATUS:'NOT SUPERSEDED',CONTROLLING_STATE:'CONTROLLING'},relationships:{},evidenceRefs:['source-evidence']}]},evidence:[{temporaryKey:'source-evidence',kind:'SOURCE_INSPECTION',description:'Controlled source inspection',authorityType:'AGENT_CLAIM',location:'verify-complete.mjs',content:'The declared source was inspected for this fixture.'}],unresolved:[],warnings:[],attachments:[]};
  const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(authorEnvelope),promptRecord:authorPrompt});assert(prepared.validation.valid,`Accepted Stage 02 refinement fixture rejected: ${JSON.stringify(prepared.validation.issues)}`);const committed=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY'});p=committed.project;const acceptedChange=committed.acceptedChange,sourceId=acceptedChange.canonicalRecordIds.find(id=>id.startsWith('SOURCE-'));assert(sourceId&&engine.records(p,'sources',{stage}).some(item=>engine.recordId(item,'sources')===sourceId),'Stage 02 refinement fixture did not create its canonical source.');
  const reason='The accepted source analysis omitted an applicable authority and must be replaced completely.';engine.invalidateAcceptedResponse(p,{stage,rawResponseId:acceptedChange.rawResponseId,reason,operatorLabel:'VERIFY'});
  assert(engine.records(p,'sources',{stage}).length===0,'Invalidated accepted response left same-stage canonical source active.');assert(engine.acceptedChanges(p,stage).length===0,'Invalidated accepted change retained current stage authority.');assert(Object.keys(p.stages[stage].agentData||{}).length===0&&p.stages[stage].acceptedDataChangeIds.length===0&&p.stages[stage].acceptedResponseIds.length===0,'Invalidated stage retained accepted agent state.');const replacement=promptForContext(p,stage,'COMPLETE',authorPrompt.scope.contextId);assert(replacement.prompt.includes(reason),'Replacement prompt omitted accepted-result refinement reason.');assert(replacement.contextManifest.acceptedResultRefinements?.some(x=>x.reason===reason),'Replacement prompt identity did not bind the refinement reason.');
}
