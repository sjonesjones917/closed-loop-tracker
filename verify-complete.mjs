...
function establishSemanticPrerequisiteFixture(p){
  const intake=engine.intakeCoverageManifest(p),capture={schema:'closed-loop-stage01-capture/1',inputVersion:intake.inputVersion,manifestSha256:intake.manifestSha256,units:intake.units.map((unit,index)=>({sourceUnitId:unit.unitId,sourceRawValueSha256:unit.rawValueSha256,disposition:'RETAINED_AS_CONTEXT',reason:'Synthetic prerequisite capture.',extractedStatements:[{statementKey:`PREREQ-${index+1}`,text:unit.rawValueText||unit.label||unit.unitId,statementClass:'CONTEXT',sourceLocations:[{kind:'OTHER',value:unit.sourceLocation||unit.unitId}]}]}))};
  p.stages[1].agentData={EXACT_DELIVERABLE_REQUESTED:'Synthetic controlled verification fixture.',ASSUMPTIONS:'NONE',UNKNOWN_INFORMATION:'NONE',INPUT_SET_CONTENTS:JSON.stringify(capture)};
  const currentScope=engine.currentScope(p),change={changeId:`CHANGE-SEARCH-${p.job.JOB_ID}`,rawResponseId:`RAW-SEARCH-${p.job.JOB_ID}`,proposalId:`PROPOSAL-SEARCH-${p.job.JOB_ID}`,jobId:p.job.JOB_ID,stage:2,responseType:'DATA_PROPOSAL',status:'COMMITTED',operation:'COMPLETE',scope:{...currentScope},eventSequence:1,canonicalRecordIds:[`EVIDENCE-SEARCH-${p.job.JOB_ID}`],stageFields:['SEARCH_EXECUTION_EVIDENCE','DISCOVERY_RISK']},evidenceId=change.canonicalRecordIds[0],searchData={SOURCE_APPLICABILITY_DETERMINATION:'APPLICABLE_SOURCES_ESTABLISHED',SEARCH_UNIVERSE:'Synthetic closed fixture universe.',SEARCH_PROCEDURE:'Inspect the declared fixture universe.',SEARCH_LOCATIONS:['fixture'],SEARCH_QUERIES_OR_STRATEGIES:['fixture lookup'],SEARCH_CUTOFF:'CURRENT FIXTURE',SEARCH_LIMITATIONS:'Synthetic acceptance fixture only.',SEARCH_EXECUTION_EVIDENCE:evidenceId,DISCOVERY_RISK:'NONMATERIAL'},evidenceRecord=record('evidenceRecords',2,{KIND:'SEARCH_EXECUTION',DESCRIPTION:'Synthetic bounded-search execution evidence.',AUTHORITY_TYPE:'AGENT_CLAIM',SOURCE_ID:'',LOCATION:'verify-complete.mjs',CONTENT:'The declared synthetic source-search universe was inspected.',ATTACHMENT_ID:'',SHA256:globalThis.closedLoopHash.sha256Text(evidenceId),STATUS:'PRESERVED'},evidenceId);
  evidenceRecord.scope={...currentScope};evidenceRecord.source='AGENT_RESPONSE';evidenceRecord.rawResponseId=change.rawResponseId;evidenceRecord.sourceProposalId=change.proposalId;evidenceRecord.temporaryKey='search-execution';evidenceRecord.jobId=p.job.JOB_ID;engine.refreshRecordHashes(evidenceRecord,'evidenceRecords');p.projectData.evidenceRecords.push(evidenceRecord);p.projectData.acceptedChanges.push(change);p.projectData.responseProposals.push({proposalId:change.proposalId,rawResponseId:change.rawResponseId,stage:2,status:'ACCEPTED',envelope:{operation:'COMPLETE',scope:{...currentScope}},proposedStageData:{SEARCH_EXECUTION_EVIDENCE:evidenceId,DISCOVERY_RISK:'NONMATERIAL'},evidence:[{id:evidenceId,temporaryKey:'search-execution',fields:evidenceRecord.fields}]});p.stages[2].agentData=searchData;
  assert(engine.evaluateIntakeSemanticChallenge(p).complete&&!engine.evaluateIntakeSemanticChallenge(p).required,'Synthetic prerequisite unexpectedly requires a semantic challenge.');assert(engine.evaluateSourceSearchClosure(p).complete,'Synthetic bounded source-search prerequisite is incomplete.');return p;
}
function establishReleaseEligibilityFixture(p,artifacts){
  establishSemanticPrerequisiteFixture(p);
  const productId=`PRODUCT-${p.job.JOB_ID}`,scope={...engine.currentScope(p),productId};p.job.CURRENT_PRODUCT_ID=productId;
  const product=record('products',21,{PRODUCT_VERSION:'PRODUCT-v001',BASELINE_ID:'BASELINE-FIXTURE',EXECUTION_ID:`EXECUTION-${p.job.JOB_ID}`,PRODUCTION_CONTEXT_ID:`CONTEXT-${p.job.JOB_ID}`,INSTRUCTION_VERSION:p.job.CURRENT_INSTRUCTION_VERSION,GENERATED_ARTIFACT_INVENTORY:artifacts.map(item=>item.artifactId),STATUS:'COMPLETED'},productId);product.scope={...scope};product.completionState='COMPLETED';p.projectData.products.push(product);
  for(const item of artifacts){
    const artifact=record('artifacts',21,{FILENAME:item.name,TYPE:'application/octet-stream',BYTE_SIZE:item.size,SHA256:item.sha256,STORAGE_REFERENCE:`indexeddb:${item.artifactId}`,AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED',AUTHORIZED_DELIVERY_FILENAME:item.name},item.artifactId);
    artifact.scope={...scope};p.projectData.artifacts.push(artifact);
  }
  const release=record('releaseRecords',27,{DETERMINATION:'ACCEPTED'},`RELEASE-${p.job.JOB_ID}`);release.scope={...scope};p.projectData.releaseRecords.push(release);
  p.projectData.acceptedChanges.push({changeId:`CHANGE-RELEASE-${p.job.JOB_ID}`,stage:27,status:'COMMITTED',responseType:'DATA_PROPOSAL',operation:'COMPLETE',rawResponseId:`RAW-RELEASE-${p.job.JOB_ID}`,scope:{...scope}});
  p.stages[26].status='COMPLETE';p.stages[26].gate={complete:true,blocked:false,reasons:[]};p.stages[27].status='COMPLETE';p.stages[27].gate={complete:true,blocked:false,reasons:[]};
  assert(engine.gate(27,p).complete,'Release-eligible fixture did not establish the real Stage 27 gate prerequisites.');
  return p;
}
function prompt(p,stage,{operation='COMPLETE',scope={},targetSlot=`STAGE-${String(stage).padStart(2,'0')}:${operation}:FIXTURE`,packageId,fixturePriorStageComplete=false}={}){
  const contract=schema.operationContract(stage,operation),requiredPackage=contract.packageIdPolicy==='REQUIRED',currentRevision=Number(p.revision||0),preview=structuredClone(p);preview.revision=currentRevision+1;
  const previewPrompt=prompts.buildPromptRecord(stage,preview,{operation,scope}),resolvedPackageId=requiredPackage?(packageId||`PACKAGE-${p.job.JOB_ID}-${stage}-${targetSlot}`):null,reservation=engine.reserveOperation(p,{stage,operation,targetSlot,packageId:resolvedPackageId,scope:previewPrompt.scope,owningTabInstance:'VERIFY-COMPLETE',payload:{fixture:'verify-complete.mjs',targetSlot}}),reservationId=engine.recordId(reservation,'operationReservations');if(fixturePriorStageComplete&&stage>1){p.stages[stage-1].status='COMPLETE';p.stages[stage-1].gate={complete:true,blocked:false,reasons:[]};}const boundState=structuredClone(p);boundState.revision=currentRevision+1;
  const r={...prompts.buildPromptRecord(stage,boundState,{operation,scope,operationReservation:reservationId}),generatedAt:new Date().toISOString()};engine.registerGeneratedPrompt(p,r);engine.bindOperationReservation(p,{reservationId,promptId:r.instructionId,packageId:resolvedPackageId});p.revision=currentRevision+1;return r;
}
...
// Structured blocking human question blocks the stage until answered and versions User Job Input.
{
  ...
  const prepared=ingestion.prepare(p,{stage,text:JSON.stringify(e),promptRecord:pr});assert(prepared.validation.valid,`Valid clarification envelope rejected: ${JSON.stringify(prepared.validation.issues)}`);
...
{
  let p=establishSemanticPrerequisiteFixture(project('JOB-STAGE11-BOUNDARY'));
  const candidateBytes=new TextEncoder().encode('stage-11-boundary-candidate'),candidateSha=globalThis.closedLoopHash.sha256Text('stage-11-boundary-candidate');
  engine.registerArtifactBytes(p,{stage:10,artifactId:'ARTIFACT-STAGE11',filename:'stage11.bin',mediaType:'application/octet-stream',byteSize:candidateBytes.byteLength,sha256:candidateSha});
  const frozen=engine.freezeCandidate(p,{stage:10,artifactIds:['ARTIFACT-STAGE11'],operatorLabel:'VERIFY'}),iterationId=engine.recordId(frozen.iteration,'iterations'),candidateId=engine.recordId(frozen.candidate,'candidateFreezes');
  p.stages[10].status='COMPLETE';p.stages[10].gate={complete:true,blocked:false,reasons:[]};
  const slots=engine.reserveRunBatch(p,{stage:11,iterationId,candidateId,count:10});
  for(let i=0;i<slots.length;i++){
    const slot=slots[i];engine.registerFreshContext(p,{stage:11,externalContextIdentifier:`external-stage11-${i}`,operatorLabel:'VERIFY'});
    p.stages[10].status='COMPLETE';p.stages[10].gate={complete:true,blocked:false,reasons:[]};
    const pr=prompt(p,11,{scope:{runId:slot.runId,contextId:slot.contextId},targetSlot:`RUN-${slot.runId}`,packageId:`PACKAGE-${slot.runId}`,fixturePriorStageComplete:true}),fields={FRESH_CONTEXT_RECORD:slot.contextId,CONTAMINATION_CHECK:'NONE',TOOL_CONFIGURATION:'CONTROLLED',EXECUTION_STATUS:'COMPLETED',COMPLETE_OUTPUT:`output-${i}`};
    const envelope={schema:schema.RESPONSE_SCHEMA,jobId:p.job.JOB_ID,stage:11,operation:pr.operation,...responseBinding(pr),promptIdentity:{instructionId:pr.instructionId,bodySha256:pr.bodySha256,contractSha256:pr.contractSha256,contextSignature:pr.contextSignature},scope:pr.scope,responseType:'DATA_PROPOSAL',humanInputRequests:[],stageData:{FROZEN_EXECUTION_PACKAGE:'Application-bound frozen candidate package.'},records:{runs:[{targetId:slot.runId,fields,relationships:{},evidenceRefs:['lane-evidence']}]},evidence:[{temporaryKey:'lane-evidence',kind:'WORKFLOW_EVIDENCE',description:'Stage 11 lane evidence',location:'verify-complete.mjs',content:`lane-${i}`}],unresolved:[],warnings:[],attachments:[]};
    const prepared=ingestion.prepare(p,{stage:11,text:JSON.stringify(envelope),promptRecord:pr});assert(prepared.validation.valid,`Stage 11 boundary lane ${i} rejected: ${JSON.stringify(prepared.validation.issues)}`);p=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'VERIFY'}).project;
  }
  p.stages[10].status='COMPLETE';p.stages[10].gate={complete:true,blocked:false,reasons:[]};
  const stage11=engine.gate(11,p);assert(stage11.complete,`Stage 11 incorrectly depends on Stage 12 verification data: ${stage11.reasons.join('; ')} | ${JSON.stringify(engine.evaluateContextIndependence(p,{role:'RUN_BATCH',iterationId}))}`);
...
{
  const audited=[{artifactId:'A',name:'a.bin',size:1,sha256:'a'.repeat(64)},{artifactId:'B',name:'b.bin',size:2,sha256:'b'.repeat(64)}],p=establishReleaseEligibilityFixture(project('JOB-ORDER'),audited),delivery=[audited[1],audited[0]];
  const r=engine.verifyArtifactIdentity(p,audited,delivery);assert(r.length===2&&r.every(item=>engine.recordValue(item,'EXACT_HASH_MATCH')===true)&&p.release.authorization==='NOT AUTHORIZED'&&p.stages[28].derivedData.DELIVERY_AUTHORIZATION_EFFECTIVE===false,'Artifact identity depends on file-selection order or Stage 28 incorrectly authorized final delivery.');
}
...
// Release identity is prohibited before ACCEPTED and exact mismatches remain unauthorized.
{
  ...
  const audited=[{artifactId:'A',name:'x.bin',size:3,sha256:'a'.repeat(64)}];establishReleaseEligibilityFixture(p,audited);
  const result=engine.verifyArtifactIdentity(p,audited,[{artifactId:'A',name:'x.bin',size:4,sha256:'b'.repeat(64)}]);
...
// Missing evidence-chain links remain missing; the application does not invent them.
{
  ...
  const chains=engine.constructEvidenceChains(p),status=engine.evidenceChainStatus(p);assert(chains.length===0&&status.complete===false&&status.reasons.includes('NO_CURRENT_APPLICABLE_MANDATORY_PROPOSITION'),'A mandatory requirement with no canonical proposition/proof obligation was fabricated as an evidence chain.');
}
...
// Exact unchanged-confirmed candidate artifact identity controls Stage 20 baseline bytes.
{
  ...
  engine.registerArtifactBytes(p,{stage:17,artifactId:'ARTIFACT-CONFIRMED',filename:'confirmed.bin',mediaType:'application/octet-stream',byteSize:10,sha256:shaA});
  const frozen=engine.freezeCandidate(p,{stage:17,artifactIds:['ARTIFACT-CONFIRMED'],operatorLabel:'VERIFY',purpose:'CORRECTED'}),candidateId=engine.recordId(frozen.candidate,'candidateFreezes');
  p.stages[18].status='COMPLETE';p.stages[18].gate={complete:true,blocked:false,reasons:[]};const iteration=engine.beginUnchangedConfirmationIteration(p,{candidateId,operatorLabel:'VERIFY'}),iterationId=engine.recordId(iteration,'iterations');
  engine.registerArtifactBytes(p,{stage:20,artifactId:'ARTIFACT-DIFFERENT',filename:'different.bin',mediaType:'application/octet-stream',byteSize:10,sha256:shaB});
  const scope={...engine.currentScope(p),iterationId,candidateId},confirmation=record('confirmationRecords',19,{ITERATION_ID:iterationId,CANDIDATE_ID:candidateId,DETERMINATION:'SATISFIED'},'CONFIRM-EXACT-CANDIDATE');confirmation.scope=scope;p.projectData.confirmationRecords.push(confirmation);
  let rejected=false,rejectionMessage='';try{engine.freezeBaseline(p,{artifactIds:['ARTIFACT-DIFFERENT'],operatorLabel:'VERIFY'});}catch(error){rejectionMessage=String(error.message);rejected=/exact artifact set/i.test(rejectionMessage);}
  assert(rejected,`Stage 20 accepted a baseline artifact set different from the unchanged-confirmed candidate: ${rejectionMessage}`);
  const baseline=engine.freezeBaseline(p,{operatorLabel:'VERIFY'});assert(JSON.stringify(engine.recordValue(baseline,'IMMUTABLE_ARTIFACT_RECORDS'))===JSON.stringify(['ARTIFACT-CONFIRMED']),'Stage 20 did not derive baseline artifacts from the unchanged-confirmed candidate manifest.');assert(engine.recordValue(baseline,'APPROVED_VERSIONS').candidateId===candidateId&&engine.recordValue(baseline,'APPROVED_VERSIONS').iterationId===iterationId,'Stage 20 baseline lost the exact unchanged-confirmation candidate/iteration identity.');
}

// Stage 19 run slots remain production-execution contexts, distinct from their run-bound reviewers.
{
 const p=project('JOB-STAGE19-ROLE-SEPARATION');engine.registerArtifactBytes(p,{stage:17,artifactId:'ARTIFACT-STAGE19-ROLE',filename:'candidate.bin',mediaType:'application/octet-stream',byteSize:1,sha256:'c'.repeat(64)});const frozen=engine.freezeCandidate(p,{stage:17,artifactIds:['ARTIFACT-STAGE19-ROLE'],operatorLabel:'VERIFY',purpose:'CORRECTED'}),candidateId=engine.recordId(frozen.candidate,'candidateFreezes');p.stages[18].status='COMPLETE';p.stages[18].gate={complete:true,blocked:false,reasons:[]};const iteration=engine.beginUnchangedConfirmationIteration(p,{candidateId,operatorLabel:'VERIFY'}),iterationId=engine.recordId(iteration,'iterations'),slots=engine.reserveRunBatch(p,{stage:19,iterationId,candidateId,count:10}),slot=slots[0],generator=engine.records(p,'freshContexts').find(record=>engine.recordId(record,'freshContexts')===slot.contextId);assert(engine.recordValue(generator,'ROLE')==='Independent production execution agent','Stage 19 reserved a reviewer-role context for product generation.');const reviewer=engine.registerFreshContext(p,{stage:19,runId:slot.runId,externalContextIdentifier:'STAGE19-ROLE-REVIEWER',operatorLabel:'VERIFY',purpose:'REVIEWER'}),reviewerContextId=engine.recordId(reviewer,'freshContexts'),independence=engine.evaluateContextIndependence(p,{role:'VERIFICATION',iterationId,runId:slot.runId,verifierContextId:reviewerContextId});assert(independence.dimensionMap.EXECUTOR_OR_REVIEWER_ROLE_SEPARATION.determination==='APPLICATION_ESTABLISHED','Stage 19 did not preserve application-controlled generator/reviewer role separation.');p.projectData.acceptedChanges.push({changeId:'CHANGE-STAGE19-EXECUTE',stage:19,status:'COMMITTED',responseType:'DATA_PROPOSAL',operation:'EXECUTE_RUN',rawResponseId:'RAW-STAGE19-EXECUTE',scope:{iterationId,runId:slot.runId,contextId:slot.contextId}},{changeId:'CHANGE-STAGE19-VERIFY',stage:19,status:'COMMITTED',responseType:'DATA_PROPOSAL',operation:'VERIFY',rawResponseId:'RAW-STAGE19-VERIFY',proposalId:'PROPOSAL-STAGE19-VERIFY',scope:{iterationId,runId:slot.runId,contextId:reviewerContextId}});const verification=record('verification',19,{RUN_ID:slot.runId},'VERIFICATION-STAGE19-TRUST');verification.scope={...engine.currentScope(p),iterationId,runId:slot.runId};verification.rawResponseId='RAW-STAGE19-VERIFY';verification.sourceProposalId='PROPOSAL-STAGE19-VERIFY';const trust=engine.releaseVerificationTrust(p,verification);assert(!trust.reasons.includes('Verifier execution receipt is the same accepted response as the generating run.'),'Stage 19 verifier response was misclassified as its own generating-run response.');const substituted={...verification,rawResponseId:'RAW-STAGE19-EXECUTE',sourceProposalId:''},substitutedTrust=engine.releaseVerificationTrust(p,substituted);assert(substitutedTrust.reasons.includes('Verifier execution receipt is the same accepted response as the generating run.'),'A true Stage 19 EXECUTE_RUN response was accepted as an independent verifier receipt.');
}
...
{
 const audited=[{artifactId:'A',name:'a.bin',size:3,sha256:'a'.repeat(64)}],p=establishReleaseEligibilityFixture(project('JOB-IDENTITY-RECOVERY'),audited),bad=[{artifactId:'A',name:'a.bin',size:4,sha256:'b'.repeat(64)}],good=[{...audited[0]}];engine.verifyArtifactIdentity(p,audited,bad);p.stages[26].status='COMPLETE';p.stages[27].status='COMPLETE';const corrected=engine.verifyArtifactIdentity(p,audited,good);assert(engine.records(p,'artifactIdentities').filter(item=>item.active!==false).length===1&&corrected.length===1&&engine.recordValue(corrected[0],'EXACT_HASH_MATCH')===true&&p.release.authorization==='NOT AUTHORIZED','A corrected Stage 28 comparison remained blocked by an older active mismatch or incorrectly authorized final delivery.');const count=p.projectData.artifactIdentities.length;p.stages[26].status='COMPLETE';p.stages[27].status='COMPLETE';const again=engine.verifyArtifactIdentity(p,audited,good);assert(p.projectData.artifactIdentities.length===count&&again[0].id===corrected[0].id,'Identical Stage 28 evidence created a duplicate comparison batch.');
}
...
}
{
 ...
 let ev=engine.evaluateContextIndependence(p,{role:'RUN_BATCH',iterationId:iid});assert(ev.determination==='VIOLATED'&&ev.dimensionMap.APPLICATION_SESSION_DISTINCTNESS.determination==='APPLICATION_ESTABLISHED'&&ev.dimensionMap.EXTERNAL_CONTEXT_IDENTIFIER_DISTINCTNESS.determination==='APPLICATION_ESTABLISHED'&&ev.dimensionMap.APPLICATION_INPUT_ISOLATION.determination==='VIOLATED'&&ev.dimensionMap.PROVIDER_CONTEXT_INDEPENDENCE.determination==='UNKNOWN','Distinct local identifiers were incorrectly elevated above their dimension-specific evidence.');const ctx2=engine.records(p,'freshContexts')[1];ctx2.fields.EXTERNAL_CONTEXT_IDENTIFIER='external-0';ctx2.EXTERNAL_CONTEXT_IDENTIFIER='external-0';ev=engine.evaluateContextIndependence(p,{role:'RUN_BATCH',iterationId:iid});assert(ev.determination==='VIOLATED'&&ev.dimensionMap.EXTERNAL_CONTEXT_IDENTIFIER_DISTINCTNESS.determination==='VIOLATED','Duplicate external context was not detected.');
}
{
...
const expectedEntailmentReviewOperations={
  12:['COMPLETE','ENTAILMENT_REVIEW'],
  17:['FREEZE','EXECUTE_RUN','VERIFY','COMPARE','ROOT_CAUSE','REGRESSION','CORRECT','ENTAILMENT_REVIEW'],
  19:['CONFIRM_FREEZE','EXECUTE_RUN','VERIFY','COMPARE','REGRESSION_VERIFY','CONFIRM','ENTAILMENT_REVIEW'],
  22:['COMPLETE','ENTAILMENT_REVIEW'],23:['COMPLETE','ENTAILMENT_REVIEW'],24:['COMPLETE','ENTAILMENT_REVIEW'],25:['COMPLETE','ENTAILMENT_REVIEW']
};
for(const [stage,operations] of Object.entries(expectedEntailmentReviewOperations))assert(JSON.stringify(schema.STAGE_OPERATIONS[stage])===JSON.stringify(operations),`Stage ${stage} distinct entailment-review operation contract is incomplete.`);
{
...
// stage04-captured-input-regression-v3
{
  ...
  assert(handoff.send.length===0,'Stage 04 filename metadata incorrectly became a file-transfer contract.');
  assert(handoff.expectBack.length===1&&handoff.expectBack[0].kind==='STRUCTURED_RESPONSE','Stage 04 must require only its final structured response, not a repeated project-file handoff.');
...
// Reviewer independence remains dimensioned: canonical identity proves only the application-observable dimensions.
{
  ...
  assert(result.determination==='VIOLATED'&&result.dimensionMap.APPLICATION_SESSION_DISTINCTNESS.determination==='APPLICATION_ESTABLISHED'&&result.dimensionMap.EXTERNAL_CONTEXT_IDENTIFIER_DISTINCTNESS.determination==='APPLICATION_ESTABLISHED'&&result.dimensionMap.EXECUTOR_OR_REVIEWER_ROLE_SEPARATION.determination==='APPLICATION_ESTABLISHED'&&result.dimensionMap.APPLICATION_INPUT_ISOLATION.determination==='VIOLATED'&&result.dimensionMap.PROVIDER_CONTEXT_INDEPENDENCE.determination==='UNKNOWN','Distinct canonical production/reviewer contexts were elevated above the exact application-observable dimensions.');
...
// Reliability measurement counts completed application-established independent operations, never reserved runs.
{
  ...
  metrics=engine.operationalMetrics(p);assert(metrics.materiallyIndependentAcceptedOperations===0,'Synthetic completed run flags without exact accepted prompt/receipt binding were falsely counted as independent accepted operations.');
}
