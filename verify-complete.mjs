...
function prompt(p,stage){for(let prior=1;prior<stage;prior++){p.stages[prior].status='COMPLETE';p.stages[prior].gate={complete:true,blocked:false,reasons:[]};}const r={...prompts.buildPromptRecord(stage,p),generatedAt:new Date().toISOString()};p.projectData.generatedPrompts.push(r);return r;}
...
// Refining one accepted run restores only that reservation and preserves unrelated accepted lanes.
{
 ...
 p.stages[10].status='COMPLETE';p.stages[10].gate={complete:true,blocked:false,reasons:[]};const slots=engine.reserveRunBatch(p,{stage,iterationId,count:10});p.stages[10].status='COMPLETE';p.stages[10].gate={complete:true,blocked:false,reasons:[]};for(const slot of slots){const run=engine.records(p,'runs').find(r=>engine.recordId(r,'runs')===slot.runId);assert(engine.recordValue(run,'CANDIDATE_ID')===candidateId,'Scoped refinement fixture did not reserve the canonical iteration candidate.');}
