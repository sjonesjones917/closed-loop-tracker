...
// Exact unchanged-confirmed candidate artifact identity controls Stage 20 baseline bytes.
{
  ...
  const baselineAuthorization=engine.recordRegisteredHumanDecision(p,{stage:20,purpose:'BASELINE_AUTHORIZATION',targetFamily:'candidateFreezes',targetId:'CANDIDATE-CONFIRMED',value:'AUTHORIZED',operatorLabel:'VERIFY'});
  const baseline=engine.freezeBaseline(p,{baselineAuthorizationDecisionId:engine.recordId(baselineAuthorization,'humanDecisions'),operatorLabel:'VERIFY'});assert(JSON.stringify(engine.recordValue(baseline,'IMMUTABLE_ARTIFACT_RECORDS'))===JSON.stringify(['ARTIFACT-CONFIRMED']),'Stage 20 did not derive baseline artifacts from the unchanged-confirmed candidate manifest.');assert(engine.recordValue(baseline,'BASELINE_AUTHORIZATION_DECISION_ID')===engine.recordId(baselineAuthorization,'humanDecisions'),'Stage 20 baseline did not retain the exact authorization decision identity.');assert(engine.recordValue(baseline,'APPROVED_VERSIONS').candidateId==='CANDIDATE-CONFIRMED'&&engine.recordValue(baseline,'APPROVED_VERSIONS').iterationId==='ITERATION-CONFIRM','Stage 20 baseline lost the exact unchanged-confirmation candidate/iteration identity.');
}
