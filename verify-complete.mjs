...
function acceptStage1Fixture(p){
  ...
  const capture={schema:'closed-loop-stage01-capture/1',inputVersion:manifest.inputVersion,manifestSha256:manifest.manifestSha256,semanticPasses:{exhaustiveExtractionCompleted:true,omissionChallengeCompleted:true,omissionsResolved:true},units:manifest.units.map((unit,index)=>({sourceUnitId:unit.unitId,sourceRawValueSha256:unit.rawValueSha256,disposition:'RETAINED_AS_CONTEXT',reason:'Accepted Stage 01 prerequisite fixture preserves current human authority.',extractedStatements:[{statementKey:'STAGE1-'+String(index+1),text:unit.rawValueText||unit.label||unit.unitId,statementClass:'CONTEXT'}]}))};
...
// Stage 06 continuously rechecks exact required artifact custody from canonical evidence and current verified bytes.
{
  ...
  p.projectData.artifacts.push({id:'ARTIFACT-ART-1',stage:6,active:true,scope:{...scope,testSuiteVersion:'TEST-SUITE-v001'},fields:{ARTIFACT_ID:'ARTIFACT-ART-1',AVAILABILITY:'METADATA_ONLY'}});plan=engine.testExecutionPlan(p);assert(plan.missingArtifactTestIds.includes('TEST-ART-1'),'Metadata-only artifact incorrectly satisfied TEST custody.');
...
{
  const p=project('JOB-ORDER');p.job.CURRENT_PRODUCT_ID='PRODUCT-ORDER';const scope=engine.currentScope(p),hashA='a'.repeat(64),hashB='b'.repeat(64);
  const product=record('products',21,{PRODUCT_VERSION:'PRODUCT-v001',GENERATED_ARTIFACT_INVENTORY:['A','B']},'PRODUCT-ORDER');product.scope={...scope};p.projectData.products.push(product);
  for(const [artifactId,filename,size,sha256] of [['A','a.bin',1,hashA],['B','b.bin',2,hashB]]){const artifact=record('artifacts',21,{FILENAME:filename,BYTE_SIZE:size,SHA256:sha256,AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED',PRODUCT_ID:'PRODUCT-ORDER'},artifactId);artifact.scope={...scope,productId:'PRODUCT-ORDER'};artifact.relationships={PRODUCT_ID:'PRODUCT-ORDER'};p.projectData.artifacts.push(artifact);}
  const release=record('releaseRecords',27,{DETERMINATION:'ACCEPTED'},'RELEASE-ORDER');release.scope={...scope,productId:'PRODUCT-ORDER'};p.projectData.releaseRecords.push(release);
  const audited=[{artifactId:'A',name:'a.bin',size:1,sha256:hashA},{artifactId:'B',name:'b.bin',size:2,sha256:hashB}];const delivery=[{artifactId:'B',name:'b.bin',size:2,sha256:hashB,storageReference:'OPERATOR_SELECTED_DELIVERY_BYTES'},{artifactId:'A',name:'a.bin',size:1,sha256:hashA,storageReference:'OPERATOR_SELECTED_DELIVERY_BYTES'}];
  const r=engine.verifyArtifactIdentity(p,audited,delivery);assert(r.length===2&&r.every(item=>engine.recordValue(item,'DELIVERY_ARTIFACT_IDENTITY_VERIFIED')===true),'Artifact identity depends on file-selection order.');assert(p.release.authorization==='NOT AUTHORIZED'&&p.release.deliveryAuthorizationEffective===false,'Stage 28 incorrectly became final delivery authorization.');
}
...
// Release identity is prohibited before ACCEPTED and exact mismatches remain unauthorized.
{
  ...
  p.job.CURRENT_PRODUCT_ID='PRODUCT-IDENTITY';const scope=engine.currentScope(p),auditedHash='a'.repeat(64),deliveryHash='b'.repeat(64),product=record('products',21,{PRODUCT_VERSION:'PRODUCT-v001',GENERATED_ARTIFACT_INVENTORY:['A']},'PRODUCT-IDENTITY');product.scope={...scope};p.projectData.products.push(product);const artifact=record('artifacts',21,{FILENAME:'x.bin',BYTE_SIZE:3,SHA256:auditedHash,AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED',PRODUCT_ID:'PRODUCT-IDENTITY'},'A');artifact.scope={...scope,productId:'PRODUCT-IDENTITY'};artifact.relationships={PRODUCT_ID:'PRODUCT-IDENTITY'};p.projectData.artifacts.push(artifact);const release=record('releaseRecords',27,{DETERMINATION:'ACCEPTED'},'RELEASE-TEST');release.scope={...scope,productId:'PRODUCT-IDENTITY'};p.projectData.releaseRecords.push(release);
  const result=engine.verifyArtifactIdentity(p,[{artifactId:'A',name:'x.bin',size:3,sha256:auditedHash}],[{artifactId:'A',name:'x.bin',size:4,sha256:deliveryHash,storageReference:'OPERATOR_SELECTED_DELIVERY_BYTES'}]);
...
{
  const p=acceptStage1Fixture(project('JOB-REFINEMENT-CANONICAL')),stage=2,refinementContext=engine.registerFreshContext(p,{stage,externalContextIdentifier:'EXTERNAL-STAGE02-REFINEMENT',operatorLabel:'VERIFY',purpose:'AUTHOR'}),refinementContextId=engine.recordId(refinementContext,'freshContexts'),refinementScope={contextId:refinementContextId},source=record('sources',2,{TITLE:'Accepted source'},'SOURCE-REFINE');
  ...
  p.projectData.acceptedChanges.push({changeId:'CHANGE-REFINE',stage,status:'COMMITTED',responseType:'DATA_PROPOSAL',proposalId:'PROPOSAL-REFINE',rawResponseId:'RAW-REFINE',promptId:'INSTRUCTION-REFINE',operation:'SEARCH_EXECUTION',scope:refinementScope,canonicalRecordIds:['SOURCE-REFINE']});
  p.projectData.responseProposals.push({proposalId:'PROPOSAL-REFINE',stage,status:'COMMITTED',rawResponseId:'RAW-REFINE',promptId:'INSTRUCTION-REFINE',scope:refinementScope,envelope:{operation:'SEARCH_EXECUTION',scope:refinementScope}});
...
assert(fs.readFileSync('app-core.js','utf8').includes('No accepted response matches the application-selected current operation and run lane.'),'Refinement UI does not target the application-selected operation/run lane.');
...
// Exact unchanged-confirmed candidate artifact identity controls Stage 20 baseline bytes.
{
  ...
  const bytesA=new TextEncoder().encode('confirmed!'),bytesB=new TextEncoder().encode('different!'),shaA=await globalThis.closedLoopHash.sha256Bytes(bytesA),shaB=await globalThis.closedLoopHash.sha256Bytes(bytesB);
  ...
  const candidate=record('candidateFreezes',17,{ITERATION_ID:'ITERATION-CONFIRM',COMPONENT_MANIFEST:[{artifactId:'ARTIFACT-CONFIRMED',filename:'confirmed.bin',byteSize:10,sha256:shaA,storageReference:'indexeddb:ARTIFACT-CONFIRMED'}],COMPONENT_HASHES:{'ARTIFACT-CONFIRMED':shaA},STATUS:'FROZEN'},'CANDIDATE-CONFIRMED');
  ...
  const scope={...engine.currentScope(p),iterationId:'ITERATION-CONFIRM',candidateId:'CANDIDATE-CONFIRMED'};candidate.scope=scope;iteration.scope=scope;a.scope=scope;b.scope=scope;
  ...
  assert(!engine.backupCheckpointStatus(p,'BASELINE_APPROVAL').complete,'Stage 20 completed before the authorized baseline itself was captured in a restore-tested backup.');
  const disclosure=engine.evaluateBackupDisclosure(p,{}),backupEntries=[{artifactId:'ARTIFACT-CONFIRMED',jobId:p.job.JOB_ID,filename:'confirmed.bin',mediaType:'application/octet-stream',byteSize:bytesA.byteLength,sha256:shaA,base64:Buffer.from(bytesA).toString('base64')},{artifactId:'ARTIFACT-DIFFERENT',jobId:p.job.JOB_ID,filename:'different.bin',mediaType:'application/octet-stream',byteSize:bytesB.byteLength,sha256:shaB,base64:Buffer.from(bytesB).toString('base64')}],restoreProof=await store.validateBackupRestoreCandidate(p,backupEntries,{milestone:'BASELINE_APPROVAL'}),packagePayload={schema:'closed-loop-complete-backup-fixture/1',milestone:'BASELINE_APPROVAL',project:structuredClone(p),artifacts:structuredClone(backupEntries),restoreTestEvidenceSha256:restoreProof.restoreTestEvidenceSha256},expandedBytes=new TextEncoder().encode(JSON.stringify(packagePayload)),compressedBytes=new Uint8Array(await new Response(new Blob([expandedBytes]).stream().pipeThrough(new CompressionStream('gzip'))).arrayBuffer()),packageBinding={schema:'closed-loop-backup-package-binding/1',packageSha256:globalThis.closedLoopHash.sha256Value(packagePayload),packageByteSha256:await globalThis.closedLoopHash.sha256Bytes(compressedBytes),packageByteLength:compressedBytes.byteLength,expandedByteLength:expandedBytes.byteLength},pending=engine.recordPendingBackupExport(p,{milestone:'BASELINE_APPROVAL',verifiedArtifactManifest:engine.backupArtifactManifest(p),restoreProof,packageBinding,disclosureHandling:disclosure.handling,createdAt:'2026-09-01T00:00:00.000Z'});assert(!engine.backupCheckpointStatus(p,'BASELINE_APPROVAL').complete,'Generating a backup package incorrectly proved external custody.');engine.confirmBackupExternalCustody(p,{pendingExportId:pending.pendingExportId,operatorLabel:'VERIFY',acknowledged:true,createdAt:'2026-09-01T00:00:00.500Z'});assert(engine.backupCheckpointStatus(p,'BASELINE_APPROVAL').complete,'Stage 20 fixture did not establish the required application restore-tested exact-package backup checkpoint and external-custody acknowledgment after baseline authorization.');
}
...
{
 const p=project('JOB-IDENTITY-RECOVERY');p.job.CURRENT_PRODUCT_ID='PRODUCT-IDENTITY-RECOVERY';const scope=engine.currentScope(p),auditedHash='a'.repeat(64),deliveryHash='b'.repeat(64),product=record('products',21,{PRODUCT_VERSION:'PRODUCT-v001',GENERATED_ARTIFACT_INVENTORY:['A']},'PRODUCT-IDENTITY-RECOVERY');product.scope={...scope};p.projectData.products.push(product);const artifact=record('artifacts',21,{FILENAME:'a.bin',BYTE_SIZE:3,SHA256:auditedHash,AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED',PRODUCT_ID:'PRODUCT-IDENTITY-RECOVERY'},'A');artifact.scope={...scope,productId:'PRODUCT-IDENTITY-RECOVERY'};artifact.relationships={PRODUCT_ID:'PRODUCT-IDENTITY-RECOVERY'};p.projectData.artifacts.push(artifact);const release=record('releaseRecords',27,{DETERMINATION:'ACCEPTED'},'RELEASE-IDENTITY-RECOVERY');release.scope={...scope,productId:'PRODUCT-IDENTITY-RECOVERY'};p.projectData.releaseRecords.push(release);const audited=[{artifactId:'A',name:'a.bin',size:3,sha256:auditedHash}],bad=[{artifactId:'A',name:'a.bin',size:4,sha256:deliveryHash,storageReference:'OPERATOR_SELECTED_DELIVERY_BYTES'}],good=[{artifactId:'A',name:'a.bin',size:3,sha256:auditedHash,storageReference:'OPERATOR_SELECTED_DELIVERY_BYTES'}];engine.verifyArtifactIdentity(p,audited,bad);const corrected=engine.verifyArtifactIdentity(p,audited,good);assert(engine.records(p,'artifactIdentities').length===1&&corrected.length===1&&engine.recordValue(corrected[0],'DELIVERY_ARTIFACT_IDENTITY_VERIFIED')===true&&p.release.authorization==='NOT AUTHORIZED'&&p.release.deliveryAuthorizationEffective===false,'A corrected Stage 28 comparison remained blocked by an older active mismatch or incorrectly authorized final delivery.');const count=p.projectData.artifactIdentities.length,again=engine.verifyArtifactIdentity(p,audited,good);assert(p.projectData.artifactIdentities.length===count&&again[0].id===corrected[0].id,'Identical Stage 28 evidence created a duplicate comparison batch.');
}
...
}
{
 ...
 let ev=engine.evaluateContextIndependence(p,{role:'RUN_BATCH',iterationId:iid});assert(ev.determination==='EXTERNALLY_SUPPORTED'&&ev.dimensions.APPLICATION_SESSION_DISTINCTNESS.truthValue==='TRUE'&&ev.dimensions.APPLICATION_INPUT_ISOLATION.truthValue==='TRUE'&&ev.dimensions.PROVIDER_CONTEXT_INDEPENDENCE.truthValue==='UNKNOWN','Application isolation and residual provider-context uncertainty were not represented separately.');const ctx2=engine.records(p,'freshContexts')[1];ctx2.fields.EXTERNAL_CONTEXT_IDENTIFIER='external-0';ctx2.EXTERNAL_CONTEXT_IDENTIFIER='external-0';ev=engine.evaluateContextIndependence(p,{role:'RUN_BATCH',iterationId:iid});assert(ev.determination==='VIOLATED','Duplicate external context was not detected.');
}
{
 const p=project('JOB-EVIDENCE-V2'),t=record('tests',6,{REQ_ID:'REQ-E',TEST_TYPE:'DETERMINISTIC',TEST_ROLE:'ADVISORY',EXECUTION_MODE:'EXTERNAL_AGENT_TOOL',REQUIRED_CAPABILITY:'sha256',ARTIFACT_REQUIREMENTS:'exact bytes',EVIDENCE_TO_PRESERVE:'byte hash',STATUS:'READY'},'TEST-E'),r=record('deterministicResults',22,{TEST_ID:'TEST-E',ACTUAL_RESULT:'same',DETERMINATION:'SATISFIED',EVIDENCE:'agent says same'},'RESULT-E');assert(!engine.evaluateEvidenceSufficiency(p,{test:t,result:r}).sufficient,'Prose satisfied a byte test.');const a=record('artifacts',22,{FILENAME:'x.bin',TYPE:'application/octet-stream',BYTE_SIZE:1,SHA256:'b'.repeat(64),AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED'},'ART-E'),e=record('evidenceRecords',22,{KIND:'TOOL_OUTPUT',DESCRIPTION:'hash',LOCATION:'tool',CONTENT:'sha256 output',ATTACHMENT_ID:'ART-E',STATUS:'PRESERVED'},'EVIDENCE-E');p.projectData.artifacts.push(a);p.projectData.evidenceRecords.push(e);r.evidenceRefs=['EVIDENCE-E'];assert(engine.evaluateEvidenceSufficiency(p,{test:t,result:r}).sufficient,'Verified byte-backed evidence was not sufficient.');
}
{
 const p=project('JOB-CONTRADICTION-V2');p.job.CURRENT_REQUIREMENTS_VERSION='R1';p.job.CURRENT_TEST_SUITE_VERSION='T1';const scope=engine.currentScope(p),req=record('requirements',4,{MANDATORY_OPTIONAL_STATUS:'MANDATORY',APPLICABILITY:'APPLICABLE'},'REQ-C');req.scope={...scope};p.projectData.requirements.push(req);const proposition=record('propositions',4,{PROPOSITION_TEXT:'The current product satisfies the controlled proposition.'},'PROP-C');proposition.scope={...scope};proposition.relationships={REQUIREMENT_ID:'REQ-C'};p.projectData.propositions.push(proposition);const obligation=record('proofObligations',6,{OBLIGATION_ROLE:'RELEASE_PROPOSITION',REQUIREMENT_ID:'REQ-C',PROPOSITION_ID:'PROP-C',REQUIRED_EPISTEMIC_BASES:['APPLICATION_OBSERVED'],STATUS:'CURRENT'},'PROOF-C');obligation.scope={...scope};obligation.relationships={REQUIREMENT_ID:'REQ-C',PROPOSITION_ID:'PROP-C'};p.projectData.proofObligations.push(obligation);for(const id of ['EVIDENCE-APP-C','EVIDENCE-TRUE-C','EVIDENCE-FALSE-C']){const evidence=record('evidenceRecords',22,{KIND:'CURRENT_OBSERVATION',AUTHORITY_TYPE:'APPLICATION',DESCRIPTION:'Current exact fixture evidence.',CONTENT:'Application-controlled observation evidence.'},id);evidence.scope={...scope};p.projectData.evidenceRecords.push(evidence);}const applicability=record('applicabilityRecords',5,{APPLICABILITY:'APPLICABLE',APPLICABILITY_REASON:'The current fixture scope activates the proposition.',EPISTEMIC_BASIS:'APPLICATION_OBSERVED',FRESHNESS_STATUS:'CURRENT'},'APPLICABILITY-C');applicability.scope={...scope};applicability.relationships={REQUIREMENT_ID:'REQ-C',PROPOSITION_ID:'PROP-C',SEMANTIC_REVIEW_ID:'SEMANTIC-APP-C'};applicability.evidenceRefs=['EVIDENCE-APP-C'];p.projectData.applicabilityRecords.push(applicability);for(const [id,relation,evidenceId] of [['OBS-TRUE-C','ESTABLISHES','EVIDENCE-TRUE-C'],['OBS-FALSE-C','REFUTES','EVIDENCE-FALSE-C']]){const observation=record('observationRecords',22,{ORIGIN_CLASS:'NATIVE_APPLICATION_OBSERVATION',EPISTEMIC_BASIS:'APPLICATION_OBSERVED',FRESHNESS_STATUS:'CURRENT'},id);observation.scope={...scope};observation.relationships={PROPOSITION_ID:'PROP-C',EVIDENCE_ID:evidenceId};observation.evidenceRefs=[evidenceId];p.projectData.observationRecords.push(observation);const entailment=record('entailmentReviews',22,{ACCEPTED_RELATION:relation,REASONING:'The exact current observation addresses the same proposition.',STATUS:'ACCEPTED'},`ENTAILMENT-${id}`);entailment.scope={...scope};entailment.relationships={OBSERVATION_ID:id,PROPOSITION_ID:'PROP-C',PROOF_OBLIGATION_ID:'PROOF-C'};p.projectData.entailmentReviews.push(entailment);}const state=engine.evaluatePropositionState(p,{proposition});assert(state.truthValue==='UNKNOWN'&&state.contradictionStatus==='CONTRADICTED','Opposite sufficient observations of the same proposition were not detected as a contradiction.');
}
...
console.log(JSON.stringify({stage04CapturedInputReuse:true}));
{
  ...
  const product=record('products',21,{PRODUCT_VERSION:'PRODUCT-v001',GENERATED_ARTIFACT_INVENTORY:['ARTIFACT-ROUTE'],STATUS:'COMPLETED'},'PRODUCT-ROUTE');product.scope={...engine.currentScope(p),productId:'PRODUCT-ROUTE'};p.projectData.products.push(product);
  ...
  const test=record('tests',6,{REQ_ID:'REQ-ROUTE',TEST_TYPE:'DETERMINISTIC',EXECUTION_MODE:'APPLICATION_DETERMINISTIC',REQUIRED_CAPABILITY:'CLOSED_LOOP_TEST_IR',ARTIFACT_REQUIREMENTS:'NONE',EXECUTABLE_KIND:'TEST_IR',EXECUTABLE_SPEC_VERSION:'closed-loop-test-spec/1',EXECUTABLE_INPUT_BINDINGS:{PRODUCT:{source:'CURRENT_PRODUCT',filename:'product.json'}},EXECUTABLE_SPEC:{version:'closed-loop-test-spec/1',steps:[{op:'LOAD_ARTIFACT',binding:'PRODUCT'},{op:'READ_BYTES'},{op:'DECODE_UTF8'},{op:'COUNT'},{op:'ASSERT_EQ',value:2}]},INPUTS:'product',TOOLS:'runtime',PROCEDURE:'decode UTF-8 and count text characters',EXPECTED_RESULT:'2',FAILURE_CONDITION:'not 2',EVIDENCE_TO_PRESERVE:'application evidence',STATUS:'READY'},'TEST-ROUTE');test.scope=engine.currentScope(p);p.projectData.tests.push(test);
...
// Reliability measurement never promotes distinct local/external IDs into materially independent trials.
{
  ...
  metrics=engine.operationalMetrics(p);assert(metrics.materiallyIndependentAcceptedOperations===0,'Ten completed runs with unknown provider independence were falsely counted as materially independent accepted operations.');assert(metrics.zeroFailure95PercentUpperBound===null,'Rule-of-three reporting was not suppressed when provider independence remained unknown.');
}
