...
{
  const p=project('JOB-BAD-REL'),stage=3;for(const prerequisite of engine.prerequisiteStages(stage)){p.stages[prerequisite].status='COMPLETE';p.stages[prerequisite].gate={complete:true};} // Controlled prerequisites for the isolated relationship violation.
  p.stages[2].agentData.SOURCE_APPLICABILITY_DETERMINATION='NO_APPLICABLE_EXTERNAL_SOURCE';p.stages[2].status='COMPLETE';p.stages[2].gate={complete:true,blocked:false,reasons:[]};const pr=prompt(p,stage);
...
// The request control preserves accepted work until a replacement is confirmed.
// Its production-handler behavior is covered in verify-semantic-review-acceptance.mjs.
...
{
 const p=project('JOB-INDEPENDENCE-V2');const art=record('artifacts',10,{FILENAME:'candidate.bin',TYPE:'application/octet-stream',BYTE_SIZE:1,SHA256:'a'.repeat(64),STORAGE_REFERENCE:'indexeddb:ART-V2',AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED'},'ART-V2');p.projectData.artifacts.push(art);const v2Selection=engine.recordRegisteredHumanDecision(p,{stage:10,purpose:'CANDIDATE_COMPONENT_SELECTION',targetFamily:'artifacts',targetId:hash.sha256Value(['ART-V2']),value:['ART-V2'],operatorLabel:'VERIFY'}),frozen=engine.freezeCandidate(p,{stage:10,artifactIds:['ART-V2'],selectionDecisionId:engine.recordId(v2Selection,'humanDecisions'),operatorLabel:'VERIFY'}),iid=engine.recordId(frozen.iteration,'iterations'),cid=engine.recordId(frozen.candidate,'candidateFreezes'),slots=engine.reserveRunBatch(p,{stage:11,iterationId:iid,candidateId:cid,count:10});for(let i=0;i<slots.length;i++){const ctx=engine.records(p,'freshContexts').find(x=>engine.recordId(x,'freshContexts')===slots[i].contextId);ctx.fields.EXTERNAL_CONTEXT_IDENTIFIER='external-'+i;ctx.EXTERNAL_CONTEXT_IDENTIFIER='external-'+i;ctx.fields.CONTAMINATION_STATUS='NONE';ctx.CONTAMINATION_STATUS='NONE';ctx.fields.AUTHORIZED_PROJECT_INPUTS=[];ctx.AUTHORIZED_PROJECT_INPUTS=[];const run=engine.records(p,'runs').find(x=>engine.recordId(x,'runs')===slots[i].runId);run.fields.CONTAMINATION_CHECK='NONE';run.CONTAMINATION_CHECK='NONE';}
