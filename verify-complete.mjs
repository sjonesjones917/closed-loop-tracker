...
{
 const p=project('JOB-RUN-BATCH-IDEMPOTENT');const bytes=new TextEncoder().encode('idempotent-candidate');engine.registerArtifactBytes(p,{stage:17,artifactId:'ARTIFACT-IDEMPOTENT',filename:'idempotent.bin',mediaType:'application/octet-stream',byteSize:bytes.byteLength,sha256:'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'});const frozen=engine.freezeCandidate(p,{stage:17,artifactIds:['ARTIFACT-IDEMPOTENT'],operatorLabel:'VERIFY'}),iterationId=engine.recordId(frozen.iteration,'iterations'),candidateId=engine.recordId(frozen.candidate,'candidateFreezes');
 const first=engine.reserveRunBatch(p,{stage:17,iterationId,candidateId,count:10}),events=p.projectData.history.filter(x=>x.type==='RUN_BATCH_RESERVED').length;
 const second=engine.reserveRunBatch(p,{stage:17,iterationId,candidateId,count:10});
 ...
 engine.records(p,'runs',{stage:17})[0].active=false;let threw=false;try{engine.reserveRunBatch(p,{stage:17,iterationId,candidateId,count:10});}catch{threw=true;}assert(threw,'Partial active batch was silently topped up.');
}
