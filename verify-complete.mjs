...
{
  assert(JSON.stringify(engine.applicationTestCapabilities())===JSON.stringify(['CANONICAL_ARTIFACT_BYTE_IDENTITY']),'Application-native executor registry must contain only the exact byte-identity executor.');
...
// Reliability hardening: execution routing, independence, evidence sufficiency, contradictions, stability, and regression lifecycle.
{
 ...
 const artifact=record('artifacts',6,{FILENAME:'fixture.step',TYPE:'model/step',BYTE_SIZE:4,SHA256:'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',ROLE:'TEST_INPUT',STORAGE_REFERENCE:'indexeddb:ART-HARDEN',AVAILABILITY:'BYTES_PERSISTED_AND_VERIFIED'},'ART-HARDEN'),evidence=record('evidenceRecords',6,{ATTACHMENT_ID:'ART-HARDEN',KIND:'ARTIFACT',DESCRIPTION:'fixture',LOCATION:'indexeddb',CONTENT:'verified',STATUS:'PRESERVED'},'EVID-HARDEN');p.projectData.artifacts.push(artifact);p.projectData.evidenceRecords.push(evidence);p.projectData.tests[0].evidenceRefs=['EVID-HARDEN'];plan=engine.testExecutionPlan(p);assert(plan.items[0].operatorAction==='BLOCKED'&&!plan.items[0].executableNow&&String(plan.items[0].blockingReason).includes('positively establishes capability'),'Verified bytes incorrectly bypassed missing capability evidence.');const ctx=record('freshContexts',11,{EXTERNAL_CONTEXT_IDENTIFIER:'tool-hardening-context',ROLE:'TOOL_EXECUTOR',AUTHORIZED_PROJECT_INPUTS:'fixture.step',TOOL_AVAILABILITY:'CAD_IMPORT available',CONTAMINATION_STATUS:'NONE',EVIDENCE:'capability present',USABILITY_DETERMINATION:'USABLE'},'CONTEXT-CAP-HARDEN');ctx.scope={inputVersion:'INPUT-v001',sourceSetVersion:'SOURCE-SET-v001',requirementsVersion:'REQUIREMENTS-v001',testSuiteVersion:'TEST-SUITE-v001'};p.projectData.freshContexts.push(ctx);plan=engine.testExecutionPlan(p);assert(plan.items[0].operatorAction==='SEND_TO_TOOL_AGENT'&&plan.items[0].executableNow&&plan.items[0].requiredArtifactNames[0]==='fixture.step','Verified artifact plus positively established capability did not restore exact external-tool route.');const handoff=engine.artifactHandoff(p,{stage:12,testIds:['TEST-TOOL-HARDEN']});assert(handoff.send[0].artifactId==='ART-HARDEN'&&handoff.withhold.some(x=>x.artifactIdOrCategory==='OTHER_VERIFIER_RESULTS')&&handoff.expectBack.length,'Deterministic handoff manifest is incomplete.');
}
{
