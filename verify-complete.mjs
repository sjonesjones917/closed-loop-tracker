...
function prompt(p,stage){for(let prior=1;prior<stage;prior++){p.stages[prior].status='COMPLETE';p.stages[prior].gate={...(p.stages[prior].gate||{}),complete:true,blocked:false,reasons:[]};}const r={...prompts.buildPromptRecord(stage,p),generatedAt:new Date().toISOString()};p.projectData.generatedPrompts.push(r);return r;}

// Application-derived stages must not require a fabricated external-agent response.
{
