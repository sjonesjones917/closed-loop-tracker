...
if old not in s:
    raise SystemExit('refinement UI anchor not found')
p.write_text(s.replace(old,new,1))

p=Path('verify-complete.mjs')
s=p.read_text()
marker="assert(fs.readFileSync('app-core.js','utf8').includes('No accepted response matches the selected operation/run scope.'),'Refinement UI does not target the selected operation/run scope.');"
if marker not in s:
    anchor="// Targeted accepted-result refinement preserves independent same-stage operation scope.\n"
    if anchor not in s:
        raise SystemExit('targeted refinement test anchor not found')
