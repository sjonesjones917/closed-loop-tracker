...
if 'const procedures=stageSpecial;' in s:
    s=s.replace('const procedures=stageSpecial;',augmentation,1)
elif 'const procedureAugmentation=Object.freeze(' not in s:
    raise SystemExit('procedures marker missing')
p.write_text(s)

p=Path('index.html')
s=p.read_text()
s=re.sub(r'runtime-20260830-live-operator-\d+','runtime-20260830-live-operator-41',s)
p.write_text(s)
