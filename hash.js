...
// Temporary compatibility bridge consumed by the intake-accounting engine. The parser recognizes all established supplied-material inventory shapes without inferring semantic content from filenames.
globalThis.suppliedMaterialReferences=function suppliedMaterialReferences(project){
  ...
  const transferMode=(type,label)=>{const declared=String(type||'').trim().toUpperCase(),value=String(label||'').trim();if(/MESSAGE|INLINE|TEXT|NOTE|CHAT/.test(declared))return 'INLINE_JOB_INPUT';if(/URL|URI|LINK|WEB|REFERENCE/.test(declared)||/^https?:\/\//i.test(value))return 'AUTHORIZED_REFERENCE';if(/FILE|ATTACH|DOCUMENT|ARCHIVE|FOLDER|DIRECTORY|IMAGE|DRAWING|MODEL|SPREADSHEET|REPOSITORY|PACKAGE/.test(declared)||/\.[A-Za-z0-9]{1,12}(?:$|[?#])/.test(value))return 'ORIGINAL_ATTACHMENT';return 'ORIGINAL_MATERIAL';};
  const add=(label,type='SUPPLIED_PROJECT_INPUT',explicitMode=null)=>{const clean=String(label||'').trim();if(!clean||/^(?:UNKNOWN|NONE|NOT APPLICABLE|NULL)$/i.test(clean))return;const key=String(type||'')+'|'+clean.toLowerCase();if(seen.has(key))return;seen.add(key);references.push({label:clean,type:String(type||'SUPPLIED_PROJECT_INPUT').trim()||'SUPPLIED_PROJECT_INPUT',transferMode:String(explicitMode||transferMode(type,clean))});};
  const walk=(value,depth=0)=>{if(depth>5||value===null||value===undefined)return;if(Array.isArray(value)){for(const item of value)walk(item,depth+1);return;}if(typeof value==='string'){for(const part of value.split(/\r?\n|;/).map(x=>x.replace(/^\s*(?:[-*•]|\d+[.)])\s*/,'').trim()).filter(Boolean))add(part);return;}if(typeof value!=='object')return;const label=value.exactNameOrReference??value.filename??value.fileName??value.name??value.label??value.title??value.reference??value.path??value.url;const type=value.type??value.materialType??value.mediaType??value.kind??value.role??'SUPPLIED_PROJECT_INPUT';if(label!==undefined&&label!==null&&String(label).trim()){add(label,type,value.transferMode??value.mode??null);return;}for(const key of ['files','materials','items','attachments','references','suppliedMaterials','inventory'])if(Object.prototype.hasOwnProperty.call(value,key))walk(value[key],depth+1);};
  if(typeof raw==='string'){const clean=raw.trim();if(!clean||/^(?:UNKNOWN|NONE|NOT APPLICABLE|NULL|\[\]|\{\}|NONE SUPPLIED|NO MATERIALS?(?: SUPPLIED)?)$/i.test(clean))return [];try{walk(JSON.parse(clean));}catch{walk(clean);}}else walk(raw);
  return references;
};
