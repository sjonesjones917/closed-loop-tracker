...
const generated=[];
for(let stage=1;stage<=30;stage++){const p=blank(`JOB-PROMPT-${stage}`);if(stage===4)prepareStage4(p);const record=prompts.buildPromptRecord(stage,p,syntheticPromptOptions(stage,p));generated.push(record.prompt);for(const token of [`JOB_ID: ${p.job.JOB_ID}`,'PROJECT-SCOPE BOUNDARY','RESPONSE CONTRACT DEFINITIONS','closed-loop-stage-response/3'])if(!record.prompt.includes(token))throw new Error(`Stage ${stage} prompt missing ${token}.`);if(stage===2&&!record.prompt.includes('independent external sources'))throw new Error('Stage 02 non-circular authority rule missing.');if(stage===3&&(!record.prompt.includes('Every current Stage 02 source must receive current research coverage')||!record.prompt.includes('conflict-and-exception pass')))throw new Error('Stage 03 complete-source research and conflict/exception boundaries are missing.');}
