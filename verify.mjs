...
for(const token of ['function responsePromptRecord(n,text)','ingestion.strictParse(text)','readStagedResponseFile','AUTHORITATIVE_RESPONSE_FILE'])if(!appSourceForStatus.includes(token))throw new Error(`Returned-instruction file-binding regression missing ${token}.`);
