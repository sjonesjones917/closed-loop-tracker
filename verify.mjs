...
if((project.runRecords||[]).length<20||(project.runRecords||[]).length%10!==0)throw new Error('Complete ten-run sets must be preserved.');
const mandatory=(project.requirements||[]).filter(r=>r.mandatory!==false).length;
if((project.verificationRecords||[]).length<mandatory*20)throw new Error('Run-by-requirement verification evidence does not cover the required run sets.');
