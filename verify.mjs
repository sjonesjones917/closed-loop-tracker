...
for(let stage=1;stage<=30;stage++){
  const p=blank(`JOB-PROMPT-${stage}`),record=prompts.buildPromptRecord(stage,p,syntheticPromptOptions(stage,p));generated.push(record.prompt);
  for(const token of [`JOB_ID: ${p.job.JOB_ID}`,'PROJECT-SCOPE BOUNDARY','STRICT RESPONSE CONTRACT','closed-loop-stage-response/3','PROMPT IDENTITY — ECHO EXACTLY'])if(!record.prompt.includes(token))throw new Error(`Stage ${stage} prompt missing ${token}.`);
  if(stage===1){for(const token of ['preserve every materially relevant fact, requirement, constraint, decision, prohibition, requested output, acceptance condition, material reference, and unresolved human-only issue','original intent file must not be repeatedly requested'])if(!record.prompt.includes(token))throw new Error(`Stage 01 complete-intent semantics missing: ${token}.`);}
  if(stage===2&&!record.prompt.includes('independent external sources'))throw new Error('Stage 02 non-circular authority rule missing.');
  if(stage===3){for(const token of ['Exhaust the complete current accepted Stage 02 independent external source set','carry forward every requirement-relevant human-origin obligation already captured and identity-bound by Stage 01','Never ask the human to repeat or reattach information already captured in Stage 01'])if(!record.prompt.includes(token))throw new Error(`Stage 03 exhaustive Stage 01 + Stage 02 semantics missing: ${token}.`);}
  if(stage===4){for(const token of ['APPLICATION OBLIGATION MANIFEST','Do not rediscover an unspecified input universe','do not ask the human to attach the original intent file again','No obligation may disappear'])if(!record.prompt.includes(token))throw new Error(`Stage 04 closed obligation semantics missing: ${token}.`);}
}
