...
function prepareVerifierPromptRelation(p){
 const requirementId='REQ-PROMPT',propositionId='PROPOSITION-PROMPT',testId='TEST-PROMPT',iterationId='ITERATION-000001',candidateId='CANDIDATE-000001',runId='RUN-000001',contextId='CONTEXT-000001';
 const requirementScope={inputVersion:p.job.CURRENT_INPUT_VERSION,sourceSetVersion:'SOURCE-SET-v001',requirementsVersion:'REQUIREMENTS-v001'},testScope={...requirementScope,testSuiteVersion:'TEST-SUITE-v001'},iterationScope={...testScope,instructionVersion:'INSTRUCTION-v001',iterationId,candidateId};
 ...
 const relation=engine.requiredVerificationRelationSet(p,iterationId);if(relation.tuples.length!==1)throw new Error('Verifier prompt fixture must have exactly one current required relation.');
 return p;
}
...
const sourceBackupRun=spawnSync(process.execPath,['verify-source-backup-closure.mjs'],{encoding:'utf8'});if(sourceBackupRun.status!==0)throw new Error(`verify-source-backup-closure.mjs failed:\n${sourceBackupRun.stdout}\n${sourceBackupRun.stderr}`);
const securityClosureRun=spawnSync(process.execPath,['verify-security-closure.mjs'],{encoding:'utf8'});if(securityClosureRun.status!==0)throw new Error(`verify-security-closure.mjs failed:\n${securityClosureRun.stdout}\n${securityClosureRun.stderr}`);
