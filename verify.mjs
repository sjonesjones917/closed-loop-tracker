...
const files=['index.html','app-core.js','hash.js','workflow-schema.js','test-runtime.js','workflow-engine.js','prompt-engine.js','response-ingestion.js','project-store.js','workbook.js','TEST_PROJECT.json','AUTHORIZED_OPERATION_01.txt'];
...
const generated=[];
for(let stage=1;stage<=30;stage++){const p=blank(`JOB-PROMPT-${stage}`);const record=prompts.buildPromptRecord(stage,p,syntheticPromptOptions(stage,p));generated.push(record.prompt);for(const token of [`JOB_ID: ${p.job.JOB_ID}`,'PROJECT-SCOPE BOUNDARY','STRICT RESPONSE CONTRACT','closed-loop-stage-response/3','PROMPT IDENTITY — ECHO EXACTLY'])if(!record.prompt.includes(token))throw new Error(`Stage ${stage} prompt missing ${token}.`);if(stage===2&&!record.prompt.includes('independent external sources'))throw new Error('Stage 02 non-circular authority rule missing.');if(stage===3&&!(record.prompt.includes('Stage 02 source set')&&record.prompt.includes('canonical Stage 01 intent ledger')&&record.prompt.includes('never reopen or request the original Stage 01 intent file')))throw new Error('Stage 03 canonical source-and-intent research boundary missing.');}
...
assert(schema.RESPONSE_SCHEMA==='closed-loop-stage-response/3','Response schema /2 is required.');
