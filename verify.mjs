...
const operation01=retained.generatedOutputs?.[0]?.output;
if(typeof operation01!=='string'||!operation01.trim()||retained.stageRecords?.['1']?.output!==operation01)throw new Error('Authorized Operation 01 output was not preserved exactly in canonical project history.');
...
const verifierContexts=new WeakMap();let verifierContextSequence=0;
function blank(jobId,promptStage){const p=core.createBlankState(jobId);p.job.JOB_ID=jobId;p.job.JOB_TITLE='Verification project';p.job.EXACT_USER_OBJECTIVE_VERBATIM='Controlled verification objective';p.job.CURRENT_INPUT_VERSION='INPUT-v001';p.job.CURRENT_SOURCE_SET_VERSION='SOURCE-SET-v001';p.job.CURRENT_REQUIREMENTS_VERSION='REQUIREMENTS-v001';p.job.CURRENT_TEST_SUITE_VERSION='TEST-SUITE-v001';p.job.CURRENT_INSTRUCTION_VERSION='INSTRUCTION-v001';engine.ensureShape(p);engine.recalculate(p);const independentReviewer=[9,12,23,24].includes(Number(promptStage)),context=engine.registerFreshContext(p,{stage:promptStage,externalContextIdentifier:`VERIFY-MAIN-${promptStage}-${++verifierContextSequence}`,operatorLabel:'VERIFY_MAIN',purpose:independentReviewer?'REVIEWER':'GENERAL'});verifierContexts.set(p,engine.recordId(context,'freshContexts'));for(let stage=1;stage<=30;stage++){p.stages[stage].status='COMPLETE';p.stages[stage].gate={complete:true,blocked:false,reasons:[]};}return p;}
function prepareStage4(p){const intake=engine.intakeCoverageManifest(p);p.stages[1].agentData.INPUT_SET_CONTENTS=JSON.stringify({schema:'closed-loop-stage01-capture/1',inputVersion:intake.inputVersion,manifestSha256:intake.manifestSha256,units:intake.units.map((u,i)=>({sourceUnitId:u.unitId,sourceRawValueSha256:u.rawValueSha256,disposition:'incorporated into the job definition',reason:'Preserved for downstream reuse.',extractedStatements:[{statementKey:'S'+String(i+1),text:u.rawValueText||('Captured '+u.label),statementClass:'FACT'}]}))});p.stages[1].status='COMPLETE';p.stages[1].gate={complete:true,blocked:false,reasons:[]};p.stages[2].status='COMPLETE';p.stages[2].gate={complete:true,blocked:false,reasons:[]};p.stages[2].agentData.SOURCE_APPLICABILITY_DETERMINATION='APPLICABLE_SOURCES_ESTABLISHED';p.projectData.sources.push({id:'SOURCE-VERIFY-MAIN',active:true,stage:2,scope:{inputVersion:'INPUT-v001',sourceSetVersion:'SOURCE-SET-v001'},fields:{SOURCE_ID:'SOURCE-VERIFY-MAIN',TITLE:'Independent verification source'}});p.projectData.research.push({id:'RESEARCH-VERIFY-MAIN',active:true,stage:3,scope:{inputVersion:'INPUT-v001',sourceSetVersion:'SOURCE-SET-v001'},fields:{RESEARCH_ID:'RESEARCH-VERIFY-MAIN',SOURCE_ID:'SOURCE-VERIFY-MAIN',PASS_NUMBER:2,EXACT_PORTION_EXAMINED:'Complete fixture source',FINDING_CLASSIFICATION:'COMPLETE',SOURCE_EVIDENCE:'Fixture evidence',SATURATION_STATUS:'SATURATED'},relationships:{SOURCE_ID:'SOURCE-VERIFY-MAIN'}});p.stages[3].agentData={ALL_KNOWN_CONTROLLING_SOURCES_EXAMINED:'TRUE',SECOND_CONFLICT_AND_EXCEPTION_PASS_COMPLETED:'TRUE',LATEST_PASS_NUMBER:2,NEW_MATERIAL_CATEGORY_FOUND_IN_LATEST_PASS:'FALSE'};p.stages[3].status='COMPLETE';p.stages[3].gate={complete:true,blocked:false,reasons:[]};return p;}
function syntheticPromptOptions(stage,p){const operation=schema.STAGE_CONTRACTS[stage].operations[0],scope={};for(const key of schema.operationContract(stage,operation).scopeRequirements){if(key==='projectRevision')scope[key]=Number(p.revision||0)+1;else if(key==='inputVersion')scope[key]=p.job.CURRENT_INPUT_VERSION;else if(key==='sourceSetVersion')scope[key]='SOURCE-SET-v001';else if(key==='requirementsVersion')scope[key]='REQUIREMENTS-v001';else if(key==='testSuiteVersion')scope[key]='TEST-SUITE-v001';else if(key==='instructionVersion')scope[key]='INSTRUCTION-v001';else if(key==='iterationId')scope[key]='ITERATION-000001';else if(key==='candidateId')scope[key]='CANDIDATE-000001';else if(key==='runId')scope[key]='RUN-000001';else if(key==='contextId')scope[key]=verifierContexts.get(p);else if(key==='baselineId')scope[key]='BASELINE-000001';else if(key==='productId')scope[key]='PRODUCT-000001';}return {operation,scope};}
function buildReservedPrompt(stage,p){const options=syntheticPromptOptions(stage,p),contextId=verifierContexts.get(p),scope=prompts.scopeFor(stage,{...p,revision:Number(p.revision||0)+1},{...options.scope,contextId}),prepared=engine.prepareCurrentOperationReservation(p,{stage,operation:options.operation,contextId,scope,owningTabInstance:'VERIFY_MAIN'}),preview=engine.clone(p);preview.revision=prepared.expectedRevision;const record=prompts.buildPromptRecord(stage,preview,{operation:options.operation,scope:prepared.scope,operationReservation:prepared});engine.registerGeneratedPrompt(p,record);engine.reserveOperation(p,{preparedReservation:prepared,promptId:record.instructionId});p.revision=prepared.expectedRevision;return record;}
const generated=[];
for(let stage=1;stage<=30;stage++){const p=blank(`JOB-PROMPT-${stage}`,stage);if(stage===4)prepareStage4(p);const record=buildReservedPrompt(stage,p);generated.push(record.prompt);for(const token of [`JOB_ID: ${p.job.JOB_ID}`,'PROJECT-SCOPE BOUNDARY','STRICT RESPONSE CONTRACT','closed-loop-stage-response/3','PROMPT IDENTITY — ECHO EXACTLY'])if(!record.prompt.includes(token))throw new Error(`Stage ${stage} prompt missing ${token}.`);if(stage===2&&!record.prompt.includes('independent external sources'))throw new Error('Stage 02 non-circular authority rule missing.');if(stage===3&&(!record.prompt.includes('Every current Stage 02 source must receive current research coverage')||!record.prompt.includes('conflict-and-exception pass')))throw new Error('Stage 03 complete-source research and conflict/exception boundaries are missing.');}
if(new Set(generated).size!==30)throw new Error('Prompts are not stage-specific.');
const pa=buildReservedPrompt(2,blank('JOB-A',2)).prompt,pb=buildReservedPrompt(2,blank('JOB-B',2)).prompt;if(pa.includes('JOB-B')||pb.includes('JOB-A'))throw new Error('Cross-project prompt contamination detected.');
