...
const operation01=String(retained.generatedOutputs?.[0]?.output||'');
...
const generated=[];
for(let stage=1;stage<=30;stage++){const p=blank(`JOB-PROMPT-${stage}`);if(stage===4)prepareStage4(p);else for(let prior=1;prior<stage;prior++){p.stages[prior].status='COMPLETE';p.stages[prior].gate={complete:true,blocked:false,reasons:[]};}const record=prompts.buildPromptRecord(stage,p,syntheticPromptOptions(stage,p));generated.push(record.prompt);for(const token of [`JOB_ID: ${p.job.JOB_ID}`,'PROJECT-SCOPE BOUNDARY','STRICT RESPONSE CONTRACT','closed-loop-stage-response/3','PROMPT IDENTITY — ECHO EXACTLY'])if(!record.prompt.includes(token))throw new Error(`Stage ${stage} prompt missing ${token}.`);if(stage===2&&!record.prompt.includes('independent external sources'))throw new Error('Stage 02 non-circular authority rule missing.');}
if(new Set(generated).size!==30)throw new Error('Prompts are not stage-specific.');
const pA=blank('JOB-A'),pB=blank('JOB-B');for(const fixture of [pA,pB]){fixture.stages[1].status='COMPLETE';fixture.stages[1].gate={complete:true,blocked:false,reasons:[]};}const pa=prompts.buildPromptRecord(2,pA).prompt,pb=prompts.buildPromptRecord(2,pB).prompt;if(pa.includes('JOB-B')||pb.includes('JOB-A'))throw new Error('Cross-project prompt contamination detected.');
