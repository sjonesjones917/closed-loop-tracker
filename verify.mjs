...
if(retained.generatedOutputs?.[0]?.output!==retained.stageRecords?.['1']?.output)throw new Error('Retained Stage 01 output history is internally inconsistent.');
...
function blank(jobId,targetStage=1){const p=core.createBlankState(jobId);p.job.JOB_ID=jobId;p.job.JOB_TITLE='Verification project';p.job.EXACT_USER_OBJECTIVE_VERBATIM='Controlled verification objective';p.job.CURRENT_INPUT_VERSION='INPUT-v001';engine.ensureShape(p);if(targetStage>1){const prior=targetStage-1;p.stages[prior].status='COMPLETE';p.stages[prior].gate={complete:true,blocked:false,reasons:[]};}engine.recalculate(p);return p;}
...
const generated=[];
for(let stage=1;stage<=30;stage++){const p=blank(`JOB-PROMPT-${stage}`,stage);if(stage===4)prepareStage4(p);const record=prompts.buildPromptRecord(stage,p,syntheticPromptOptions(stage,p));generated.push(record.prompt);for(const token of [`JOB_ID: ${p.job.JOB_ID}`,'PROJECT-SCOPE BOUNDARY','STRICT RESPONSE CONTRACT','closed-loop-stage-response/3','PROMPT IDENTITY — ECHO EXACTLY'])if(!record.prompt.includes(token))throw new Error(`Stage ${stage} prompt missing ${token}.`);if(stage===2&&!record.prompt.includes('independent external sources'))throw new Error('Stage 02 non-circular authority rule missing.');if(stage===3&&!record.prompt.includes('Research only the complete current accepted Stage 02 independent external source set'))throw new Error('Stage 03 external-source research boundary missing.');}
if(new Set(generated).size!==30)throw new Error('Prompts are not stage-specific.');
const pa=prompts.buildPromptRecord(2,blank('JOB-A',2)).prompt,pb=prompts.buildPromptRecord(2,blank('JOB-B',2)).prompt;if(pa.includes('JOB-B')||pb.includes('JOB-A'))throw new Error('Cross-project prompt contamination detected.');
