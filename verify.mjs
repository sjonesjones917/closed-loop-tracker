...
if(!project.stageRecords['2'].record.includes('STAGE 02 — BUILD THE SOURCE INVENTORY')||!project.stageRecords['2'].record.includes('SOURCE_SET_VERSION')||!project.stageRecords['2'].record.includes('AUTHORITY_HIERARCHY'))throw new Error('Stage 02 is not initialized as a usable source-inventory record.');
if(!project.stageRecords['30'].record.includes('DEFECT_REGISTRY_VERSION')||!project.stageRecords['30'].record.includes('REGRESSION_REGISTRY_VERSION'))throw new Error('Stage 30 is not initialized as a usable permanent-registry record.');
...
if((project.generatedOutputs||[]).length!==1||project.generatedOutputs[0]?.output!==operation01)throw new Error('The complete authorized Operation 01 output is not preserved consistently.');
for(const marker of ['OPERATION 01 — DEFINE JOB','EXACT USER OBJECTIVE VERBATIM','UNRESOLVED UNKNOWNS','EXACT DECISIONS','OPERATION 01 COMPLETION EVIDENCE','Proceed to Operation 02 — Build the Source Inventory.'])if(!operation01.includes(marker))throw new Error(`Operation 01 output is missing ${marker}.`);
...
for(const token of ['validateStageDraft','saveAppendix','sha256Bytes','compareArtifactSets','Complete project record','Generated instructions','Generated outputs','Output receipts','Original project input','Supporting records for this stage','currentStagePrompt','invalidateDownstream','stageRecordText','stageOutputText','isObsoleteRetainedProject','requestedView=params.get'])if(!app.includes(token))throw new Error(`Application control missing: ${token}`);
...
if(!app.includes("projects=readStored().filter(Boolean).map(normalize).filter(project=>!isObsoleteRetainedProject(project))"))throw new Error('Obsolete retained projects are not removed from browser storage.');
if(!app.includes("projects=projects.filter(p=>!(p.isRetainedTestProject||p.job.JOB_ID===test.job.JOB_ID))"))throw new Error('The authorized retained project is not deduplicated.');
if(!app.includes("if(!req.length||!tests.length)return 'BLOCKED';"))throw new Error('Release can be accepted without requirements and tests.');
...
if(!html.includes('max-height:38px'))throw new Error('Compact phone button sizing is missing.');
...
if(/GEN-042|field status report|maintenance[- ]handoff/i.test(app+html+core))throw new Error('Unrelated demonstration terminology remains in application source.');
console.log(JSON.stringify({application:'single',stages:30,testProject:project.title,jobId:project.jobId,currentStage:project.currentStage,state:project.currentState,generatedInstructions:project.generatedPrompts.length,generatedOutputs:project.generatedOutputs.length,outputReceipts:project.outputReceipts.length,readableStageRecords:30,deduplicatedRetainedProject:true,phoneViewsAddressable:true},null,2));
