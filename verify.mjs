...
for(let stage=1;stage<=30;stage++){
 ...
 for(const operation of schema.STAGE_CONTRACTS[stage].operations){
  ...
  const record=prompts.buildPromptRecord(stage,p,options);generated.push(record.prompt);externalPromptsVerified++;
  for(const token of [`JOB_ID: ${p.job.JOB_ID}`,'PROJECT-SCOPE BOUNDARY','STRICT RESPONSE CONTRACT','closed-loop-stage-response/3','PROMPT IDENTITY — ECHO EXACTLY'])if(!record.prompt.includes(token))throw new Error(`Stage ${stage}/${operation} prompt missing ${token}.`);
  if(stage===2&&operation==='COMPLETE'){
   for(const semantic of [/external source category/i,/supplied project material is project input, not automatically independent authority/i,/never invent sources/i,/authority role/i])if(!semantic.test(record.prompt))throw new Error(`Stage 02 COMPLETE prompt is missing non-circular source-authority semantics: ${semantic}.`);
  }
  if(stage===3&&operation==='COMPLETE'){
   for(const semantic of [/every current accepted Stage 02 source has current research coverage/i,/conflict\/exception\/saturation pass/i,/latest complete pass finds no new material/i])if(!semantic.test(record.prompt))throw new Error(`Stage 03 COMPLETE prompt is missing complete-source research/saturation semantics: ${semantic}.`);
  }
 }
}
