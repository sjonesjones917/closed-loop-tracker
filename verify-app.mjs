...
if (/Closed-Loop Agent Reliability\s+v\d/i.test(html)) fail('An arbitrary public application number remains.');
...
for (const forbidden of [
  ...
  'schemaVersion:13',
  'Reload verified self-project export'
]) {
  if (html.toLowerCase().includes(forbidden.toLowerCase())) fail(`Forbidden repair, application-number, or prompt-relay implementation remains: ${forbidden}`);
}
...
for (let index = 0; index < requiredStages.length; index += 1) {
  if (manifest[index]?.number !== index + 1) fail(`Stage ${index + 1} is renumbered.`);
  if (manifest[index]?.name !== requiredStages[index]) fail(`Stage ${index + 1} name changed: ${manifest[index]?.name}`);
}
...
const requiredArchitectureTokens = [
  ...
  'assignedActorType',
  'assignedActorName',
  'data-self-project-proof="true"',
  'data-self-project-status',
...
if (!html.includes('This is a normal completed project about the entire Closed-Loop Agent Reliability application.')) fail('The retained project is not presented as a normal complete-application project.');
...
const result = {
  ...
  retainedApplicationProject: true,
  retainedProjectNormalProject: true,
