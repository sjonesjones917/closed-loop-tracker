...
s=p.read_text()
marker='/* OPERATION REGISTRY COMPATIBILITY AUTHORITY /2 */'
if marker not in s:
    s += r'''
/* OPERATION REGISTRY COMPATIBILITY AUTHORITY /2 */
;(()=>{
'use strict';
...
const registry=Object.freeze(Object.fromEntries(Object.entries(b.STAGE_OPERATION_REGISTRY).map(([key,c])=>{
  ...
  return [key,Object.freeze({...c,writableFamilies:writable,agentWritableCollections:writable,allowedStageData,scopeRequirements})];
})));
