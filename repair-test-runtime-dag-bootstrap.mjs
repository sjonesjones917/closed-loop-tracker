...
const regexCompileBoundary="  try{if(!issues.length)new RegExp(text,flagText);}catch(error)";
if(!owner.includes(regexCompileBoundary))throw new Error('Expected regex compile boundary was not found after materialization.');
const nestedGuard="  if(/\\([^()]*[+*][^()]*\\)\\s*(?:[+*]|\\{)/.test(text))issues.push('Regex nested unbounded quantification is outside the registered safe subset.');\n";
owner=owner.replace(regexCompileBoundary,nestedGuard+regexCompileBoundary);
