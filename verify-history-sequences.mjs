...
for(const sequence of sequences()){
 ...
 try{
  ...
  for(const [index,operation] of sequence.entries()){
   ...
   if(operation==='SAVE_DRAFT'){const id=await store.saveCheckpoint(jobId,{expectedProjectRevision:p.revision,view:{activeStage:selected,activeView:'Workflow',scrollX:0,scrollY:sequenceNumber,drafts:{'#stage-output':{value:'Unaccepted draft '+sequenceNumber}}}});known.set(id,copy(p));assert.deepEqual(await store.readProject(jobId),p,'Draft capture changed accepted project data');}
