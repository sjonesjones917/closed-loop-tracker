...
import {responseFixture,OBJECTIVE,OUTPUT} from './operator-journey-fixtures.mjs';
...
try{
  ...
  for(stage=1;stage<=30;stage++){
    ...
    for(let steps=0;steps<80;steps++){
      ...
      if(controls[action.actionType]){await browser.click(controls[action.actionType]);report.operations.push({stage,command:action.actionType});}
