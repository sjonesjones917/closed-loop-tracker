...
function responseContractDescriptor(stage,operation){const contract=schema.STAGE_CONTRACTS[stage],op=schema.operationContract(stage,operation);if(!op)throw new Error(`Operation ${operation} is not valid for Stage ${stage}.`);return {schema:schema.RESPONSE_SCHEMA,stage,operation,responseRequired:op.responseRequired,responseTypes:op.responseRequired?schema.RESPONSE_TYPES:[],scopeRequirements:op.scopeRequirements,agentStageFields:op.allowedStageData,agentWritableCollections:op.agentWritableCollections,resourceLimits:contract.resourceLimits};}
function responseContract(stage,operation,instructionId,bodySha256,contractSha256,contextSignature,scope){
 const contract=schema.STAGE_CONTRACTS[stage],op=schema.operationContract(stage,operation);if(!op)throw new Error(`Operation ${operation} is not valid for Stage ${stage}.`);
 if(!op.responseRequired)return JSON.stringify({schema:schema.RESPONSE_SCHEMA,jobId:'<exact current JOB_ID>',stage,operation,promptIdentity:{instructionId,bodySha256,contractSha256,contextSignature},scope,responseRequired:false,instruction:'APPLICATION_CONTROLLED_OPERATION: do not submit an external-agent response. Complete this operation through validated application or human-authority controls.'},null,2);
 const recordShape=Object.fromEntries(op.agentWritableCollections.map(collection=>{const definition=schema.RECORD_SCHEMAS[collection],reserved=definition.commitPolicy===schema.COLLECTION_POLICIES.UPDATE_RESERVED;return [collection,[{tempKey:reserved?null:'response-local-key',targetId:reserved?'<application-reserved canonical ID>':null,fields:Object.fromEntries(schema.recordAgentFields(collection).map(name=>[name,'<value>'])),relationships:{},evidenceRefs:['evidence-1']}]];}));
 return JSON.stringify({schema:schema.RESPONSE_SCHEMA,jobId:'<exact current JOB_ID>',stage,operation,promptIdentity:{instructionId,bodySha256,contractSha256,contextSignature},scope,responseType:'DATA_PROPOSAL',humanInputRequests:[],stageData:Object.fromEntries(op.allowedStageData.map(name=>[name,'<value>'])),records:recordShape,evidence:[{temporaryKey:'evidence-1',kind:'WORKFLOW_EVIDENCE',description:'Exact evidence supporting proposed values',location:'<source/output location>',content:'<exact evidence or faithful excerpt>'}],unresolved:[],warnings:[],attachments:[]},null,2);
}
function body(stage,state,operation){
 const d=core.STAGES[stage-1],j=state?.job||{};
 const contract=schema.STAGE_CONTRACTS[stage],op=schema.operationContract(stage,operation);if(!op)throw new Error(`Operation ${operation} is not valid for Stage ${stage}.`);
 const fields=op.allowedStageData.length?op.allowedStageData.map(x=>`- ${x}`).join('\n'):'- No agent-owned stageData fields; use permitted records/evidence only.';
 ...
 const responseRules=op.responseRequired?`MANDATORY RESPONSE RULES
...
- If required authority, evidence, capability, or decision rule is unavailable, return BLOCKED or HUMAN_INPUT_REQUIRED as applicable rather than inventing data.`:`APPLICATION-CONTROLLED OPERATION
- No external-agent response is required or accepted for this operation.
- Complete the operation only through the application-owned and human-authority controls declared by the workflow schema.
- Do not fabricate an empty DATA_PROPOSAL merely to advance the stage.`;
