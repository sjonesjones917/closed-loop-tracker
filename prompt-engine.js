...
15:'Convert every confirmed failure in this job into permanent regression data. Link REG_ID to DEFECT_ID and REQ_ID; preserve failure fixture and identity/hash when available, reproduction procedure, detection method, and an actual pre-correction execution that demonstrates failure with evidence. Define the expected post-correction result, permanent test location, applicability, active/retired state, and retirement authority where applicable. Do not claim post-correction success at Stage 15; that success must come from an actual later corrected execution.',
...
const recordValue=(record,name)=>record?.fields?.[name]??record?.[name];
function contextRecords(state,collection,scope={}){
 const list=Array.isArray(state?.projectData?.[collection])?state.projectData[collection]:[];
 const scopeKeys=['inputVersion','sourceSetVersion','requirementsVersion','testSuiteVersion','instructionVersion','iterationId','candidateId','baselineId','productId'];
 return list.filter(record=>{if(record?.active===false||record?.invalidatedBy)return false;const rs=record?.scope||{};for(const key of scopeKeys)if(rs[key]!=null&&scope[key]!=null&&String(rs[key])!==String(scope[key]))return false;if(scope.runId){const value=String(recordId(record,collection)||''),declared=String(recordValue(record,'RUN_ID')||rs.runId||'');if(collection==='runs'&&value!==String(scope.runId)&&declared!==String(scope.runId))return false;if(rs.runId!=null&&String(rs.runId)!==String(scope.runId))return false;}if(scope.contextId){const value=String(recordId(record,collection)||''),declared=String(recordValue(record,'CONTEXT_ID')||rs.contextId||'');if(collection==='freshContexts'&&value!==String(scope.contextId)&&declared!==String(scope.contextId))return false;if(rs.contextId!=null&&String(rs.contextId)!==String(scope.contextId))return false;}return true;});
}
function boundedCollection(state,collection,scope={}){
 const active=contextRecords(state,collection,scope);if(!active.length)return 'NONE';
 return show({totalActive:active.length,records:active.map(record=>({id:recordId(record,collection),stage:record.stage??'UNKNOWN',scope:record.scope||{},fields:record.fields||record,relationships:record.relationships||{},contentSha256:record.contentSha256||record.sha256||'UNKNOWN'})),omitted:0,selectionRule:'All and only active records selected by the exact operation readCollections contract and controlling project scope; run/context-scoped instructions include only their selected run/context records. Large artifact bytes are referenced by canonical artifact identity.'});
}
function contextFor(stage,state,operation,scope={}){
 ...
 const failed=(state?.projectData?.responseValidations||[]).filter(x=>Number(x.stage)===stage&&!x.valid).at(-1);
 if(failed)parts.push(`LATEST APPLICATION VALIDATION FAILURE TO CORRECT\n${show({validationId:failed.validationId,issues:(failed.issues||[]).map(x=>({code:x.code,path:x.path,message:x.message}))})}`);
...
- Stage 02 may contain only genuinely independent external sources appropriate to the job: governing/controlling authority where it exists, otherwise reputable direct evidence. Target-product and repository artifacts are implementation evidence, not independent external authority.
...
function buildPromptRecord(stageOrDefinition,state,options={}){
 ...
 const opContract=schema.operationContract(stage,operation);const scope=scopeFor(stage,state,options.scope||{}),contextManifest={stage,operation,scope,readCollections:Object.fromEntries((opContract?.readCollections||schema.STAGE_CONTRACTS[stage].readCollections||[]).map(collection=>[collection,contextRecords(state,collection,scope).map(record=>({id:recordId(record,collection),scope:record.scope||{},contentSha256:record.contentSha256||record.sha256||hash.sha256Value(record.fields||record)}))])),answeredHumanClarifications:(state?.projectData?.humanInputAnswers||[]).filter(x=>Number(x.stage)===stage).map(x=>({requestId:x.requestId,answerId:x.answerId,inputVersion:x.inputVersion||state?.job?.CURRENT_INPUT_VERSION||null})),operatorCorrectionRequests:(state?.projectData?.rejectedResponses||[]).filter(x=>Number(x.stage)===stage&&x.requestCorrection&&!x.invalidatedBy).map(x=>({rejectedResponseId:x.rejectedResponseId,reason:x.reason,rawResponseId:x.rawResponseId})),latestValidationFailure:(state?.projectData?.responseValidations||[]).filter(x=>Number(x.stage)===stage&&!x.valid).slice(-1).map(x=>({validationId:x.validationId,issues:(x.issues||[]).map(i=>({code:i.code,path:i.path,message:i.message}))}))};
