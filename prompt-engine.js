...
function contextFor(stage,state,operation){
 ...
 const corrections=(state?.projectData?.rejectedResponses||[]).filter(x=>Number(x.stage)===stage&&x.requestCorrection===true&&!x.invalidatedBy&&(!acceptedAt||String(x.rejectedAt||'')>acceptedAt));
 if(corrections.length)parts.push(`OPERATOR CORRECTION OR REFINEMENT REQUESTS SINCE THE LAST ACCEPTED STAGE RESPONSE\n${show(corrections.map(x=>({rejectedResponseId:x.rejectedResponseId,reason:x.reason,operatorLabel:x.operator||'HUMAN_OPERATOR',rejectedAt:x.rejectedAt})))}`);
...
const idOrNull=value=>{const text=String(value??'').trim();return !text||['NONE','NOT APPLICABLE','UNKNOWN','PENDING'].includes(text.toUpperCase())?null:text;};
function scopeFor(stage,state,overrides={}){const j=state?.job||{};return {projectRevision:Number(state?.revision||0),inputVersion:j.CURRENT_INPUT_VERSION||null,sourceSetVersion:idOrNull(j.CURRENT_SOURCE_SET_VERSION),requirementsVersion:idOrNull(j.CURRENT_REQUIREMENTS_VERSION),testSuiteVersion:idOrNull(j.CURRENT_TEST_SUITE_VERSION),instructionVersion:idOrNull(j.CURRENT_INSTRUCTION_VERSION),iterationId:idOrNull(j.CURRENT_ITERATION),candidateId:idOrNull(state?.projectData?.candidateFreezes?.filter(x=>x?.active!==false&&!x?.invalidatedBy).at(-1)?.id),runId:idOrNull(overrides.runId),contextId:idOrNull(overrides.contextId),baselineId:idOrNull(j.CURRENT_BASELINE_ID),productId:idOrNull(j.CURRENT_PRODUCT_ID)};}
...
function body(stage,state,operation){
 const d=core.STAGES[stage-1],j=state?.job||{},contract=schema.STAGE_CONTRACTS[stage],op=schema.operationContract(stage,operation||contract.operations[0]);
...
- If a prior response was rejected or application validation failed but the available context is sufficient to correct it, return a complete replacement response addressing the supplied correction or validation feedback without restarting the project.
