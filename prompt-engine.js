...
12:'Verify each of this job’s ten executions independently. Create exactly one applicable verification record for every required REQ_ID × RUN_ID × TEST_ID triple. Do not collapse multiple tests for the same requirement/run into one record. Preserve verifier and verifier-context identity, independence status, inputs, procedure, expected result, observed result, exact evidence, SATISFIED/VIOLATED/UNDETERMINED, DEFECT_ID where applicable, and reason for UNDETERMINED. Reconcile counts mathematically and do not let the generator act as its sole validator.',
...
- Before producing DATA_PROPOSAL, explicitly determine whether the combination of current human input, authorized application context, and accepted prior-stage output is sufficient to perform this stage reliably. Do not return a partial DATA_PROPOSAL merely to fill fields.
- If required information can only be supplied or decided by the human, return HUMAN_INPUT_REQUIRED with the smallest exact blocking questions. If required application context, prior canonical evidence, artifact identity, authority, or capability is missing, return BLOCKED with the exact missing dependency. If an attempted execution/tool failed, return EXECUTION_FAILED. If prior agent output is inadequate or contradictory, identify the exact deficiency and require correction rather than assuming it away.
