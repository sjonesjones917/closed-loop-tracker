...
async function uploadValid(){const files=await stageFiles(),response=responseFixture(files);await browser.selectFiles('#response-json-file',[{filename:'response.json',bytes:Buffer.from(JSON.stringify(response)+'\n')}]);await browser.click('#process-response-file');assert.ok(await browser.exists('#accept-proposal'));return response;}
...
try{
 ...
 await uploadValid();await browser.click('#accept-proposal');await browser.click('#confirm-stage-one');
