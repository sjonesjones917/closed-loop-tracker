...
# Replace obsolete tests that required prohibited hard-coded project-subject branches or legacy headings.
...
new="  }else if(!record.prompt.includes('Work too large for the actually available environment must not be represented as completed')||!record.prompt.includes('missing downstream tool possession is not the same as missing artifact-generation capability'))issues.push('ENVIRONMENT_LIMIT_RULE_MISSING');"
if old not in s: raise SystemExit('obsolete environment-limit heading assertion not found')
s=s.replace(old,new)
