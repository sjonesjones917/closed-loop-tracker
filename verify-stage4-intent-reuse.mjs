...
if(!appCoreSource.includes("function artifactControlMarkup(n,locked){if(n===4){const intake=engine.currentIntakeCoverageManifest(current),obligations=engine.currentObligationManifest(current);"))throw new Error('Stage 04 still falls through to the generic repeat-attachment control instead of canonical captured intent.');
if(!appCoreSource.includes('Do not reattach or re-enter the original intent file.'))throw new Error('Stage 04 operator surface does not state the capture-once rule.');
