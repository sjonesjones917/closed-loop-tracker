...
for(const altered of [{checkpointId:checkpoint.CHECKPOINT_ID,packageId:'OTHER',packageSha256:checkpoint.PACKAGE_SHA256},{checkpointId:checkpoint.CHECKPOINT_ID,packageId:checkpoint.PACKAGE_ID,packageSha256:'0'.repeat(64)},{checkpointId:'OTHER',packageId:checkpoint.PACKAGE_ID,packageSha256:checkpoint.PACKAGE_SHA256},{note:checkpoint.CHECKPOINT_ID+' '+checkpoint.PACKAGE_ID+' '+checkpoint.PACKAGE_SHA256}]){const bad=structuredClone(checkpointProject),e=bad.projectData.evidenceRecords.at(-1);e.fields.APPLICATION_EVIDENCE_CONTENT=JSON.stringify(altered);e.APPLICATION_EVIDENCE_CONTENT=e.fields.APPLICATION_EVIDENCE_CONTENT;engine.refreshRecordHashes(e,'evidenceRecords');assert.throws(()=>engine.recordPreDeliveryCheckpointExport(bad,{checkpointId:checkpoint.CHECKPOINT_ID,exportEvidenceIds:['EVID-EXPORT-1']}),/exact created checkpoint/i,'Every export evidence tuple field must match the actual package.');}
...
const exportProjectSha=globalThis.closedLoopHash.sha256Value(checkpointProject);assert.equal(engine.recordPreDeliveryCheckpointExport(checkpointProject,{checkpointId:checkpoint.CHECKPOINT_ID,exportEvidenceIds:['EVID-EXPORT-1']}).id,exported.id);assert.equal(globalThis.closedLoopHash.sha256Value(checkpointProject),exportProjectSha,'Repeating the export-custody command must not mutate its receipt or project.');
