...
function validateLedger(state,mutation=null){
  ...
  if(mutation==='skipped-stage'){s.stages['01'].status='NOT_STARTED';s.stages['02'].status='DONE';s.stages['02'].endCommit=s.startingMainCommit;s.stages['02'].testsActuallyRun=[{command:'intentional-invalid',result:'PASS',exitCode:0,evidence:'fixture'}];}
  if(mutation==='false-done-open-item'){s.stages['01'].status='DONE';s.stages['01'].endCommit=s.startingMainCommit;s.stages['01'].testsActuallyRun=[{command:'intentional-invalid',result:'PASS',exitCode:0,evidence:'fixture'}];s.stages['01'].openAcceptanceItems=['still open'];}
  if(mutation==='done-without-execution'){s.stages['01'].status='DONE';s.stages['01'].endCommit=s.startingMainCommit;s.stages['01'].testsActuallyRun=[];s.stages['01'].openAcceptanceItems=[];}
