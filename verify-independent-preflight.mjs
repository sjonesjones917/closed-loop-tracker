...
function base(jobId){
  ...
  p.projectData.semanticReviews.push({id:'SEM-APP-1',stage:5,active:true,scope:{...requirementScope},fields:{SEMANTIC_REVIEW_ID:'SEM-APP-1',REVIEWED_RECORD_IDS:['APP-1'],REVIEWED_HASHES:[],AUTHOR_CONTEXT_ID:'CTX-AUTHOR',REVIEWER_CONTEXT_ID:'CTX-REVIEWER',AUTHOR_RESERVATION_ID:'RES-AUTHOR',REVIEWER_RESERVATION_ID:'RES-REVIEWER',INDEPENDENCE_DETERMINATION:'APPLICATION_ESTABLISHED',REVIEW_QUESTION:'Does current applicability preserve the mandatory fixture obligation?',FINDING:'Applicability is supported for this controlled downstream fixture.',REASONING:'Independent semantic review fixture for the already-resolved Stage 05 prerequisite.',RESULT:'ACCEPTED',ACCEPTED_DISPOSITION:'ACCEPTED',RECONCILIATION_STATUS:'NOT_REQUIRED',GATE_EFFECT:'ALLOW_IF_OTHER_GATES_PASS'}});
  p.projectData.tests.push({id:'TEST-1',stage:6,active:true,scope:{...requirementScope},fields:{TEST_ID:'TEST-1',REQ_ID:'REQ-1',TARGET_PROPOSITION_IDS:['PROP-1'],SEMANTIC_COVERAGE_DISPOSITION:'EQUIVALENT',SEMANTIC_REVIEW_IDS:['REVIEW-1'],TEST_ROLE:'REQUIRED_PROOF',TEST_TYPE:'MEANING',EXECUTION_MODE:'INDEPENDENT_AGENT_REVIEW',REQUIRED_CAPABILITY:'independent semantic review',ARTIFACT_REQUIREMENTS:'NONE',EVIDENCE_TO_PRESERVE:'Independent per-run review evidence',STATUS:'READY',VERIFICATION_PHASE:'PREPRODUCT_ITERATION',EARLIEST_EXECUTABLE_STAGE:12,REQUIRED_BY_STAGE:12,PER_RUN_REQUIRED:true,FINAL_PRODUCT_REQUIRED:false,DELIVERY_REQUIRED:false,TARGET_AVAILABILITY_CONDITION:{phaseTarget:true}},relationships:{REQ_ID:'REQ-1'}});
  p.projectData.proofExpressions.push({id:'PROOF-1',stage:6,active:true,scope:{...requirementScope},fields:{PROOF_EXPRESSION_ID:'PROOF-1',TARGET_PROPOSITION_ID:'PROP-1',PROPOSED_EXPRESSION:{type:'LEAF',testId:'TEST-1'},NORMALIZED_EXPRESSION:{type:'LEAF',testId:'TEST-1'},SEMANTIC_EQUIVALENCE_DISPOSITION:'EQUIVALENT',ACCEPTED_SEMANTIC_REVIEW_IDS:['REVIEW-1']},relationships:{TARGET_PROPOSITION_ID:'PROP-1'}});
  return p;
}
