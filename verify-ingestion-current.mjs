...
assert(core&&schema&&engine&&prompts&&ingestion,'Ingestion runtime failed to load.');
assert(core.STAGES.length===30,'Exactly 30 stages are required.');
assert(schema.RESPONSE_SCHEMA==='closed-loop-stage-response/3','Current response schema must be /3.');

function project(jobId='JOB-INGESTION-CURRENT'){
  const p=core.createBlankState(jobId);
  Object.assign(p.job,{JOB_ID:jobId,JOB_TITLE:'Lifecycle-valid ingestion verification',EXACT_USER_OBJECTIVE_VERBATIM:'Verify raw-first fail-closed response ingestion without bypassing workflow prerequisites.',CURRENT_INPUT_VERSION:'INPUT-v001'});
  engine.ensureShape(p);engine.recalculate(p);return p;
}
function persistPrompt(p,stage,options={}){
  const pr={...prompts.buildPromptRecord(stage,p,options),generatedAt:new Date().toISOString(),iteration:p.job.CURRENT_ITERATION||'NOT APPLICABLE'};
  p.projectData.generatedPrompts.push(pr);return pr;
}
function intakeCapture(p){
  const manifest=prompts.buildPromptRecord(1,p).contextManifest.intakeCoverageManifest;
...
function makeStage1Complete(p){
  p.stages[1].agentData={EXACT_DELIVERABLE_REQUESTED:'Verified deliverable',ASSUMPTIONS:'NONE',UNKNOWN_INFORMATION:'NONE',INPUT_SET_CONTENTS:intakeCapture(p)};
  p.stages[1].acceptedData={...p.stages[1].agentData};p.stages[1].status='COMPLETE';p.stages[1].gate={complete:true,blocked:false,reasons:[]};return p;
}
function makeStage4Reachable(p){
  makeStage1Complete(p);
  p.stages[2].agentData={SOURCE_APPLICABILITY_DETERMINATION:'NO_APPLICABLE_EXTERNAL_SOURCE',KNOWN_CONTROLLING_SOURCES_EXAMINED:'TRUE',UNRESOLVED_CONTROLLING_CONFLICTS:'NONE'};
  p.stages[2].acceptedData={...p.stages[2].agentData};p.stages[2].status='COMPLETE';p.stages[2].gate={complete:true,blocked:false,reasons:[]};
  p.stages[3].agentData={ALL_KNOWN_CONTROLLING_SOURCES_EXAMINED:'TRUE',SECOND_CONFLICT_AND_EXCEPTION_PASS_COMPLETED:'TRUE',LATEST_PASS_NUMBER:1,NEW_MATERIAL_CATEGORY_FOUND_IN_LATEST_PASS:'FALSE',RESEARCH_GAPS_AND_BLOCKERS:'NONE'};
  p.stages[3].acceptedData={...p.stages[3].agentData};p.stages[3].status='COMPLETE';p.stages[3].gate={complete:true,blocked:false,reasons:[]};return p;
}
...
let negativeCount=0;
function rejectCase(name,mutate,expectedCode){
  const {p,pr,e}=stage2Fixture(`JOB-NEG-${name.replace(/[^A-Z0-9]/gi,'').toUpperCase()}`);const changed=mutate(e,p,pr);const candidate=changed===undefined?e:changed;const result=prepare(p,2,pr,candidate);
  assert(!result.validation.valid,`${name}: invalid response was accepted.`);
  if(expectedCode)assert(result.validation.issues.some(issue=>issue.code===expectedCode),`${name}: expected ${expectedCode}; got ${result.validation.issues.map(i=>i.code).join(', ')}.`);
  assert(result.project.projectData.acceptedChanges.length===0,`${name}: canonical state changed before acceptance.`);
  assert(result.project.projectData.rawResponses.length===1&&result.project.projectData.responseValidations.length===1,`${name}: raw response/validation was not preserved.`);
  negativeCount++;
}
...
// Reachable happy path: raw capture -> validation -> proposal -> explicit commit -> durable reload.
{
  ...
  assert(prepared.proposal?.status==='PENDING_OPERATOR_REVIEW','Valid response did not produce a pending proposal.');
  assert(prepared.project.projectData.acceptedChanges.length===0,'Canonical state mutated before acceptance.');
  const committed=ingestion.commit(prepared.project,prepared.proposal.proposalId,{operator:'INGESTION_REGRESSION'});p=committed.project;
  assert(p.projectData.acceptedChanges.length===1,'Accepted response did not create one canonical change.');
  assert(p.projectData.extractionManifests.length===1,'Accepted response did not create an extraction manifest.');
  const reloaded=JSON.parse(JSON.stringify(p));engine.ensureShape(reloaded);
...
// Attachment claims require matching application-known bytes identity.
{
  ...
  {const {p,pr,e}=make('JOB-ATTACHMENT-VALID'),result=prepare(p,2,pr,e,[exact]);assert(result.validation.valid,`Verified attachment rejected: ${JSON.stringify(result.validation.issues)}`);}
...
console.log(JSON.stringify({ingestionLifecycleValid:true,responseSchema:schema.RESPONSE_SCHEMA,rawFirst:true,atomicPrecommit:true,stage04ObligationClosure:true,attachmentByteBinding:true,semanticDuplicateIdempotency:true,totalNegativeCases:negativeCount},null,2));
