...
assert match
body=match.group(1)
if 'archiveMigrationPayload' not in body: body=body.replace('clearLegacy','archiveMigrationPayload,clearLegacy',1)
...
'''
s=s.replace(marker,helper+marker,1)
anchor='registerGeneratedPrompt,recordHumanInputVersion'
assert anchor in s
s=s.replace(anchor,'recordMigratedAcceptedChange,registerGeneratedPrompt,recordHumanInputVersion',1)
p.write_text(s)

p=Path('app-core.js');s=p.read_text()
# Original payload and stage records are quarantined through the project-store migration boundary.
...
# The deterministic Stage 01 migration is created by the engine.
...
new=f"engine.recordMigratedAcceptedChange(p,{inside});"
s=s[:old.start()]+new+s[old.end():]
# Enforce the source boundary now, before final verification.
