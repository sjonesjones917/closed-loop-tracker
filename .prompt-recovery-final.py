...
# workbook owns stage definitions, not a competing prompt implementation.
...
# Stage 01 must convert an infeasible implementation request into a feasible, confirmable deliverable rather than leave an ambiguous dead-end.
...
"If required repository, runtime, account, network, deployment, or tool access is absent, do not pretend implementation occurred; when a complete reliable implementation-ready specification, architecture, patch plan, acceptance package, or other self-contained substitute can satisfy the user's underlying objective, propose that feasible substitute as EXACT_DELIVERABLE_REQUESTED for human intent confirmation and state the execution limitation explicitly.")

# Global recovery semantics must agree with Stage 01 and Stage 21.
replace('prompt-engine.js',
...
# Refining an already accepted result must feed the exact reason into the next same-lane prompt and prompt identity.
...
# First-class semantic regression tests, not token-only acceptance.
