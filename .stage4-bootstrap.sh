...
git fetch origin e7e72ccd309e9a609eb489e353d5e246d505acb8
git merge --no-ff --no-commit -X ours e7e72ccd309e9a609eb489e353d5e246d505acb8
...
"""
if text.count(anchor)!=1: raise SystemExit(f'app-core importSeed anchor count={text.count(anchor)}')
...
b.write_text(browser.replace(old,new))
...
run_browser_verifier(){ script="$1"; for attempt in 1 2 3; do log="/tmp/${script}.${attempt}.log"; if node "$script" >"$log" 2>&1; then cat "$log"; return 0; fi; cat "$log"; if ! grep -Eq 'ECONNREFUSED 127\.0\.0\.1:[0-9]+' "$log"; then return 1; fi; test "$attempt" -lt 3 || return 1; done; }
...
git commit -m 'Fix Stage 04 canonical intent reuse across reloads'
git push origin HEAD:fix/stage4-canonical-reuse-proofed
