...
function replaceOnce(label, search, replacement) {
  if (typeof search === 'string') {
    ...
    if (first < 0 || first !== last) {
      throw new Error(`${label}: expected exactly one literal match; first=${first}, last=${last}`);
    }
    ...
    if (count !== 1) throw new Error(`${label}: expected exactly one regular-expression match; found ${count}`);
...
function validateProjectData(input){const project=ensureProjectShape(input);for(let n=1;n<=31;n++){if(project.stages[n-1].status!=='COMPLETE')throw Error(\`Retained project validation failed: Stage \\${n} is not COMPLETE.\`);validateStage(project,n)}return project}
