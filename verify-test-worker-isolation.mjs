...
function workerFixture(source=workerSource){
...
context.self=context;context.globalThis=context;vm.createContext(context);vm.runInContext(source,context,{filename:'test-worker.js'});bootstrapOpen=false;assert.equal(typeof listener,'function');return {context,listener,messages};
}
