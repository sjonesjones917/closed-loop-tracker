...
function amendedOperationContract(stage,operation){const base=s0.operationContract(stage,operation);if(!base)return null;const key=`${stage}:${operation}`,narrow=NARROW_SEMANTIC_OPERATION_KEYS.has(key),addedAgent=narrow?[]:(addedAgentCollections[stage]||[]),addedRead=stage>=5&&!narrow?completionReadCollections:[],addedApplication=stage===30?['operationReservations','proofObligations','deliveryRecords','deploymentManifests']:['operationReservations','proofObligations'],allowedStageData=REVIEW_ONLY_OPERATION_KEYS.has(key)?[]:(CONTRACTS[stage]?.allowedStageData||base.allowedStageData||[]),merged=Object.freeze({...base,readCollections:Object.freeze([...new Set([...(base.readCollections||[]),...addedRead])]),agentWritableCollections:Object.freeze([...new Set([...(base.agentWritableCollections||[]),...addedAgent])]),applicationCollections:Object.freeze([...new Set([...(base.applicationCollections||[]),...addedApplication])]),allowedStageData:Object.freeze([...allowedStageData])});if(Number(stage)!==29||String(operation)!=='CALCULATE_EVIDENCE_CHAINS')return merged;return Object.freeze({...merged,executorClass:'APPLICATION',acceptsExternalResponse:false,responseTypes:Object.freeze([]),acceptanceMode:'DIRECT_COMMAND',reservationRequired:false,completionPredicate:'STAGE_29_CALCULATE_EVIDENCE_CHAINS_COMPLETION',retryRule:'IDEMPOTENT_COMMAND',minimumInputBindingBasis:'APPLICATION_OBSERVED'});}
