...
addRequiredFamily('backupCheckpoints',{title:'Backup checkpoints',idField:'CHECKPOINT_ID',prefix:'CHECKPOINT',application:['CHECKPOINT_ID','PREVIOUS_CHECKPOINT_ID','EXPORT_EVENT_ID','PACKAGE_ID','PACKAGE_SHA256','PROJECT_REVISION','PROJECT_SHA256','ARTIFACT_MANIFEST_SHA256','CUSTODY_STATE','EXTERNAL_EVIDENCE_IDS','RESTORE_EVIDENCE_IDS','SCOPE','STATUS'],types:{PROJECT_REVISION:{valueType:'INTEGER'},EXTERNAL_EVIDENCE_IDS:{valueType:'REFERENCE_ARRAY'},RESTORE_EVIDENCE_IDS:{valueType:'REFERENCE_ARRAY'},SCOPE:{valueType:'OBJECT'},CUSTODY_STATE:{enumValues:['BACKUP_PACKAGE_GENERATED','BACKUP_EXPORT_ACTION_COMPLETED','EXTERNAL_COPY_CONFIRMED','RESTORE_TESTED_FROM_EXPORTED_COPY']}},relationships:{PREVIOUS_CHECKPOINT_ID:'backupCheckpoints'},commitPolicy:C.APPEND_ONLY});
...
// These objects live in the existing persistence adapter. Their full snapshots
// and external execution facts are recovery data, never stage handoff inputs.
const RECOVERY_STORAGE_CONTRACT=Object.freeze({schema:'closed-loop-recovery/1',maxCheckpoints:2048,maxCheckpointBytes:268435456,maxArtifactBytes:536870912,maxHistoryViews:8192,maxViewBytes:67108864,retention:'No automatic eviction. A full history blocks the next change; export it before starting a new project.'});
const EXTERNAL_ACTION_STORAGE_CONTRACT=Object.freeze({schema:'closed-loop-external-actions/1',maxActions:8192,maxBytes:33554432,states:Object.freeze(['STARTED','SUCCEEDED','FAILED','OUTCOME_UNCONFIRMED']),restoration:'Preserve execution facts across project versions; never replay an external action.'});
const RECOVERY_VIEW_NAMES=Object.freeze(['Overview','Project','Workflow','Records','Files','Release']);
const nestedStorage=[
 ['backupCheckpoints.recoveryHistory','backupCheckpoints','jobId','ATOMIC_ACTIVE_POINTER_AND_APPEND_ONLY_INDEX',['schema','jobId','activeId','activationRevision','restoration','entries','sessionStarts','checkpointBytes','partSizes','retainedArtifactIds','latestViews','viewCount','viewBytes']],
 ['backupCheckpoints.savedVersion','backupCheckpoints','versionId','IMMUTABLE',['schema','versionId','jobId','parentId','createdAt','label','project','projectSha256','artifacts','view','checkpointSha256']],
 ['backupCheckpoints.savedVersionPart','backupCheckpoints','sha256','IMMUTABLE_CONTENT_ADDRESSED',['schema','jobId','sha256','data']],
 ['backupCheckpoints.encodedVersion','backupCheckpoints','versionId','IMMUTABLE',['schema','versionId','jobId','parentId','createdAt','label','projectSha256','artifacts','view','checkpointSha256','projectEncoding','projectNode']],
 ['backupCheckpoints.historyEntry','backupCheckpoints','entryId','IMMUTABLE_VERSION_BINDING_REPLACEABLE_VIEW',['entryId','jobId','versionId','view','viewSha256']],
 ['backupCheckpoints.savedView','backupCheckpoints','PARENT_ENTRY_ID','CAPTURE_BEFORE_NAVIGATION',['stage','view','operation','run','scrollX','scrollY','forms','files']],
 ['backupCheckpoints.selectedFiles','backupCheckpoints','inputId','VIEW_OWNED',['inputId','files']],
 ['backupCheckpoints.selectedFile','backupCheckpoints','artifactId','IMMUTABLE',['artifactId','filename','mediaType','byteSize','sha256']],
 ['backupCheckpoints.recoveryArchive','backupCheckpoints','history.jobId','IMMUTABLE_EXPORT_VALIDATED_ATOMIC_MERGE',['history','checkpoints','parts','views']],
 ['deliveryAttempts.executionLedger','deliveryAttempts','jobId','APPEND_ONLY_FACTS_OUTSIDE_RESTORED_VERSIONS',['schema','jobId','actions']],
 ['deliveryAttempts.execution','deliveryAttempts','actionId','IMMUTABLE_BINDING_APPEND_ONLY_EVENTS',['actionId','requestId','jobId','authorizationId','projectRevision','versionId','plan','events','sha256']],
 ['deliveryAttempts.executionEvent','deliveryAttempts','sequence','IMMUTABLE_HASH_CHAIN',['sequence','previousSha256','state','result','deviceReportedTime','sha256']]
];
for(const [identity,parentFamily,idField,policy,properties] of nestedStorage){
 durable[identity]=Object.freeze({family:identity,parentFamily,idField,stage:'GLOBAL',policy,lifecyclePolicy:policy,producerPartitions:Object.freeze({human:[],humanDecision:[],agent:[],application:Object.freeze(properties)}),scope:'EXACT_PROJECT_AND_SAVED_VERSION',scopeDimensions:Object.freeze(['jobId','versionId']),relationships:Object.freeze({jobId:'PROJECT.JOB_ID',versionId:'backupCheckpoints.savedVersion'}),hashInclusion:'ALL_REGISTERED_PROPERTIES_EXCEPT_SELF_DIGEST',migrationBehavior:'VALIDATE_LEGACY_FULL_CHECKPOINTS; PRESERVE_RETAINED_EXTERNAL_RECEIPTS; REJECT_UNKNOWN_ENCODING',gateUse:'REQUIRED_BEFORE_COMMIT_OR_RESTORATION',implementationOwner:'project-store.js',agentFacing:false,closedProperties:Object.freeze(properties)});
 for(const name of properties){const key='STORAGE.'+identity+'.'+name;fields[key]=Object.freeze({name,path:'/storage/'+identity+'/'+name,producer:s.PRODUCER.APPLICATION,valueType:['stage','projectRevision','activationRevision','checkpointBytes','viewCount','viewBytes','scrollX','scrollY','sequence','byteSize'].includes(name)?'INTEGER':['entries','artifacts','forms','files','checkpoints','parts','views','actions','events'].includes(name)?'OBJECT_ARRAY':['data','projectNode'].includes(name)?'JSON':['project','view','history','restoration','sessionStarts','partSizes','latestViews','plan','result'].includes(name)?'OBJECT':name==='retainedArtifactIds'?'STRING_ARRAY':'STRING',cardinality:'ONE',requiredAtStage:null,writableOperation:'PERSISTENCE_APPLICATION_COMMAND',classification:'RECOVERY_OR_EXECUTION_RECORD',relationshipTarget:null,relationshipDirection:null,invalidationOwner:'project-store.js',enumValues:Object.freeze([]),nullable:['versionId','parentId','activeId','restoration','operation','run','result','previousSha256'].includes(name),requiredness:'REGISTERED_STORAGE_CONTRACT',scope:'EXACT_PROJECT_AND_SAVED_VERSION',scopeDimensions:Object.freeze(['jobId','versionId']),hashInclusion:'ALL_EXCEPT_SELF_DIGEST',normalizerIdentity:NO_NORMALIZER_ID,derivationIdentity:NO_DERIVATION_ID,migrationRule:'VALIDATE_SAVED_SCHEMA_BEFORE_ACTIVATION',agentWritable:false});}
}
...
// Specification 14.6: advisory reviews are explicitly non-gating. All other registered operations retain their prerequisite dependencies.
for(const [key,operation] of Object.entries(operations))operations[key]=Object.freeze({...operation,affectsDependentWork:key!=='27:ADVISORY_REVIEW'});
globalThis.closedLoopWorkflowSchema=Object.freeze({...s,RECOVERY_STORAGE_CONTRACT,EXTERNAL_ACTION_STORAGE_CONTRACT,RECOVERY_VIEW_NAMES,FIELD_REGISTRY:Object.freeze(fields),STAGE_OPERATION_REGISTRY:Object.freeze(operations),DURABLE_OBJECT_REGISTRY:Object.freeze(durable),normalizerRegistry:Object.freeze({identity:'closed-loop-normalizer-registry/1',entries:Object.freeze(normalizers)}),derivationRegistry:Object.freeze({identity:'closed-loop-derivation-registry/1',entries:Object.freeze(derivations)}),ATTACHMENT_SLOT_CONTRACT,validateAttachmentSlotDefinition,IDENTITY_ASSURANCE_VALUES,HUMAN_DECISION_PURPOSE_REGISTRY,identityAssuranceSatisfies,stage03ContractClosureVersion:VERSION});
})();
