...
ending="try{await main();}finally{await cleanup();}"
for name in ['verify-browser.mjs','verify-browser-extra.mjs']:
    p=Path(name);s=p.read_text();assert old in s;s=s.replace(old,new,1);assert ending in s;s=s.replace(ending,ending+'\nprocess.exit(0);',1);p.write_text(s)
