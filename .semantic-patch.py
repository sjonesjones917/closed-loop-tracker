...
def replace_once(path, old, new):
    ...
    if s.count(old)!=1:
        raise SystemExit(f'{path}: expected exactly one occurrence, found {s.count(old)} for {old[:100]!r}')
...
replace_once('prompt-engine.js',
  "Prefer primary, official, controlling sources when applicable.",
  "Prefer the most authoritative and reputable sources appropriate to the information type, prioritizing primary, official, and controlling sources when applicable. Quality, applicability, currency, and authority control over source count.")
replace_once('prompt-engine.js',
...
replace_once('prompt-engine.js',
  ...
  "function selectedCurrentRecords(state,collection,scopeRule={}){return engine.recordsForCurrentScope(state,collection,scopeRule);}\nfunction boundedCollection(state,collection,scopeRule={}){\n const current=selectedCurrentRecords(state,collection,scopeRule);if(!current.length)return 'NONE';\n return show({totalCurrent:current.length,records:current.map(record=>({id:recordId(record,collection),stage:record.stage??'UNKNOWN',scope:record.scope||{},fields:record.fields||record,relationships:record.relationships||{},contentSha256:record.contentSha256||record.sha256||'UNKNOWN'})),omitted:0,selectionRule:'All and only active records matching the current canonical project scope and explicit stage readCollections contract; large artifact bytes are referenced by canonical artifact identity.'});\n}")
replace_once('prompt-engine.js', "${show(open.slice(-20))}", "${show(open)}")
replace_once('prompt-engine.js',
...
replace_once('prompt-engine.js',
  ...
  "EXPLICIT USER REQUIREMENTS:\n${show(j.EXPLICIT_USER_REQUIREMENTS)}\n\nDESIRED / SUGGESTED SOURCE COUNT (optional; Stage 02 search target, never a quota):\n${show(j.DESIRED_SOURCE_COUNT)}\n\nAUTHORIZED BOUNDED CONTEXT FOR THIS STAGE")
replace_once('prompt-engine.js',
...
replace_once('prompt-engine.js',
  ...
  "readCollections:Object.fromEntries((opContract?.readCollections||schema.STAGE_CONTRACTS[stage].readCollections||[]).map(collection=>[collection,selectedCurrentRecords(state,collection,scope).map(record=>({id:recordId(record,collection),scope:record.scope||{},contentSha256:record.contentSha256||record.sha256||hash.sha256Value(record.fields||record)}))]))")
...
replace_once('workbook.js', "KNOWN_AUTHORITATIVE_SOURCES:'',AVAILABLE_TOOLS:''", "KNOWN_AUTHORITATIVE_SOURCES:'',DESIRED_SOURCE_COUNT:'',AVAILABLE_TOOLS:''")

replace_once('app-core.js', "['KNOWN_AUTHORITATIVE_SOURCES','Known authority supplied by user','textarea'],['AVAILABLE_TOOLS'", "['KNOWN_AUTHORITATIVE_SOURCES','Known authority supplied by user','textarea'],['DESIRED_SOURCE_COUNT','Desired / suggested source count (optional)','number'],['AVAILABLE_TOOLS'")
replace_once('app-core.js', "`<input data-job=\"${k}\" value=\"${esc(current.job[k]||'')}\">`", "`<input data-job=\"${k}\" type=\"${t==='number'?'number':'text'}\"${t==='number'?' min=\"1\" step=\"1\"':''} value=\"${esc(current.job[k]??'')}\">`")
replace_once('app-core.js', "async function saveJob(){const next=clone(current),changed=[];document.querySelectorAll('[data-job]').forEach(x=>{const key=x.dataset.job,value=x.value;if(next.job[key]!==value){next.job[key]=value;changed.push(key);}});", "async function saveJob(){const sourceCount=$('[data-job=\"DESIRED_SOURCE_COUNT\"]');if(sourceCount?.value!==''&&(!Number.isInteger(Number(sourceCount.value))||Number(sourceCount.value)<1)){sourceCount.focus();alert('Desired / suggested source count must be a positive whole number or left blank.');return;}const next=clone(current),changed=[];document.querySelectorAll('[data-job]').forEach(x=>{const key=x.dataset.job,value=key==='DESIRED_SOURCE_COUNT'&&x.value!==''?Number(x.value):x.value;if(next.job[key]!==value){next.job[key]=value;changed.push(key);}});")
replace_once('app-core.js', "knownAuthorities:next.job.KNOWN_AUTHORITATIVE_SOURCES,availableTools:next.job.AVAILABLE_TOOLS", "knownAuthorities:next.job.KNOWN_AUTHORITATIVE_SOURCES,desiredSourceCount:next.job.DESIRED_SOURCE_COUNT,availableTools:next.job.AVAILABLE_TOOLS")
replace_once('app-core.js', "KNOWN_AUTHORITATIVE_SOURCES:raw.userJobInput?.knownAuthorities||'',AVAILABLE_TOOLS:", "KNOWN_AUTHORITATIVE_SOURCES:raw.userJobInput?.knownAuthorities||'',DESIRED_SOURCE_COUNT:raw.userJobInput?.desiredSourceCount||'',AVAILABLE_TOOLS:")
replace_once('app-core.js', "<div class=\"button-row\"><button class=\"primary\" id=\"accept-proposal\">Accept response</button><button id=\"reject-proposal\">Reject response</button><button id=\"request-correction\">Request correction</button></div>", "<div class=\"field full\"><label>Correction guidance</label><textarea id=\"correction-reason\" placeholder=\"When requesting correction, describe exactly what the next response should add, correct, or clarify.\"></textarea></div><div class=\"button-row\"><button class=\"primary\" id=\"accept-proposal\">Accept response</button><button id=\"reject-proposal\">Reject response</button><button id=\"request-correction\">Request correction</button></div>")
replace_once('app-core.js', "async function rejectPendingProposal(requestCorrection=false){const p=pendingProposal();if(!p)return;const result=ingestion.reject(current,p.proposalId,{operator:'HUMAN_OPERATOR',reason:requestCorrection?'Correction requested after proposal review.':'Rejected after proposal review.',requestCorrection});", "async function rejectPendingProposal(requestCorrection=false){const p=pendingProposal();if(!p)return;const guidance=$('#correction-reason')?.value.trim()||'';if(requestCorrection&&!guidance){$('#correction-reason')?.focus();alert('Describe what the next response should correct or add.');return;}const result=ingestion.reject(current,p.proposalId,{operator:'HUMAN_OPERATOR',reason:requestCorrection?guidance:'Rejected after proposal review.',requestCorrection});")

replace_once('workflow-engine.js', "const blockers=openBlockers(project);const defects=unresolvedMaterialDefects(project);", "const blockers=openBlockers(project);const currentIterationId=recordId(latestIteration(project,[19,17,10]),'iterations');const defects=unresolvedMaterialDefects(project).filter(r=>!currentIterationId||!r.scope?.iterationId||String(r.scope.iterationId)===String(currentIterationId));")
...
if marker not in s: raise SystemExit('verify-prompt-semantics marker missing')
insert="""
...
if marker not in s: raise SystemExit('verify-complete final marker missing')
insert="""
// Release evaluates current blocking defects, while permanent defect history remains preserved.
{
...
p=Path('verify-browser.mjs'); s=p.read_text(); marker="console.log('step:stage01-clarification');"
if marker in s:
    p.write_text(s.replace(marker,"assert(await evalValue(cdp,`Boolean(document.querySelector('[data-job=\\\"DESIRED_SOURCE_COUNT\\\"]'))`),'Optional desired source-count input is missing from Project UI.');\n  "+marker,1))
