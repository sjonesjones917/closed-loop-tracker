...
engine_block = r"""function acceptedEmptyUniverseReview(project,metricId,scopeRule=currentScope(project)){
...
function stage18RegressionIsDue(record){
  ...
  return Number.isFinite(earliest)&&Number.isFinite(required)&&earliest<=18&&required<=18;
}
...
function recordConvergenceDetermination(project){
  ...
  const record=commandRecord(project,'convergenceRecords',{ITERATION_ID:metrics.iterationId||'UNKNOWN',REQUIREMENT_COVERAGE:metrics.requirementCoverage===null?'UNKNOWN':String(metrics.requirementCoverage),VERIFICATION_COVERAGE:metrics.verificationCoverage===null?'UNKNOWN':String(metrics.verificationCoverage),REGRESSION_SUCCESS:metrics.regressionSuccess===null?'UNKNOWN':String(metrics.regressionSuccess),CRITICAL_DEFECT_COUNT:String(metrics.criticalDefects),MAJOR_DEFECT_COUNT:String(metrics.majorDefects),MANDATORY_UNRESOLVED_UNKNOWN_COUNT:String(metrics.mandatoryUnresolvedUnknowns),CORRECTNESS_AFFECTING_CONTRADICTION_COUNT:String(metrics.contradictions),CORRECTNESS_AFFECTING_AMBIGUITY_COUNT:String(metrics.ambiguities),UNEXPLAINED_CORRECTNESS_AFFECTING_VARIANCE_COUNT:String(metrics.unexplainedVariance),CONVERGED:metrics.converged?'TRUE':'FALSE'},{stage:18,source:'APPLICATION_DERIVATION',scope});
  record.convergenceEvidenceSha256=evidenceSha256;record.closedMetrics={requirement:metrics.requirementMetric,verification:metrics.verificationMetric,regression:metrics.regressionMetric};refreshRecordHashes(record,'convergenceRecords');addHistory(project,'CONVERGENCE_DETERMINATION_CALCULATED',{stage:18,recordId:recordId(record,'convergenceRecords'),iterationId:metrics.iterationId||null,converged:metrics.converged,convergenceEvidenceSha256:evidenceSha256});recalculate(project);return record;
}"""

replace_between('workflow-engine.js', 'function coverageMetrics(project,iterationIdOverride=null){', 'function releaseVerificationTrust', engine_block)
...
replace_once('workflow-engine.js', old_gate, new_gate)

replace_once(
    'workflow-engine.js',
    "TOTAL_STILL_APPLICABLE_REGRESSION_TESTS:regs.length,SUCCESSFUL_REGRESSION_TESTS:Math.round(regs.length*convergence.regressionSuccess),REGRESSION_TEST_SUCCESS:convergence.regressionSuccess",
    "TOTAL_STILL_APPLICABLE_REGRESSION_TESTS:convergence.regressionMetric.denominator,SUCCESSFUL_REGRESSION_TESTS:convergence.regressionMetric.numerator,REGRESSION_TEST_SUCCESS:convergence.regressionSuccess"
)
replace_once('workflow-engine.js',"const ACTION_TYPES=Object.freeze(['RUN_APP_TESTS','CALCULATE_RELEASE'","const ACTION_TYPES=Object.freeze(['RUN_APP_TESTS','CALCULATE_CONVERGENCE','CALCULATE_RELEASE'")
...
replace_once('workflow-engine.js', stage19_marker, stage18_action + stage19_marker)
replace_once('workflow-engine.js',"recordHumanInputVersion,recordStageConfirmation,recordReleaseDetermination,acceptedControlEvents","recordHumanInputVersion,recordStageConfirmation,closedMetricFromUniverse,recordConvergenceDetermination,recordReleaseDetermination,acceptedControlEvents")

replace_once('app-core.js',"RUN_APP_TESTS:'Application',CALCULATE_RELEASE:'Application'","RUN_APP_TESTS:'Application',CALCULATE_CONVERGENCE:'Application',CALCULATE_RELEASE:'Application'")
replace_once('app-core.js',"action.actionType==='RUN_APP_TESTS'?`<button class=\"primary\" id=\"run-native-tests\" type=\"button\">${esc(action.primaryButton)}</button>`:action.actionType==='CALCULATE_RELEASE'","action.actionType==='RUN_APP_TESTS'?`<button class=\"primary\" id=\"run-native-tests\" type=\"button\">${esc(action.primaryButton)}</button>`:action.actionType==='CALCULATE_CONVERGENCE'?`<button class=\"primary\" id=\"calculate-stage18-convergence\" type=\"button\">${esc(action.primaryButton)}</button>`:action.actionType==='CALCULATE_RELEASE'")
replace_once('app-core.js',"if($('#run-native-tests'))$('#run-native-tests').onclick=()=>{if(Number(current.activeStage)!==22)","if($('#calculate-stage18-convergence'))$('#calculate-stage18-convergence').onclick=async()=>{try{if(Number(current.activeStage)!==18)throw new Error('Convergence calculation belongs only to Stage 18.');const next=clone(current);engine.recordConvergenceDetermination(next);await persistReplacement(next);announce('convergence determination calculated');render();requestAnimationFrame(()=>$('#next-required-action')?.focus());}catch(error){announce('convergence calculation blocked');alert(error.message||error);}};\nif($('#run-native-tests'))$('#run-native-tests').onclick=()=>{if(Number(current.activeStage)!==22)")
replace_once('app-core.js',"17:'The corrected candidate is rerun through the complete ten-execution verification loop.',19:'The unchanged final candidate","17:'The corrected candidate is rerun through the complete ten-execution verification loop.',18:'The application calculates convergence only from current due preproduct requirements, verification, regressions, contradictions, defects, unknowns, ambiguity, and unexplained variance. No external response or human approval sets this calculation.',19:'The unchanged final candidate")

replace_once('verify-full-cycle.mjs',"data(18,{stageData:{RETURN_STAGE_FOR_EACH_FAILURE:'NONE'}});complete(18);","assert(engine.acceptedChanges(p,18).length===0,'Stage 18 convergence must not depend on an external accepted proposal.');engine.recordConvergenceDetermination(p);complete(18);")
replace_once('verify-definition-of-done-invariants.mjs',"['matrix-coverage',engineSource.includes('verificationCoverage:matrix.coverage')]","['matrix-coverage',engineSource.includes(\"verificationMetric=closedMetricFromUniverse(project,{metricId:'stage18.verificationCoverage'\")&&engineSource.includes('verificationCoverage:verificationMetric.value')]")
...
if 'node verify-convergence.mjs' not in workflow:
    ...
    if workflow.count(marker)!=1:
        raise SystemExit(f'pages.yml: expected one corrected-iteration marker, found {workflow.count(marker)}')
    pages.write_text(workflow.replace(marker,marker+'\n          node verify-convergence.mjs',1))

Path('verify-convergence.mjs').write_text(r"""import fs from 'node:fs';import vm from 'node:vm';import assert from 'node:assert/strict';
...
assert.equal(schema.STAGE_OPERATION_REGISTRY['18:COMPLETE'].executorClass,'APPLICATION','Stage 18 COMPLETE is not application-executed.');
assert.equal(schema.STAGE_OPERATION_REGISTRY['18:COMPLETE'].acceptsExternalResponse,false,'Stage 18 COMPLETE still accepts an external response.');
assert.equal(schema.RECORD_SCHEMAS.convergenceRecords.commitPolicy,schema.COLLECTION_POLICIES.APPLICATION_DERIVED,'Convergence records are not application-derived.');
assert.deepEqual(schema.RECORD_SCHEMAS.convergenceRecords.required,[],'Application-derived convergence incorrectly requires agent explanatory fields.');
assert.equal(typeof engine.recordConvergenceDetermination,'function','Stage 18 lacks its application-owned convergence command.');
...
assert.equal(empty.requirementMetric.disposition,'BLOCKED','Empty mandatory-requirement universe passed without independent EMPTY_UNIVERSE evidence.');
assert.equal(empty.regressionMetric.disposition,'BLOCKED','Empty regression universe passed without independent EMPTY_UNIVERSE evidence.');
assert(empty.requirementMetric.reasons.includes('EMPTY_UNIVERSE_REVIEW_REQUIRED'),'Empty requirement universe lacks its explicit rejection reason.');
assert(empty.regressionMetric.reasons.includes('EMPTY_UNIVERSE_REVIEW_REQUIRED'),'Empty regression universe lacks its explicit rejection reason.');
assert.equal(engine.gate(18,blank).complete,false,'Invalid empty convergence fixture passed Stage 18.');
assert.equal(engine.operationalNextAction(blank,18).actionType,'CALCULATE_CONVERGENCE','Stage 18 operator path still requests an external response instead of application calculation.');
await import('./verify-full-cycle.mjs');
console.log('verify-convergence: PASS');
""")
print('Stage 21 applicator completed.')
