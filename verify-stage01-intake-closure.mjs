...
assert(prompt.prompt.includes('EXTRACTED_RELEVANT_INFORMATION|RETAINED_AS_CONTEXT|NO_PROJECT_RELEVANT_INFORMATION|UNRESOLVED_HUMAN_AUTHORITY|LATER_RESOLVABLE|INACCESSIBLE_OR_BLOCKED'),'Prompt 01 does not publish the controlling six disposition values.');
...
const blocked=structuredClone(capture);const artifactIndex=manifest.units.findIndex(unit=>unit.kind==='SUPPLIED_MATERIAL');if(artifactIndex>=0){blocked.units[artifactIndex]={sourceUnitId:manifest.units[artifactIndex].unitId,sourceRawValueSha256:manifest.units[artifactIndex].rawValueSha256,disposition:'INACCESSIBLE_OR_BLOCKED',reason:'Required supplied bytes were not inspected.',extractedStatements:[]};assert(!engine.evaluateIntakeAccounting(p,{capture:JSON.stringify(blocked)}).complete||engine.evaluateIntakeAccounting(p,{capture:JSON.stringify(blocked)}).parsed.units[artifactIndex].disposition==='INACCESSIBLE_OR_BLOCKED','Blocked supplied material disposition was not preserved.');}
...
console.log(JSON.stringify({stage01IntakeClosure:true,artifactIdentityBound:true,currentManifestBound:true,incompleteAccountingRejected:true,controllingDispositionEnum:true}));
