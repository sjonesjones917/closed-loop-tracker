...
/* One-time migration transforms product/runtime source only. Acceptance verifiers are independent evidence and MUST NOT be rewritten by the implementation transform. */
const sourceFiles=rootFiles.filter(file=>/\.(?:js|mjs|json|md)$/.test(file)&&file!=='AUTHORIZED_OPERATION_01.txt'&&!file.startsWith('verify-')&&!file.startsWith('upgrade-v3')&&!file.startsWith('v3-upgrade-'));
...
/* Preserve the demonstrated small assertion primitives while keeping the language closed and declarative. */
{
...
/* Add current verifiers to broad syntax/proof runners where explicitly enumerated. */
for(const file of ['verify.mjs']){
  if(!fs.existsSync(path.join(root,file)))continue;
  let text=read(file);
  text=text.replace("'verify-test-runtime.mjs'", "'verify-test-runtime.mjs','verify-test-runtime-v3.mjs','verify-v3-contract.mjs','verify-v3-migration.mjs','verify-v3-definition-of-done.mjs'");
  write(file,text);
}

execFileSync(process.execPath,['build-test-project.mjs'],{cwd:root,stdio:'inherit'});
console.log(JSON.stringify({schemaUpgrade:true,responseUpgrade:true,executableKind:'TEST_IR',migrationLayer:true,workerRuntimeOrder:true,pagesWorkflowUpdated:true,transformedFiles:sourceFiles.length}));
