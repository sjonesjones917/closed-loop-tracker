...
async function probeResult(){return browser.evaluate(`(()=>{const key=[...document.querySelectorAll('#mobile-acceptance-panel .record-key')].find(node=>node.textContent==='Capability probe');return key?.parentElement.innerText||'';})()`);}
try{
...
}catch(error){cases.push({name:'journey failure',result:'FAIL',message:error.stack});console.error(error);process.exitCode=1;}
