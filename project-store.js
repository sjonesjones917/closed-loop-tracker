...
async function removeProjectIndexed(projectsOrJobId){const jobId=String(projectsOrJobId||''),db=await openDatabase(),tx=db.transaction([PROJECTS,ARTIFACTS],'readwrite');tx.objectStore(PROJECTS).delete(jobId);const artifacts=await request(tx.objectStore(ARTIFACTS).getAll());for(const a of artifacts)if(a.jobId===jobId)tx.objectStore(ARTIFACTS).delete(a.artifactId);await complete(tx);return true;}
...
async function putArtifact({artifactId,jobId,blob,filename,mediaType,lineage={}}){
  ...
  fault('during-artifact-blob-write');const id=String(artifactId),owner=String(jobId),byteSize=blob.size,sha256=await hash.sha256Bytes(await blob.arrayBuffer()),db=await openDatabase(),tx=db.transaction(ARTIFACTS,'readwrite'),store=tx.objectStore(ARTIFACTS);let prior=null;
  ...
  const verified=await getArtifact(id);if(!verified||String(verified.jobId)!==owner||verified.byteSize!==byteSize||await hash.sha256Bytes(await verified.blob.arrayBuffer())!==sha256){const cleanup=await openDatabase(),cleanupTx=cleanup.transaction(ARTIFACTS,'readwrite');const row=await request(cleanupTx.objectStore(ARTIFACTS).get(id));if(row&&String(row.jobId)===owner)cleanupTx.objectStore(ARTIFACTS).delete(id);await complete(cleanupTx);throw new Error('Artifact byte read-back verification failed; the unverified artifact was removed.');}return verified;
}
...
async function deleteArtifact(artifactId,jobId){const id=String(artifactId),owner=String(jobId||''),db=await openDatabase(),tx=db.transaction(ARTIFACTS,'readwrite'),store=tx.objectStore(ARTIFACTS);try{const row=await request(store.get(id));if(!row){await complete(tx);return false;}if(owner&&String(row.jobId)!==owner)throw storageError(`Artifact ${id} belongs to another project and was not deleted.`,'CROSS_PROJECT_ARTIFACT_DELETE');store.delete(id);await complete(tx);return true;}catch(error){try{tx.abort();}catch{}throw error;}}
...
async function importPackage(blob){
  ...
  try{
    const prior=await request(projects.get(id)),currentRevision=Number(prior?.revision||0),next=clone(project);next.revision=currentRevision+1;delete next.projectSha256;const digest=projectSha256(next);const existingArtifacts=await request(artifacts.getAll()),incomingIds=new Set(verifiedArtifacts.map(a=>String(a.artifactId)));
...
globalThis.closedLoopProjectStore=Object.freeze({archiveMigrationPayload,version:'closed-loop-project-store/2',DB_NAME,DB_VERSION,stores:Object.freeze({projects:PROJECTS,artifacts:ARTIFACTS,meta:META}),STORE_KEY,LEGACY_KEYS,clone,projectIdentity,projectSha256,openDatabase,ready,readAll,writeAll,writeProject,replaceProject,transact,removeProject,putArtifact,getArtifact,deleteArtifact,listArtifacts,exportPackage,importPackage,storageHealth,metaGet,metaPut,clearLegacy});
})();