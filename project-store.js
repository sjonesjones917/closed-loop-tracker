...
const RECOVERY_CONTRACT=Object.freeze({schema:'closed-loop-recovery/1',maxCheckpoints:2048,maxCheckpointBytes:268435456,maxArtifactBytes:536870912,maxHistoryViews:8192,maxViewBytes:67108864,retention:'No automatic eviction. A full history blocks the next change; export it before starting a new project.'});
...
const RECOVERY_VIEWS=Object.freeze(['Overview','Project','Workflow','Records','Files','Release']);
...
async function metaPut(key,value,tx=null){if(/^(projectCheckpoint:|projectCheckpointPart:|projectRecovery:|projectHistoryView:)/.test(String(key)))throw storageError('Recovery metadata can only be changed by its authoritative transaction.','RECOVERY_METADATA_PROTECTED');const own=tx||await openTransaction(META,'readwrite');own.objectStore(META).put({key,value,updatedAt:now()});if(!tx)await complete(own);return value;}
...
globalThis.closedLoopProjectStore=Object.freeze({readHistoryViewFiles,HANDOFF_ARCHIVE_PROFILE,EXTERNAL_LAUNCHER,deterministicHandoffArchive,previewProjectChange,persistPromptContextFiles,readPromptContextFile,archiveMigrationPayload,RECOVERY_CONTRACT,beginRecoverySession,readRecoveryHistory,readCheckpoint,restoreVersion,saveHistoryView,readHistoryView,version:'closed-loop-project-store/2',DB_NAME,DB_VERSION,stores:Object.freeze({projects:PROJECTS,artifacts:ARTIFACTS,meta:META}),STORE_KEY,LEGACY_KEYS,clone,projectIdentity,projectSha256,validateProjectIntegrity,openDatabase,ready,readAll,readProject,listProjectSummaries,writeAll,writeProject,replaceProject,transact,removeProject,putArtifact,getArtifact,deleteArtifact,listArtifacts,verifyProjectArtifacts,createExecutionPackage,exportPackage,importPackage,stageResponseFile,readStagedResponseFile,removeStagedResponseFile,storageHealth,metaGet,metaPut,clearLegacy});
})();
;(()=>{
'use strict';
