...
const stage3Pattern=/data\(3,\{stageData:\{[^}]*\}\}\);complete\(3\);/;
if(stage3Pattern.test(s))s=s.replace(stage3Pattern,"data(3,{stageData:{EXCEPTIONS_AND_EDGE_CONDITIONS:'NONE',CONFLICTING_OR_INVALIDATING_MATERIAL:'NONE',RESEARCH_GAPS_AND_BLOCKERS:'NONE'}});complete(3);");
for(const forbidden of ['ALL_KNOWN_CONTROLLING_SOURCES_EXAMINED','SECOND_CONFLICT_AND_EXCEPTION_PASS_COMPLETED','LATEST_PASS_NUMBER','NEW_MATERIAL_CATEGORY_FOUND_IN_LATEST_PASS']){
 const line=s.match(/data\(3,\{stageData:\{[^}]*\}\}\);complete\(3\);/)?.[0]||'';
 if(line.includes(forbidden))throw new Error('Full-cycle Stage 03 response still attempts to write application-owned '+forbidden);
}
...
console.log('Full-cycle Stage 01 response bound to exact prompt manifest; application-owned Stage 03 fields removed; Stage 04 accounting aligned.');