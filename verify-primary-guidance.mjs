...
const source=fs.readFileSync(process.env.APP_SOURCE||'app-core.js','utf8'),pages=[];
for(const stage of core.STAGES){
 for(const operation of schema.STAGE_CONTRACTS[stage.number].operations||['COMPLETE']){
  ...
  pages.push({stage:stage.number,operation,html:appMarkup(globalThis,project,{source,operations:{[stage.number]:typeof operation==='string'?operation:operation.id||operation.operation}})});
 }
}
...
console.log(JSON.stringify({synthetic:true,actualBrowser:false,source:process.env.APP_SOURCE||'app-core.js',expected:'Every stage and available operation keeps internal identities out of primary guidance; exact instruction bindings and diagnostic disclosures remain available',cases},null,2));
