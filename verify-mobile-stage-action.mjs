...
async function click(cdp,selector){await waitFor(cdp,`document.querySelector('#app')?.getAttribute('aria-busy')!=='true'`);await clickVisibleControl(cdp,selector);await waitFor(cdp,`document.querySelector('#app')?.getAttribute('aria-busy')!=='true'`);}
async function fill(cdp,selector,value){await waitFor(cdp,`document.querySelector('#app')?.getAttribute('aria-busy')!=='true'`);await fillVisibleControl(cdp,selector,value);await waitFor(cdp,`document.querySelector('#app')?.getAttribute('aria-busy')!=='true'`);}
...
async function openStage(cdp,stage){await click(cdp,'[data-view="Workflow"]');await fill(cdp,'#stage-picker',String(stage));await waitFor(cdp,`document.querySelector('.stage-hero h2')?.textContent.startsWith('Stage ${String(stage).padStart(2,'0')} — ')`);}
...
async function main(){
  ...
  await cdp.ready;await cdp.send('Runtime.enable');await cdp.send('Page.enable');await cdp.send('Log.enable');
  ...
  const errors=cdp.events.filter(event=>event.method==='Runtime.exceptionThrown'||event.method==='Page.javascriptDialogOpening'||(event.method==='Log.entryAdded'&&['error','assert'].includes(event.params?.entry?.level)));assert(errors.length===0,`Browser/runtime errors: ${JSON.stringify(errors)}`);
  console.log(JSON.stringify({mobileStageActionRegression:true,runtimeErrors:errors.length,widths:[320,393],longFilenameWrapped:true,stateAndActionExplicit:true,primaryActionReachable:true,promptCssDimensionsChecked:true,visualBaselineAcceptance:false,horizontalOverflow:false,mobileCapabilityEvidence:'verify-mobile-capability-journey.mjs performs actual export, selection, and restore'}));
  cdp.close();
}
