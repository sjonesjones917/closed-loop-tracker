...
async function click(cdp,selector,timeout=20000){await waitFor(cdp,`document.querySelector('#app')?.getAttribute('aria-busy')!=='true'`,timeout);assert(await evaluate(cdp,`(()=>{const node=document.querySelector(${JSON.stringify(selector)});if(!node||node.disabled)return false;node.click();return true})()`),`Missing clickable ${selector}`);await waitFor(cdp,`document.querySelector('#app')?.getAttribute('aria-busy')!=='true'`,timeout);}
...
async function main(){
  ...
  await click(cdp,'#project-actions-toggle');await click(cdp,'#export-project',60000);await waitFor(cdp,`__fileDownloads.length===2`,60000);
